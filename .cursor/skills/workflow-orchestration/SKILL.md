...
description: Execute non-trivial work using plan-first, verify-first, and lessons-fed defaults. Use when running 3+ step tasks, architectural work, or anything that may require re-planning.
---

# Workflow Orchestration

Executable checklist for plan-first, verify-first execution. Use for any non-trivial task (3+ steps, architectural decisions, multi-stage verification).
...
| Optional todo mirror | `tasks/todo.md` (MAS repo) | Lightweight operational summary; can be generated from `.cursor/plans` |
| Optional lessons | `tasks/lessons.md` (MAS repo) | Lessons learned; appended by pattern; pruned periodically |

**Relationship:** `.cursor/plans` and `docs/` are primary. `tasks/todo.md` and `tasks/lessons.md` are optional mirrors—use them only if they add operational value. Do not replace the stronger systems.
...
- Read `.cursor/CURSOR_DOCS_INDEX.md` and task-relevant docs. For new plans: read last 72 hours of docs per `read-recent-docs-before-planning.mdc`.
...
- See `subagents-must-test-before-complete.mdc`.
...
- See `plan-and-task-completion-docs.mdc` and `create-dated-document` skill.
...
- Use `myca-autonomous-operator` for lesson extraction and rule hardening.
