...
PATCHED VERSION - Feb 13, 2026:
- Line 32: Auto-convert weight_scb to float instead of erroring
- Fixes RuntimeError when using q8 models with TORCHDYNAMO_DISABLE=1
"""
...
class QLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        # PATCH: Auto-convert to float if needed (handles bfloat16 cast after init)
        if state.SCB.dtype != torch.float:
            state.SCB = state.SCB.float()
