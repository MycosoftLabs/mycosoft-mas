#!/usr/bin/env python3
"""Execute a single command over SSH using VM creds from .credentials.local."""
...
def main() -> int:
    if len(sys.argv) < 3:
        print("Usage: _remote_exec.py <host> <command...>")
        return 2
    host = sys.argv[1]
    cmd = " ".join(sys.argv[2:])
    pw = load_password()
    ...
    try:
        _stdin, stdout, stderr = ssh.exec_command(cmd, timeout=1800)
        code = stdout.channel.recv_exit_status()
        out = (stdout.read() + stderr.read()).decode("utf-8", errors="replace")
        safe = out.strip().encode("ascii", errors="replace").decode("ascii", errors="replace")
        print(safe)
        return code
    finally:
        ssh.close()
