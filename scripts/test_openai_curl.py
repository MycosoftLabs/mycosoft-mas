#!/usr/bin/env python3
"""Test OpenAI API directly from VM using curl"""
...
stdin, stdout, stderr = client.exec_command('docker exec myca-orchestrator-new printenv OPENAI_API_KEY', timeout=10)
openai_key = stdout.read().decode('utf-8', errors='replace').strip()
...
cmd = f'''curl -s 'https://api.openai.com/v1/chat/completions' \\
  ...
  -d '{{"model":"gpt-4o-mini","max_tokens":100,"messages":[{{"role":"user","content":"Say hello in one sentence."}}]}}' \\
  --max-time 30'''

print("\nTesting OpenAI API directly via curl...")
