#!/usr/bin/env python3
"""Test Groq API directly from VM using curl"""
...
stdin, stdout, stderr = client.exec_command('docker exec myca-orchestrator-new printenv GROQ_API_KEY', timeout=10)
groq_key = stdout.read().decode('utf-8', errors='replace').strip()
...
cmd = f'''curl -s 'https://api.groq.com/openai/v1/chat/completions' \\
  ...
  -d '{{"model":"llama-3.3-70b-versatile","max_tokens":100,"messages":[{{"role":"user","content":"Say hello in one sentence."}}]}}' \\
  --max-time 30'''

print("\nTesting Groq API directly via curl...")
