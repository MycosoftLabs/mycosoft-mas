...
try:
    ...
    out, err = run(ssh, "PGPASSWORD='REDACTED_VM_SSH_PASSWORD' psql -U mycosoft -d mindex -h localhost -c \"SELECT schemaname, tablename FROM pg_tables WHERE schemaname NOT IN ('pg_catalog','information_schema') ORDER BY schemaname, tablename;\" 2>/dev/null")
    if not out:
    ...
    # Check taxa count
    out, err = run(ssh, "PGPASSWORD='REDACTED_VM_SSH_PASSWORD' psql -U mycosoft -d mindex -h localhost -c \"SELECT COUNT(*) as taxa_count FROM taxa;\" 2>/dev/null")
    if not out:
        out, err = run(ssh, "sudo -u postgres psql -d mindex -c \"SELECT COUNT(*) as taxa_count FROM taxa;\" 2>/dev/null")
    print(f"\n[TAXA COUNT]\n{out or err or 'Could not query'}")

    # Check sample taxa data
    out, err = run(ssh, "PGPASSWORD='REDACTED_VM_SSH_PASSWORD' psql -U mycosoft -d mindex -h localhost -c \"SELECT id, scientific_name, common_name, rank, source, image_url IS NOT NULL as has_image FROM taxa LIMIT 10;\" 2>/dev/null")
    if not out:
    ...
    for table in ["observations", "compounds", "genomes", "traits", "external_ids", "synonyms"]:
        out, _ = run(ssh, f"PGPASSWORD='REDACTED_VM_SSH_PASSWORD' psql -U mycosoft -d mindex -h localhost -c \"SELECT COUNT(*) FROM {table};\" 2>/dev/null")
        if not out:
