...
class HazardPredictor(BasePredictor):
    ...
    async def predict_positions(
        self, state: EntityState, from_time: datetime, to_time: datetime, resolution_seconds: int
        ...
        if hazard_type == "earthquake":
            return await self._predict_aftershocks(state, from_time, to_time, resolution_seconds)
            ...
            return await self._predict_storm_track(state, from_time, to_time, resolution_seconds)
        elif hazard_type == "tsunami":
            return await self._predict_tsunami(state, from_time, to_time, resolution_seconds)
        elif hazard_type == "volcano":
            return await self._predict_ash_cloud(state, from_time, to_time, resolution_seconds)
        else:
    ...
    async def _predict_aftershocks(
        ...
        while current_time <= to_time:
            ...
            aftershock_mag = random.uniform(max(2.0, mainshock_magnitude - 3), max_aftershock_mag)
    ...
    async def _predict_wildfire_spread(
        ...
        while current_time <= to_time:
            ...
            spread_rate * resolution_seconds * 0.1
    ...
    async def _predict_ash_cloud(
        ...
        while current_time <= to_time:
            ...
            current_height = max(
                1000, eruption_height_m - descent_rate * dt_hours  # Minimum height
            )
