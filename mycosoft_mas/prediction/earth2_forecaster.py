...
class Earth2Forecaster:
    ...
    async def initialize(self):
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.gpu_gateway_url}/health", timeout=5.0)
    ...
    async def _fetch_storm_tracks(
        self, bounds: Tuple[float, float, float, float], from_time: datetime, to_time: datetime
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_synthetic_weather(
        self, location: GeoPoint, from_time: datetime, to_time: datetime, resolution_hours: int
    ) -> List[Dict[str, Any]]:
        ...
        while current_time <= to_time:
            ...
            precipitation = (
                precip_prob * random.uniform(0, 10) if random.random() < precip_prob else 0
            )
            ...
            forecasts.append(
                {
                    ...
                    "uv_index": max(
                        0, round(8 * math.sin(math.radians(hour * 7.5)) + random.uniform(-1, 1))
                    ),
                    "model": "synthetic",
                    "source": "development",
                }
