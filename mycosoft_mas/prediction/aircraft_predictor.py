...
class AircraftPredictor(BasePredictor):
    ...
    async def get_current_state(self, entity_id: str) -> Optional[EntityState]:
        ...
        if self.timeline_cache:
            try:
                from mycosoft_mas.cache import EntityType as CacheEntityType
    ...
    async def _predict_from_flight_plan(
        ...
        if not route or not route.waypoints:
            ...
            return await self._predict_from_vector(state, from_time, to_time, resolution_seconds)
        ...
        while current_time <= to_time:
            ...
            while elapsed < time_to_wp and current_time <= to_time:
                ...
                if next_wp.position.altitude:
                    pos.altitude = current_alt + fraction * (
                        next_wp.position.altitude - current_alt
                    )
                else:
    ...
    def _parse_flight_plan(self, flight_plan: Optional[Dict]) -> Optional[Route]:
        ...
        return Route(
            waypoints=waypoints,
            departure_time=(
                datetime.fromisoformat(flight_plan["departure"])
                if flight_plan.get("departure")
                else None
            ),
            arrival_time=(
                datetime.fromisoformat(flight_plan["arrival"])
                if flight_plan.get("arrival")
                else None
            ),
        )
