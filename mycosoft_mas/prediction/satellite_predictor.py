...
class SatellitePredictor(BasePredictor):
    ...
    async def _predict_simplified(
        ...
        while current_time <= to_time:
            ...
            lat = math.degrees(math.asin(math.sin(math.radians(inclination)) * math.sin(theta)))
            lng = (raan + math.degrees(theta) - (dt / 86400) * 360.98564736629) % 360
            if lng > 180:
                lng -= 360
    ...
    def _eci_to_geodetic(
        self, r: Tuple[float, float, float], time: datetime
    ...
    def _local_sidereal_time(self, time: datetime, longitude: float) -> float:
        ...
        # Julian date
        jd = (
            time - datetime(2000, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        ).total_seconds() / 86400 + 2451545.0
