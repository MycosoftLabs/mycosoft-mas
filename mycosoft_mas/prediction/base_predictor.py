...
class BasePredictor(ABC):
    ...
    async def predict(self, request: PredictionRequest) -> PredictionResult:
        ...
        if to_time < request.to_time:
            warnings.append(f"Prediction horizon clamped to {self.max_prediction_horizon}")
    ...
    def calculate_uncertainty(self, prediction_age_seconds: float) -> UncertaintyCone:
        ...
        """
        radius = (
            self.base_uncertainty_meters + self.uncertainty_growth_rate * prediction_age_seconds
        )

        return UncertaintyCone(radius_meters=radius)

    def _apply_confidence_decay(
        self, predictions: List[PredictedPosition], reference_time: datetime
    ) -> List[PredictedPosition]:
...
def destination_point(start: GeoPoint, bearing_deg: float, distance_m: float) -> GeoPoint:
    ...
    return GeoPoint(lat=math.degrees(lat2), lng=math.degrees(lng2), altitude=start.altitude)


def interpolate_position(p1: GeoPoint, p2: GeoPoint, fraction: float) -> GeoPoint:
    """
    ...
    return GeoPoint(lat=math.degrees(lat3), lng=math.degrees(lng3), altitude=alt)
