...
class WildlifePredictor(BasePredictor):
    ...
    def _get_migration_pattern(self, species: str, time: datetime) -> Optional[Dict]:
    ...
    def _get_species_speed(self, species: str) -> float:
        ...
        return SPECIES_SPEEDS.get(species.lower().replace(" ", "_"), SPECIES_SPEEDS["default"])
        ...
        while current_time <= to_time:
            ...
            predictions.append(
                PredictedPosition(
                    ...
                    position=new_pos,
                    velocity=(
                        Velocity(speed=step_speed, heading=heading) if step_speed > 0 else None
                    ),
                    confidence=1.0,
