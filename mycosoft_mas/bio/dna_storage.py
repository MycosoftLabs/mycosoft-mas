...
class DNAStorageSystem:
    ...
    async def get_capacity(self) -> StorageCapacity:
        ...
        return StorageCapacity(
            ...
            synthesisQueueLength=len(
                [j for j in self.jobs.values() if j.status == DNAJobStatus.SYNTHESIZING]
            ),
            sequencingQueueLength=len(
                [j for j in self.jobs.values() if j.status == DNAJobStatus.SEQUENCING]
            ),
        )
    ...
    async def encode_data(
        self, data: str, name: str, redundancy: int = 3, error_correction: str = "medium"
    ) -> DNAStorageJob:
    ...
    async def duplicate_data(self, data_id: str, copies: int) -> Dict[str, Any]:
        ...
        return {"dataId": data_id, "newRedundancy": stored.redundancy, "copiesAdded": copies}
