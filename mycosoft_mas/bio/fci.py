...
class FungalComputerInterface:
    ...
    async def start_session(
        self, device_id: UUID, species: str, strain: Optional[str] = None
    ) -> FCISession:
        self.active_session = FCISession(
            session_id=uuid4(),
            device_id=device_id,
            species=species,
            strain=strain,
    ...
    async def stimulate(
        self, stim_type: StimulationType, electrodes: List[int], params: Dict[str, Any]
    ) -> Dict[str, Any]:
        voltage_mv = params.get("voltage_mv", 100)
        duration_ms = params.get("duration_ms", 100)
        return {
            "stimulation_id": str(uuid4()),
            "type": stim_type.value,
