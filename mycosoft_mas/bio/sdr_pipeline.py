...
class SDRPipeline:
    ...
    def process_sample(
        self, value: float, channel: int = 0, timestamp: Optional[datetime] = None
    ) -> ProcessedSample:
        """
        ...
        self._running_var = (
            self._running_var * (1 - self._alpha)
            + (value - old_mean) * (value - self._running_mean) * self._alpha
        )
        ...
        snr_db = (
            10 * math.log10(max(signal_power / max(noise_power, 1e-20), 1e-10))
            if noise_power > 0
            else 0.0
        )
