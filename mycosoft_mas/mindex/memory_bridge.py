...
class MINDEXMemoryBridge:
    ...
    def __init__(self):
        ...
        self._url = os.getenv(
            "MINDEX_DATABASE_URL",
            os.getenv("DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/mindex"),
        )
        self._memory_url = os.getenv(
            "DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/mycosoft"
        )
    ...
    async def store_observation(
        ...
        value = {
            ...
            "location": (
                {"latitude": latitude, "longitude": longitude} if latitude and longitude else None
            ),
            "observed_at": (
                observed_at.isoformat() if observed_at else datetime.now(timezone.utc).isoformat()
            ),
            ...
            async with self._memory_pool.acquire() as conn:
                await conn.execute(
                    ...
                    ON CONFLICT (scope, namespace, key) DO UPDATE SET
                        ...
                        updated_at = NOW()
                """,
                    f"mindex:observations:{taxon_id}",
                    observation_id,
                    value,
                    embedding,
                )
    ...
    async def search_observations_semantic(
        self, query_embedding: List[float], taxon_filter: Optional[str] = None, limit: int = 10
    ...
    async def search_hypotheses_semantic(
        self, query_embedding: List[float], status_filter: Optional[str] = None, limit: int = 10
    ) -> List[Tuple[Dict[str, Any], float]]:
        if not self._memory_pool:
            await self.connect()
