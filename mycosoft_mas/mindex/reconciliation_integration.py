...
class ReconciledScraper:
    ...
    async def fetch_and_reconcile(self, limit: int = 1000, **scraper_kwargs) -> Dict[str, Any]:
        """
        ...
        if hasattr(self.scraper, "fetch_species"):
            ...
            raise ValueError(
                f"Scraper {type(self.scraper)} doesn't have fetch_species or fetch_observations"
            )
    ...
    def _enrich_record(
        self, match: TaxonomicMatch, license_info: LicenseInfo, citation_hash: str
    ) -> Dict[str, Any]:
