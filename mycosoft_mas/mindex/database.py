        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS species (
                    ...
                    -- Taxonomic reconciliation (GBIF backbone)
                    gbif_id INTEGER,
                    gbif_match_type TEXT,
                    gbif_confidence INTEGER,
                    accepted_gbif_id INTEGER,
                    accepted_name TEXT,
                    citation_hash TEXT,
                    
                    -- Index Fungorum (fungal nomenclature)
                    index_fungorum_lsid TEXT,
                    index_fungorum_name_id TEXT,
                    
                    -- License compliance
                    license TEXT,
                    license_url TEXT,
                    license_compliant INTEGER DEFAULT 0,
                    rights_holder TEXT,
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            stats["species_by_kingdom"] = {
                row["kingdom"] or "Unknown": row["count"] for row in cursor.fetchall()
            }
            ...
            """)
            stats["top_genera"] = [
                {"genus": row["genus"], "count": row["count"]} for row in cursor.fetchall()
            ]
