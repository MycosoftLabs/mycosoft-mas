...
class AuditLogger:
    ...
    def log(
        self, collector: str, action: str, details: Dict[str, Any], success: bool = True
    ) -> None:
        entry = AuditLogEntry(
    ...
    def get_entries(
        self, collector: Optional[str] = None, since: Optional[datetime] = None, limit: int = 100
    ) -> List[Dict]:
    ...
    async def _run_collector(self, name: str) -> None:
        ...
        while self._running and not collector._stop_event.is_set():
            try:
                ...
                self.audit_logger.log(name, "fetch", {"events": len(events)}, success=True)
                ...
                self.audit_logger.log(name, "fetch", {"error": str(e)}, success=False)

            # Wait for next poll
            try:
                await asyncio.wait_for(
                    collector._stop_event.wait(), timeout=collector.poll_interval_seconds
                )
    ...
    async def trigger_fetch(self, collector_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.audit_logger.log(
                collector_name, "manual_fetch", {"events": len(events)}, success=True
            )
            return {"success": True, "events": len(events)}
        except Exception as e:
            self.audit_logger.log(collector_name, "manual_fetch", {"error": str(e)}, success=False)
