...
class FungalKnowledgeGraph:
    ...
    async def add_fungal_species(self, species_data: Dict) -> None:
        ...
        try:
            ...
            self.graph.add(
                (species_uri, FUNGI.scientificName, Literal(species_data["scientific_name"]))
            )
            ...
            for relationship in species_data.get("relationships", []):
                target_uri = self._get_entity_uri(
                ...
                self.graph.add(
                    (target_uri, RELATIONS[f"is_{relationship['type']}_by"], species_uri)
                )
    ...
    async def query_relationships(
        self, species_id: str, relationship_type: Optional[str] = None
    ) -> List[Dict]:
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            self.graph.serialize(
                destination=str(self.data_dir / "final_state.ttl"), format="turtle"
            )
