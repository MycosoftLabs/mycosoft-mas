...
SAFE_TRANSFORMS = {
    "normalize_temperature": lambda data: (
        {**data, "temp_c": (data.get("temp_f", 32) - 32) * 5 / 9} if "temp_f" in data else data
    ),
...
class BioDataTranslator:
        ...
        try:
            ...
            if ontology_file.exists():
                ...
                with self.ontology:
                    ...
                    class ChemicalCompound(ChemicalEntity):
                        pass

                    class Metabolite(ChemicalCompound):
                        pass

                    class Enzyme(ChemicalCompound):
                        pass

                    class PhysicalProperty(PhysicalEntity):
                        pass

                    class Structure(PhysicalProperty):
                        pass

                    class Interaction(PhysicalProperty):
                        pass
    ...
    async def register_agent(self, agent_id: str, agent_info: Dict) -> Dict:
        ...
        try:
            ...
            if agent_id in self.connected_agents:
                return {"success": False, "message": f"Agent {agent_id} already registered"}
    ...
    async def register_schema(self, schema_data: Dict) -> Dict:
        ...
        try:
            ...
            await self.schema_queue.put({"schema_id": schema_id, "action": "register"})
    ...
    async def update_schema(self, schema_id: str, schema_data: Dict) -> Dict:
        ...
        try:
            ...
            if schema_id not in self.schemas:
                return {"success": False, "message": f"Schema {schema_id} not found"}
            ...
            # Add to schema queue for processing
            await self.schema_queue.put({"schema_id": schema_id, "action": "update"})
    ...
    async def delete_schema(self, schema_id: str) -> Dict:
        ...
        try:
            ...
            # Add to schema queue for processing
            await self.schema_queue.put({"schema_id": schema_id, "action": "delete"})
    ...
    async def create_translation_rule(self, rule_data: Dict) -> Dict:
        ...
        try:
            ...
            if source_schema_id not in self.schemas:
                return {"success": False, "message": f"Source schema {source_schema_id} not found"}

            if target_schema_id not in self.schemas:
                return {"success": False, "message": f"Target schema {target_schema_id} not found"}
            ...
            await self.rule_queue.put({"rule_id": rule_id, "action": "create"})
    ...
    async def update_translation_rule(self, rule_id: str, rule_data: Dict) -> Dict:
        ...
        try:
            ...
            if rule_id not in self.translation_rules:
                return {"success": False, "message": f"Translation rule {rule_id} not found"}
            ...
            # Add to rule queue for processing
            await self.rule_queue.put({"rule_id": rule_id, "action": "update"})
    ...
    async def delete_translation_rule(self, rule_id: str) -> Dict:
        ...
        try:
            ...
            # Add to rule queue for processing
            await self.rule_queue.put({"rule_id": rule_id, "action": "delete"})
    ...
    async def translate_data(
        self, data: Dict, source_schema_id: str, target_schema_id: str
    ) -> Dict:
        ...
        try:
            ...
            for r_id, rule in self.translation_rules.items():
                if (
                    rule.source_schema_id == source_schema_id
                    and rule.target_schema_id == target_schema_id
                ):
                    rule_id = r_id
                    break
    ...
    async def get_translation_status(self, translation_id: str) -> Dict:
        ...
        try:
            ...
            if translation_id not in self.translated_data:
                return {"success": False, "message": f"Translation {translation_id} not found"}
    ...
    async def _find_translation_path(
    ...
    async def _remove_rule_from_ontology(self, rule_id: str):
        ...
        try:
            ...
            # Get source and target schemas
            self.schemas[rule.source_schema_id]
            target_schema = self.schemas[rule.target_schema_id]
