...
class SnapshotManager:
            ...
            try:
                with open(self._index_file, "r") as f:
                    ...
                    self._metadata_cache = {
                        k: SnapshotMetadata.from_dict(v) for k, v in data.items()
                    }
    ...
    def _save_index(self) -> None:
        ...
        try:
            with open(self._index_file, "w") as f:
                json.dump({k: v.to_dict() for k, v in self._metadata_cache.items()}, f, indent=2)
        except Exception as e:
    ...
    async def create_snapshot(
        self, entity_type: str, entries: List[Dict[str, Any]], bucket_start_ms: int
    ) -> SnapshotMetadata:
        """
        ...
        logger.info(
            f"Created snapshot {bucket_key}: {len(entries)} entries, {len(compressed)} bytes"
        )
    ...
    async def query_snapshots(
        self, entity_type: str, start_time_ms: int, end_time_ms: int
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def list_snapshots(
        self, entity_type: Optional[str] = None, since_ms: Optional[int] = None
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "time_range_hours": (
                ...
                if oldest_bucket and newest_bucket
                else 0
            ),
        }
