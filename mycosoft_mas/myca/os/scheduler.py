...
class Scheduler:
        ...
        try:
            ...
            if bridge._pg_pool:
                async with bridge._pg_pool.acquire() as conn:
                    ...
                    for row in rows:
                        self._items.append(ScheduledItem(
                            id=row["id"],
                            title=row["title"],
    ...
    async def add(self, title: str, schedule_type: str = "once",
                  time_of_day: time = None, day_of_week: int = None,
                  scheduled_at: datetime = None, cron_expr: str = None,
                  task_type: str = "general", task_data: dict = None,
                  description: str = "") -> ScheduledItem:
                ...
                async with bridge._pg_pool.acquire() as conn:
                    item.id = await conn.fetchval(
                        """INSERT INTO myca_schedule
                           ...
                           RETURNING id""",
                        title, description, schedule_type, cron_expr, day_of_week,
                        time_of_day, scheduled_at, task_type,
    ...
    def _compute_next_run(self, schedule_type: str, time_of_day: time = None,
                          day_of_week: int = None,
                          scheduled_at: datetime = None) -> Optional[datetime]:
                ...
                async with bridge._pg_pool.acquire() as conn:
                    await conn.execute(
                        """UPDATE myca_schedule
                           ...
                           WHERE id = $4""",
                        item.enabled, item.last_run, item.next_run, item.id,
                    )
        except Exception as e:
