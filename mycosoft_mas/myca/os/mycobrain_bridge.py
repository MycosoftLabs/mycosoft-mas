...
MAS device registry: /api/devices/network (on 188).
    ...
    async def get_device_status(self) -> list:
        ...
        try:
            async with self._session.get(
                f"{self._mas_url.rstrip('/')}/api/devices/network",
...
def _format_device_summary(devices: list) -> str:
    """Format device list as a short summary."""
    if not devices:
        return ""
    parts = [f"MycoBrain devices: {len(devices)} registered."]
    for d in devices[:5]:
        name = d.get("name", d.get("device_id", "unknown"))
