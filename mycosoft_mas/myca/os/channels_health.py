...
async def _validate_slack() -> dict[str, Any]:
    token = _env_any(
        "SLACK_APP_TOKEN", "MYCA_SLACK_APP_TOKEN", "SLACK_OAUTH_TOKEN", "MYCA_SLACK_BOT_TOKEN"
    )
    if not token:
        return {
            "channel": "slack",
            "status": "missing_credential",
            "message": "No Slack token set.",
            "pass": False,
        }
    try:
        ...
        await client.close()
        return (
            {"channel": "slack", "status": "connected", "message": "OK.", "pass": True}
            if ok
            else {
                "channel": "slack",
                "status": "auth_failed",
...
async def _validate_asana() -> dict[str, Any]:
    if not _env_any("ASANA_API_KEY", "ASANA_PAT", "ASANA_ACCESS_TOKEN", "MYCA_ASANA_TOKEN"):
        return {
            "channel": "asana",
            "status": "missing_credential",
            "message": "Asana credential not set.",
            "pass": False,
        }
    try:
        ...
        if workspaces:
            return {
                "channel": "asana",
                "status": "connected",
                "message": f"OK. {len(workspaces)} workspace(s).",
                "pass": True,
            }
        return {
            "channel": "asana",
            "status": "auth_failed",
            "message": "No workspaces returned.",
            "pass": False,
        }
    except Exception as e:
...
async def _validate_signal() -> dict[str, Any]:
    if not _env_any("MYCA_SIGNAL_NUMBER", "SIGNAL_SENDER_NUMBER"):
        return {
            "channel": "signal",
            "status": "missing_credential",
            "message": "MYCA_SIGNAL_NUMBER not set.",
            "pass": False,
        }
    try:
        ...
        await client.close()
        return (
            {"channel": "signal", "status": "connected", "message": "signal-cli OK.", "pass": True}
            if ok
            else {
                "channel": "signal",
                "status": "unreachable",
                "message": "signal-cli REST API not reachable.",
                "pass": False,
            }
        )
    except Exception as e:
...
async def _validate_discord() -> dict[str, Any]:
    ...
    try:
        ...
        await client.close()
        return (
            {"channel": "discord", "status": "connected", "message": "Bot connected.", "pass": True}
            if ok
            else {
                "channel": "discord",
                "status": "auth_failed",
    ...
    try:
        ...
        return (
            ...
            if ok
                ...
                "channel": "whatsapp",
                "status": "unreachable",
                "message": "Evolution API not reachable.",
                "pass": False,
            }
        )
    except Exception as e:
