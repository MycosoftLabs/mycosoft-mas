...
class MYCAConsciousness:
    ...
    async def _world_model_loop(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                await self._report_error(
                    str(e), source="background_task", context={"loop": "world_model"}
                )
    ...
    async def _pattern_recognition_loop(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                await self._report_error(
                    str(e), source="background_task", context={"loop": "pattern_recognition"}
                )
    ...
    async def _dream_loop(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                await self._report_error(
                    str(e), source="background_task", context={"loop": "dream"}
                )
                self._state = ConsciousnessState.CONSCIOUS
    ...
    async def _save_state(self) -> None:
        ...
        if self._memory_coordinator:
            try:
                ...
                await self._report_error(
                    str(e), source="consciousness", context={"phase": "save_state"}
                )
        ...
        try:
            ...
            if GROUNDED_COGNITION_ENABLED:
                ...
                ep = await gate.build_experience_packet(
                    content, source, context, session_id, user_id
                )
            ...
            async def safe_working_context():
                try:
                    return await asyncio.wait_for(
                        self._working_memory.load_context(focus, session_id, user_id), timeout=2.0
                    )
            ...
            async def safe_world_context():
                try:
                    return await asyncio.wait_for(
                        self._world_model.get_relevant_context(focus), timeout=2.0
                    )
                except asyncio.TimeoutError:
    ...
    async def _recall_relevant_memories(
        ...
        if self._memory_coordinator:
            try:
                ...
                results = await self._memory_coordinator.semantic_search(query=content, limit=5)
                memories.extend(results)

                # Also get recent episodic memories
                recent = await self._memory_coordinator.recall(
                    query=f"recent_episodes:{focus.category}", layer="episodic", limit=3
                )
                if recent:
        ...
        is_creator = (context or {}).get("is_creator") is True or (
            user_id or ""
        ).lower() == "morgan"
        if is_creator:
            try:
    ...
    async def _store_interaction(
        self, content: str, source: str, session_id: Optional[str], user_id: Optional[str]
    ) -> None:
    ...
    async def delegate_to_agent(self, agent_type: str, task: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if source in ("chat", "consciousness") and result.feasibility in (
                ...
                await self.alert_morgan(
                    ...
                    priority=(
                        "high" if result.feasibility == FixFeasibility.REQUIRES_HUMAN else "normal"
                    ),
                )
        except Exception as e:
        ...
        if event in self._event_handlers:
            for handler in self._event_handlers[event]:
                try:
                    ...
                    await self._report_error(
                        str(e), source="consciousness", context={"event": event}
                    )
