...
class DigitalSubstrate(BaseSubstrate):
    ...
    async def store(self, key: str, data: Any) -> bool:
        ...
        try:
            self._storage[key] = {"data": data, "stored_at": datetime.now(timezone.utc).isoformat()}
            return True
        except Exception as e:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "capabilities": [
                {"name": c.name, "available": c.available} for c in self._capabilities.values()
            ],
        }
...
class WetwareSubstrate(BaseSubstrate):
    ...
    async def send_chemical_signal(self, compound: str, concentration: float) -> bool:
        """
...
class HybridSubstrate(BaseSubstrate):
    ...
    async def initialize(self) -> bool:
        ...
        await self._wetware.initialize()
...
def create_substrate(
    consciousness: "MYCAConsciousness", substrate_type: SubstrateType = SubstrateType.DIGITAL
) -> BaseSubstrate:
    """
