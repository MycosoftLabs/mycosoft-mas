...
class UnifiedRouter:
    ...
    async def route(
        self, message: str, context: Optional[Dict[str, Any]] = None, stream: bool = False
        ...
        logger.info(
            f"Classified intent: {intent.intent_type.value} (confidence: {intent.confidence:.2f})"
        )

        # Add intent to context for handlers
        context["intent"] = intent
        context["original_message"] = message
        context.setdefault("answers_persona", "scientific_grounded_concise_context_aware_honest")
    ...
    async def _handle_agent_task(
        self, message: str, intent: IntentResult, context: Dict[str, Any]
    ) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            if orchestrator:
                ...
                task_data = {
                    ...
                    "context": (
                        context.get("conversation_history", [])[-3:]
    ...
    async def _handle_tool_call(
        ...
        try:
            ...
            if tool_manager:
                ...
                tool_call = ToolCall(id=str(uuid4()), name=target_tool, arguments=intent.entities)
    ...
    async def _handle_knowledge_query(
        ...
        try:
            ...
            if mindex_sensor:
                ...
                search_query = (
                    intent.entities.get("species_name") or intent.entities.get("topic") or message
                )
    ...
    async def _handle_system_command(
        ...
        try:
            ...
            if n8n_client:
                ...
                if result:
                    yield "Workflow triggered successfully. "
    ...
    async def _handle_status_query(
        ...
        try:
            ...
            if active_perception:
                ...
                if any(
                    word in message.lower() for word in ["you", "your", "alive", "well", "working"]
                ):
                ...
                if world_model:
                    ...
                    if status_target == "flights" or "flight" in message.lower():
                        ...
                        yield "- Consciousness: Active\n"
                        yield f"- World Model: {len(world_state.get('sensors', {}))} sensors connected\n"
                        yield "- CREP: Monitoring flights, vessels, satellites, weather\n"
                        yield "- Earth2: Prediction models available\n"
                        yield "- MINDEX: Database connected\n"
                else:
                    # Fallback status
    ...
    def _generate_consciousness_fallback(self, message: str, context: Dict[str, Any]) -> str:
        ...
        if any(
            ...
            for word in ["who are you", "what are you", "your name", "identity"]
        ):
            return (
        ...
        if any(
            word in message_lower
            for word in ["how are you", "how do you feel", "feeling", "emotion"]
        ):
            return (
    ...
    async def _get_conscious_response_generator(self):
        ...
        if self._conscious_response_gen is None:
            try:
                from mycosoft_mas.consciousness.conscious_response_generator import (
                    get_conscious_response_generator,
                )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                response = await client.post(
                    ...
                    json={"target_agent": agent_name, "task": task_data},
                )
