...
class SelfModel:
    ...
    async def initialize(self) -> None:
        ...
        logger.info(
            f"Self model initialized with {len(self._traits)} traits, {len(self._skills)} skills"
        )
    ...
    async def _load_from_db(self) -> None:
        ...
        for row in cursor.fetchall():
            self._skills[row["name"]] = Skill(
                ...
                last_practiced=(
                    datetime.fromisoformat(row["last_practiced"]) if row["last_practiced"] else None
                ),
            )
    ...
    async def _initialize_default_traits(self) -> None:
        ...
        default_traits = {
            "curiosity": PersonalityTrait(
                name="curiosity", value=0.85, description="Drive to learn and understand new things"
            ),
            "independence": PersonalityTrait(
                name="independence", value=0.70, description="Ability to think and act autonomously"
            ),
            ...
            "confidence": PersonalityTrait(
                name="confidence", value=0.70, description="Trust in own abilities and decisions"
            ),
...
the answer together, one conversation at a time."""
    ...
    async def update_trait(
        self, name: str, value: float, description: str = "", reason: str = "trait update"
    ) -> None:
    ...
    async def learn_skill(self, name: str, initial_proficiency: float = 0.1) -> Skill:
        """Learn a new skill."""
        skill = Skill(
            name=name, proficiency=initial_proficiency, learned_at=datetime.now(timezone.utc)
        )
    ...
    async def record_interaction(
        self, user_id: str, quality: float = 0.5, note: Optional[str] = None
    ) -> None:
        """Record an interaction with a user."""
        if user_id not in self._relationships:
            # Create new relationship
            relationship = Relationship(user_id=user_id, depth=0.1, relationship_type="user")
    ...
    async def get_personality_summary(self) -> str:
        ...
        for name, trait in sorted_traits:
            level = (
                "highly" if trait.value > 0.8 else "moderately" if trait.value > 0.6 else "somewhat"
            )
