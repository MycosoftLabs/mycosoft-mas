...
class EmotionalState:
    ...
    async def feel(self, emotion: EmotionType, intensity: float, source: str) -> None:
        """
        ...
        """
        event = EmotionEvent(
            emotion=emotion, intensity=min(1.0, max(0.0, intensity)), source=source
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "emotions": {e.value: round(v, 2) for e, v in self._current_emotions.items()},
            "tone": {
    ...
    async def process_interaction(self, content: str, source: str) -> None:
        ...
        if any(
            word in content_lower for word in ["thank", "great", "excellent", "amazing", "good job"]
        ):
