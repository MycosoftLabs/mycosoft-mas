...
if TYPE_CHECKING:
    from mycosoft_mas.consciousness.attention import AttentionFocus, PatternAlert
...
class IntuitionEngine:
    ...
    async def quick_response(
        self, content: str, focus: "AttentionFocus", working_context: "WorkingContext"
        ...
        for heuristic in self._heuristics.values():
            if heuristic.matches(content):
                ...
                if is_followup and heuristic.type in (
                    HeuristicType.GREETING,
                    HeuristicType.IDENTITY,
                ):
                    logger.debug(
                ...
                response = self._personalize_response(heuristic.response_template, working_context)
        ...
        if simple_response:
            elapsed = (datetime.now(timezone.utc) - start_time).total_seconds() * 1000
            return IntuitiveResponse(
                response=simple_response, confidence=0.7, thinking_time_ms=elapsed
            )
    ...
    def _personalize_response(self, template: str, context: "WorkingContext") -> str:
    ...
    async def _try_simple_response(self, content: str, focus: "AttentionFocus") -> Optional[str]:
    ...
    async def scan_for_patterns(self, world_state: Optional["WorldState"]) -> List["PatternAlert"]:
        """
    ...
    def _scan_crep_patterns(self, crep_data: Dict[str, Any]) -> List["PatternAlert"]:
        ...
        from mycosoft_mas.consciousness.attention import AttentionPriority, PatternAlert
    ...
    async def learn_heuristic(
        self, trigger: str, response: str, heuristic_type: HeuristicType = HeuristicType.SHORTCUT
    ) -> Heuristic:
    ...
    def get_heuristic_stats(self) -> Dict[str, Any]:
        ...
        most_used = sorted(self._heuristics.values(), key=lambda h: h.use_count, reverse=True)[:5]
