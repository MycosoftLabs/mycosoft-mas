...
class MINDEXSensor(BaseSensor):
    ...
    async def read(self) -> Optional["SensorReading"]:
        ...
        try:
            ...
            reading = SensorReading(
                ...
                freshness=(
                    DataFreshness.LIVE if data.get("available") else DataFreshness.UNAVAILABLE
                ),
    ...
    async def query(self, query_text: str) -> Dict[str, Any]:
        ...
        try:
            response = await self._client.post(
                f"{self._api_base}/search", json={"query": query_text, "limit": 10}
            )
            if response.status_code == 200:
                return response.json()
    ...
    async def semantic_search(
        ...
        try:
            ...
            response = await self._client.post(f"{self._api_base}/semantic-search", json=params)
            if response.status_code == 200:
    ...
    async def get_fungi_info(self, species: str) -> Dict[str, Any]:
        ...
        try:
            response = await self._client.get(f"{self._api_base}/fungi/{species}")
            if response.status_code == 200:
                return response.json()
