...
class CREPSensor(BaseSensor):
    ...
    async def get_region_data(
        ...
        try:
            ...
            response = await self._client.get(f"{self._api_base}/flights/region", params=params)
            if response.status_code == 200:
                data["flights"] = response.json()

            # Get regional marine data
            response = await self._client.get(f"{self._api_base}/marine/region", params=params)
            if response.status_code == 200:
