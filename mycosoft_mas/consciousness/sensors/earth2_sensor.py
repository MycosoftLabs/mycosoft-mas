...
class Earth2Sensor(BaseSensor):
    ...
    async def forecast_weather(self, lat: float, lon: float, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            response = await self._client.post(
                f"{self._api_base}/forecast/location", json={"lat": lat, "lon": lon, "hours": hours}
            )
            if response.status_code == 200:
                return response.json()
    ...
    async def predict_spore_dispersal(
        self, lat: float, lon: float, species: str = "generic"
    ) -> Dict[str, Any]:
    ...
    async def nowcast(self, lat: float, lon: float, minutes: int = 60) -> Dict[str, Any]:
        ...
        try:
            response = await self._client.post(
                f"{self._api_base}/nowcast", json={"lat": lat, "lon": lon, "minutes": minutes}
            )
            if response.status_code == 200:
                return response.json()
            return {"error": f"API error: {response.status_code}"}
