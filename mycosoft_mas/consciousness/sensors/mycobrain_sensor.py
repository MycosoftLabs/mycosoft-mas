...
class MycoBrainSensor(BaseSensor):
    ...
    async def read(self) -> Optional["SensorReading"]:
        ...
        try:
            ...
            if devices:
                ...
                data["active_count"] = sum(1 for d in devices if d.get("status") == "online")
    ...
    async def get_device_telemetry(self, device_id: str) -> Dict[str, Any]:
        ...
        try:
            response = await self._client.get(f"{self._api_base}/devices/{device_id}/telemetry")
            if response.status_code == 200:
                return response.json()
    ...
    async def check_presence(self, user_id: str = "morgan") -> Dict[str, Any]:
        ...
        try:
            response = await self._client.get(f"{self._api_base}/presence/{user_id}")
            if response.status_code == 200:
                return response.json()
