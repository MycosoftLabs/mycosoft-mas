...
class VoiceInterface:
        ...
        try:
            if self._http_client:
                response = await self._http_client.get(
                    f"{self.PERSONAPLEX_API}/health", timeout=5.0
                )
    ...
    async def start_session(
        self, user_id: str, mode: VoiceMode = VoiceMode.FULL_DUPLEX
    ) -> VoiceSession:
        ...
        try:
            if self._http_client:
                await self._http_client.post(
                    f"{self.PERSONAPLEX_BRIDGE}/session/end", json={"session_id": session.id}
                )
        except Exception as e:
    ...
    async def speak_stream(self, text_generator: AsyncGenerator[str, None]) -> None:
        """
    ...
    async def initiate_call(self, user_id: str, message: str, priority: str = "normal") -> bool:
        """
            ...
            try:
                if self._http_client:
                    response = await self._http_client.post(
                        ...
                        json={"user_id": user_id, "message": message, "priority": priority},
                    )
    ...
    async def alert_morgan(self, message: str, priority: str = "normal") -> bool:
        ...
        """
        return await self.initiate_call(user_id="morgan", message=message, priority=priority)
