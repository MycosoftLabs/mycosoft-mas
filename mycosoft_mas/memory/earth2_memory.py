...
class Earth2Memory:
    ...
    async def get_user_forecasts(
        self, user_id: str, limit: int = 10, model: Optional[str] = None
    ) -> List[ForecastResult]:
        ...
        if not self._pool:
            # Return from cache
            return [f for f in self._forecast_cache.values() if f.user_id == user_id][:limit]
            ...
            try:
                async with self._pool.acquire() as conn:
                    row = await conn.fetchrow(
                        "SELECT * FROM mindex.earth2_user_preferences WHERE user_id = $1", user_id
                    )
                    if row:
                        prefs = UserWeatherPreferences(
                            user_id=user_id,
                            favorite_locations=(
                                json.loads(row["favorite_locations"])
                                if row["favorite_locations"]
                                else []
                            ),
                            preferred_models=(
                                [Earth2Model(m) for m in json.loads(row["preferred_models"])]
                                if row["preferred_models"]
                                else []
                            ),
                            common_lead_times=(
                                json.loads(row["common_lead_times"])
                                if row["common_lead_times"]
                                else []
                            ),
                            variables_of_interest=(
                                [
                                    WeatherVariable(v)
                                    for v in json.loads(row["variables_of_interest"])
                                ]
                                if row["variables_of_interest"]
                                else []
                            ),
    ...
    async def get_model_stats(self, model: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {model.value: stats.to_dict() for model, stats in self._model_stats.items()}
    ...
    async def get_popular_locations(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        if not self._pool:
            ...
            return [
                ...
                for loc, cnt in sorted(location_counts.items(), key=lambda x: x[1], reverse=True)[
                    :limit
                ]
            ]
        ...
        try:
            async with self._pool.acquire() as conn:
                ...
                return [
                    {"location": row["location_name"] or row["location"], "count": row["cnt"]}
                    for row in rows
                ]
        except Exception as e:
            ...
            "recent_forecasts": [
                {
                    ...
                    or f"{f.location.get('lat', 0):.1f}, {f.location.get('lng', 0):.1f}",
                    "model": f.model.value,
    ...
    async def _update_user_preferences(self, user_id: str, forecast: ForecastResult) -> None:
        """Update user preferences based on a forecast."""
        prefs = await self.get_user_preferences(user_id)

        # Update favorite locations
        loc_entry = {"location": forecast.location, "name": forecast.location_name, "count": 1}

        # Check if location already in favorites
        found = False
        for loc in prefs.favorite_locations:
            if loc.get("location", {}).get("lat") == forecast.location.get("lat") and loc.get(
                "location", {}
            ).get("lng") == forecast.location.get("lng"):
        ...
        try:
            async with self._pool.acquire() as conn:
                ...
                """,
                    ...
                    json.dumps(forecast.location),
                    forecast.location_name,
                    forecast.lead_time_hours,
                    json.dumps([v.value for v in forecast.variables]),
                    forecast.timestamp,
                    json.dumps(forecast.result_summary) if forecast.result_summary else None,
                    forecast.inference_time_ms,
                    forecast.source,
                )
            return True
        except Exception as e:
    ...
    async def _persist_preferences(self, prefs: UserWeatherPreferences) -> bool:
        ...
        try:
            async with self._pool.acquire() as conn:
                ...
                """,
                    prefs.user_id,
                    json.dumps(prefs.favorite_locations),
