...
class VoiceSearchBridge:
    ...
    async def create_link(
        ...
        link = VoiceSearchLink(
            voice_session_id=voice_session_id, search_session_id=search_session_id, user_id=user_id
        )
    ...
    async def link_voice_to_search(
        self, voice_session_id: UUID, search_session_id: UUID
        ...
        if link.search_session_id and self._search_memory:
            try:
                ...
                await self._search_memory.add_query(
                    session_id=link.search_session_id, query=query, source="voice"
                )
                ...
                if species_mentioned:
                    for species_id in species_mentioned:
                        await self._search_memory.record_focus(
                            session_id=link.search_session_id, species_id=species_id, topic=topic
                        )
    ...
    async def record_voice_ai_exchange(
        self, voice_session_id: UUID, role: str, content: str, topic: Optional[str] = None
    ) -> bool:
        """
        ...
        if link.search_session_id and self._search_memory:
            try:
                await self._search_memory.add_ai_turn(
                    session_id=link.search_session_id, role=role, content=content, topic=topic
                )
                logger.debug("Propagated AI turn to search session")
            except Exception as e:
