                ...
                async with client.post(
                    ...
                    json={
                        ...
                        "messages": [
                            {
                                "role": "system",
                                "content": "Summarize the following conversation in 2-3 sentences, capturing the key topics and outcomes.",
                            },
...
class PersonaPlexMemory:
    ...
    async def start_session(
        ...
        session = VoiceSession(
            id=uuid4(), user_id=user_id, speaker_profile=speaker_profile, context=context or {}
        )
        ...
        try:
            async with self._pool.acquire() as conn:
                ...
                """,
                    ...
                    session.summary,
                    session.topics,
                    json.dumps(session.emotional_arc),
        ...
        try:
            async with self._pool.acquire() as conn:
                row = await conn.fetchrow(
                    "SELECT * FROM voice.sessions WHERE id = $1", str(session_id)
                )
                if row:
                    return VoiceSession.from_dict(
                        {
                            ...
                            "emotional_arc": (
                                json.loads(row["emotional_arc"]) if row["emotional_arc"] else []
                            ),
    ...
    async def get_recent_sessions(
        self, user_id: str, limit: int = 10, since: Optional[datetime] = None
    ) -> List[VoiceSession]:
