...
class PersistentKnowledgeGraph:
    ...
    async def _load_cache(self) -> None:
        ...
        logger.info(
            f"Loaded {len(self._node_cache)} nodes and {len(self._edge_cache)} edges into cache"
        )
    ...
    async def get_neighbors(
        self, node_id: UUID, edge_type: Optional[EdgeType] = None, direction: str = "outgoing"
    ) -> List[Node]:
    ...
    async def get_subgraph(self, center_id: UUID, depth: int = 2) -> Dict[str, Any]:
    ...
    async def list_edges(
        self, edge_type: Optional[EdgeType] = None, limit: int = 100
    ) -> List[Edge]:
...
class MultiHopReasoner:
    ...
    async def query(
        self, start_node: str, relationship_path: List[str], max_results: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        for rel in relationship_path:
            ...
            for node in current_nodes:
                neighbors = await self._graph.get_neighbors(
                    node.id, edge_type=edge_type, direction="outgoing"
                )
    ...
    async def find_all_paths(
        self, source_name: str, target_name: str, max_depth: int = 4
    ) -> List[Dict[str, Any]]:
    ...
    async def infer_relationship(self, node_a: str, node_b: str) -> Dict[str, Any]:
        ...
        if direct_paths:
            return {"relationship": "direct", "paths": direct_paths, "confidence": 1.0}
        ...
        return {"relationship": "none", "confidence": 0.0}
...
class RegistryAutoPopulator:
    ...
    async def _create_relationships(self) -> int:
        ...
        if mas_node:
            ...
            for agent in agents:
                edge = Edge(source_id=mas_node.id, target_id=agent.id, edge_type=EdgeType.CONTAINS)
        ...
        if natureos:
            ...
            for device in devices:
                edge = Edge(source_id=natureos.id, target_id=device.id, edge_type=EdgeType.MANAGES)
                await self._graph.add_edge(edge)
                count += 1
        ...
        if personaplex:
            ...
            for agent_name in voice_agents:
                ...
                if agent:
                    edge = Edge(
                        source_id=personaplex.id, target_id=agent.id, edge_type=EdgeType.CONNECTS_TO
                    )
                    await self._graph.add_edge(edge)
                    count += 1
        ...
        if n8n:
            ...
            for agent_name in workflow_agents:
                agent = await self._graph.find_node_by_name(agent_name, NodeType.AGENT)
                if agent:
                    edge = Edge(source_id=n8n.id, target_id=agent.id, edge_type=EdgeType.HOSTS)
                    await self._graph.add_edge(edge)
                    count += 1
