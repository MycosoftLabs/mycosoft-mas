...
class SearchSession:
    ...
    def add_query(
        ...
        query: str,
        result_count: int = 0,
        result_types: Optional[Dict[str, int]] = None,
        source: str = "text",
    ) -> SearchQuery:
        sq = SearchQuery(
            id=uuid4(),
    ...
    def explore_topic(
        self, topic: SearchTopic, species_id: Optional[str] = None
    ) -> WidgetInteraction:
        ...
        interaction = WidgetInteraction(
            widget=topic,
            species_id=species_id or self.current_species,
    ...
    def add_ai_message(
        self, role: str, content: str, context_topic: Optional[SearchTopic] = None
    ) -> AIMessage:
        msg = AIMessage(
            id=uuid4(),
            role=role,
            content=content,
            timestamp=datetime.now(timezone.utc),
            context_species=self.current_species,
            context_topic=context_topic or self.current_topic,
        )
    ...
    def has_ai_breakthrough(self) -> bool:
        return len(self.ai_conversation) >= 4 and any(
            len(m.content) > 200 for m in self.ai_conversation if m.role == "assistant"
        )
    ...
    def get_ai_summary(self) -> Dict[str, Any]:
        return {
            ...
            "topics_discussed": list(
                set(m.context_topic.value for m in self.ai_conversation if m.context_topic)
            ),
            "species_discussed": list(
                set(m.context_species for m in self.ai_conversation if m.context_species)
            ),
            "last_exchange": {
                "user": next(
                    (m.content for m in reversed(self.ai_conversation) if m.role == "user"), None
                ),
                "assistant": next(
                    (m.content for m in reversed(self.ai_conversation) if m.role == "assistant"),
                    None,
                ),
            },
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SearchSession":
        return cls(
            ...
            widget_interactions=[
                WidgetInteraction.from_dict(w) for w in data.get("widget_interactions", [])
            ],
            started_at=datetime.fromisoformat(data["started_at"]),
            last_activity=datetime.fromisoformat(data["last_activity"]),
            ended_at=datetime.fromisoformat(data["ended_at"]) if data.get("ended_at") else None,
            voice_session_id=(
                UUID(data["voice_session_id"]) if data.get("voice_session_id") else None
            ),
...
class SearchMemoryManager:
    ...
    async def start_session(
        ...
        session = SearchSession(
            id=uuid4(), user_id=user_id, voice_session_id=voice_session_id, metadata=metadata or {}
        )
        history = await self.get_user_search_history(user_id, limit=3)
        if history:
            session.metadata["previous_searches"] = [
                {"query": h.queries[0].query if h.queries else "", "species": h.focused_species[:3]}
                for h in history
            ]
    ...
    async def update_query_insight(
        self, session_id: UUID, query_id: UUID, insight: str, associations: List[str]
    ) -> bool:
        session = self._active_sessions.get(session_id)
        if not session:
            return False
    ...
    async def record_focus(
        self, session_id: UUID, species_id: str, topic: Optional[str] = None
    ) -> bool:
        session = self._active_sessions.get(session_id)
        if not session:
            return False
        ...
        if topic:
            try:
                ...
                pass
        await self._record_user_interest(
            session.user_id, species_id, "focus", {"topic": topic, "session_id": str(session_id)}
        )
        return True

    async def add_ai_turn(
        self, session_id: UUID, role: str, content: str, topic: Optional[str] = None
    ) -> Optional[AIMessage]:
        session = self._active_sessions.get(session_id)
        if not session:
            return None
        ...
        if session.current_species:
            await self._record_user_interest(
                session.user_id,
                session.current_species,
                "ai_question",
                {"question": content[:200] if role == "user" else None},
            )
        return msg
    ...
    async def get_session_context(self, session_id: UUID) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_ai": [
                {"role": m.role, "content": m.content[:100]} for m in session.ai_conversation[-4:]
            ],
    ...
    async def end_session(self, session_id: UUID) -> Optional[SearchSessionSummary]:
        ...
        if (
            ...
            and self._user_sessions[session.user_id] == session_id
        ):
        ...
        summary = SearchSessionSummary(
            ...
            unique_species_explored=len(session.focused_species),
            topics_explored=[t.value for t in session.explored_topics],
            ai_message_count=len(session.ai_conversation),
    ...
    async def enrich_to_mindex(
        self, query: str, user_id: str, taxon_ids: Optional[List[int]] = None
    ) -> bool:
        ...
        try:
            async with self._pool.acquire() as conn:
                rows = await conn.fetch(
                    "SELECT * FROM mindex.search_sessions WHERE user_id = $1 ORDER BY started_at DESC LIMIT $2",
                    user_id,
                    limit,
                )
                sessions = []
    ...
    def _calculate_interests(self, session: SearchSession) -> List[Tuple[str, float]]:
        ...
        for species_id in session.focused_species:
            ...
            score += min(
                0.2,
                sum(1 for i in session.widget_interactions if i.species_id == species_id) * 0.05,
            )
            score += min(
                0.2,
                sum(1 for m in session.ai_conversation if m.context_species == species_id) * 0.1,
            )
            interests[species_id] = min(1.0, score)
        return sorted(interests.items(), key=lambda x: x[1], reverse=True)

    async def _record_user_interest(
        self, user_id: str, species_id: str, interest_type: str, metadata: Dict[str, Any]
    ) -> None:
    ...
    async def _record_search_analytics(self, session: SearchSession, query: SearchQuery) -> None:
        ...
        try:
            async with self._pool.acquire() as conn:
                await conn.execute(
                    "INSERT INTO mindex.search_analytics (query, user_id, session_id, result_count, clicked_results) VALUES ($1, $2, $3, $4, $5::jsonb)",
                    query.query,
                    session.user_id,
                    str(session.id),
                    query.result_count,
                    json.dumps(query.clicked_results),
                )
        except Exception as e:
    ...
    async def _persist_session(self, session: SearchSession) -> bool:
        ...
        try:
            async with self._pool.acquire() as conn:
                await conn.execute(
                    "INSERT INTO mindex.search_sessions (id, user_id, started_at, ended_at, session_data) VALUES ($1, $2, $3, $4, $5::jsonb) ON CONFLICT (id) DO UPDATE SET ended_at = EXCLUDED.ended_at, session_data = EXCLUDED.session_data",
                    str(session.id),
                    session.user_id,
                    session.started_at,
                    session.ended_at,
        ...
        try:
            async with self._pool.acquire() as conn:
                row = await conn.fetchrow(
                    "SELECT * FROM mindex.search_sessions WHERE id = $1", str(session_id)
                )
            ...
            for species_id, score in self._calculate_interests(session):
                if score >= 0.7:
                    await myca_memory.remember(
                        ...
                        importance=score,
                        tags=["search", "user_interest", session.user_id],
                    )
            if session.has_ai_breakthrough():
                await myca_memory.remember(
                    content={
                        ...
                        "user_id": session.user_id,
                        "session_id": str(session.id),
                        "summary": session.get_ai_summary(),
                    },
                    layer=MemoryLayer.EPISODIC,
                    importance=0.8,
                    tags=["search", "ai_discovery", session.user_id],
                )
        except Exception as e:
