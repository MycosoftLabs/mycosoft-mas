...
class PostgresBackend(MemoryBackend):
    ...
    def __init__(self):
        ...
        self._url = os.getenv(
            "DATABASE_URL",
            os.getenv(
                "MINDEX_DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/mycosoft"
            ),
        )
            ...
            async with self._pool.acquire() as conn:
                if key:
                    rows = await conn.fetch(
                        "SELECT * FROM memory.entries WHERE scope = $1 AND namespace = $2 AND key = $3 AND (expires_at IS NULL OR expires_at > NOW())",
                        scope.value,
                        namespace,
                        key,
                    )
                    ...
                    rows = await conn.fetch(
                        "SELECT * FROM memory.entries WHERE scope = $1 AND namespace = $2 AND (expires_at IS NULL OR expires_at > NOW())",
                        scope.value,
                        namespace,
                    )
                if rows:
                    ...
                    await conn.execute(
                        "UPDATE memory.entries SET access_count = access_count + 1, last_accessed_at = NOW() WHERE id = ANY($1::uuid[])",
                        ids,
                    )
                entries = []
                for row in rows:
        ...
        try:
            async with self._pool.acquire() as conn:
                result = await conn.execute(
                    "DELETE FROM memory.entries WHERE scope = $1 AND namespace = $2 AND key = $3",
                    scope.value,
                    namespace,
                    key,
                )
    ...
    async def write(self, entry: MemoryEntry) -> bool:
        ...
        try:
            ...
            point_id = hashlib.md5(
                f"{entry.scope}:{entry.namespace}:{entry.key}".encode()
            ).hexdigest()
            self._client.upsert(
                collection_name=self._collection_name,
                points=[
...
class UnifiedMemoryService:
    ...
    def _log_operation(
        ...
        logger.log(
            level, f"Memory {operation}: {scope.value}/{namespace}/{key or '*'} -> {backends}"
        )
    ...
    async def write(self, entry: MemoryEntry) -> bool:
        ...
        for backend in backends:
            ...
            if result:
                ...
                success = False
        self._log_operation(
            "write", entry.scope, entry.namespace, entry.key, success, backend_names
        )
        return success
    ...
    async def get_user_profile(self, user_id: str) -> Optional[UserProfile]:
        ...
        if entries:
            ...
            return UserProfile(
                ...
                memory_consent=data.get("memory_consent", True),
            )
        return None
    ...
    async def update_user_profile(self, profile: UserProfile) -> bool:
        entry = MemoryEntry(
            ...
            namespace=f"profile:{profile.user_id}",
            key="profile",
    ...
    async def summarize_conversation(
        self, conversation_id: str, archive_to_user: Optional[str] = None
    ) -> Optional[str]:
        ...
        if archive_to_user:
            archive_entry = MemoryEntry(
                scope=MemoryScope.USER,
                namespace=f"conversations:{archive_to_user}",
    ...
    async def health_check(self) -> Dict[str, Any]:
        results = {
            ...
            "postgresql": await self._postgres.health_check(),
    ...
    async def cleanup_expired(self) -> int:
        ...
        try:
            async with self._postgres._pool.acquire() as conn:
                result = await conn.execute(
                    "DELETE FROM memory.entries WHERE expires_at IS NOT NULL AND expires_at < NOW()"
                )
