...
class VRAMSnapshot:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "utilization_percent": (
                round(self.used_vram_mb / self.total_vram_mb * 100, 1)
...
class GPUMemory:
    ...
    async def record_model_unload(
        self, model_name: str, triggered_by: str = "api"
    ...
    async def record_inference(
        ...
        if model_name in self._usage_patterns:
            ...
            total_time = (
                pattern.avg_inference_time_ms * (pattern.total_inference_calls - 1)
                + inference_time_ms
            )
    ...
    async def get_vram_history(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        return [s.to_dict() for s in self._vram_snapshots if s.timestamp > cutoff]

    async def get_usage_patterns(self) -> Dict[str, Any]:
        """Get all model usage patterns."""
        return {name: pattern.to_dict() for name, pattern in self._usage_patterns.items()}

    async def get_preload_recommendations(
        self, available_vram_mb: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for name, pattern in self._usage_patterns.items():
            ...
            if pattern.last_used:
                hours_since_use = (
                    datetime.now(timezone.utc) - pattern.last_used
                ).total_seconds() / 3600
                recency_score = max(0, 24 - hours_since_use) / 24 * 10

            total_score = (
                hourly_score * 2 + daily_score + recency_score + pattern.total_inference_calls / 100
            )
    ...
    async def can_load_model(self, vram_mb: int) -> Tuple[bool, str]:
        ...
        unloadable_vram = sum(info.get("vram_mb", 0) for info in self._loaded_models.values())
        ...
        return (
            False,
            f"Insufficient VRAM: {available}MB available, {vram_mb}MB needed, only {unloadable_vram}MB can be freed",
        )
    ...
    async def get_context_for_llm(self) -> Dict[str, Any]:
        ...
        model_list = [
            f"{name} ({info.get('vram_mb', 0)}MB)" for name, info in self._loaded_models.items()
        ]
    ...
    async def _update_usage_pattern(
        self, model_name: str, model_type: GPUModelType, vram_mb: int, load_time_ms: int
    ) -> None:
        ...
        if model_name not in self._usage_patterns:
            self._usage_patterns[model_name] = ModelUsagePattern(
                model_type=model_type, model_name=model_name, vram_mb=vram_mb
            )
