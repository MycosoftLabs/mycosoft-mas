...
class MYCAMemory:
    ...
    def __init__(self):
        ...
        if os.getenv("MINDEX_DATABASE_URL") or not _is_test_environment():
            ...
            logger.warning(
                "MINDEX_DATABASE_URL not set under test; using in-memory persistent backend fallback."
            )
    ...
    async def recall(
        self, entry_id: Optional[UUID] = None, query: Optional[MemoryQuery] = None
    ) -> List[MemoryEntry]:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {"layers": {}, "total_memories": 0, "initialized": self._initialized}
