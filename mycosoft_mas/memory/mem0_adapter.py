...
class LoCoMoFactExtractor:
    ...
    async def extract_facts(
        self, text: str, context: Optional[Dict[str, Any]] = None, use_llm: bool = False
    ) -> List[ExtractedFact]:
                ...
                if "favorite" in pattern:
                    ...
                    facts.append(
                        ExtractedFact(
                            ...
                            subject="user",
                            predicate=(
                                "prefers" if "like" in pattern or "love" in pattern else "dislikes"
                            ),
                ...
                async with client.post(
                    ...
                    json={
                        ...
                        "messages": [
                            {
                                "role": "system",
                                "content": "You are a fact extraction assistant. Output only valid JSON.",
                            },
...
class Mem0Adapter:
    ...
    async def add(
        ...
        return {"message": f"Added {len(added_memories)} memories", "memories": added_memories}
                ...
                for i in range(len(cat_memories)):
                    for j in range(i + 1, len(cat_memories)):
                        ...
                        if similarity > 0.9:
                            # Merge memories
                            logger.info(
                                f"Consolidating similar memories: {cat_memories[i].id} and {cat_memories[j].id}"
                            )
                            consolidated += 1
