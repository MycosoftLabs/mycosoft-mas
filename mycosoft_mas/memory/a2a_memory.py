...
class A2AMemoryIntegration:
    ...
    async def broadcast_memory(
        ...
        await self._broker.add_to_stream(
            self.STREAM_SHARED,
            {
    ...
    async def query_shared_memory(
        self, requester_id: str, query: str, tags: Optional[List[str]] = None, timeout: float = 5.0
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def _handle_query(self, data: str) -> None:
        ...
        try:
            ...
            results = await self._coordinator.agent_recall(
                agent_id="shared", query=query, tags=tags + ["shared"], layer="semantic", limit=20
            )
            ...
            response = MemoryMessage(
                ...
                timestamp=datetime.now(timezone.utc).isoformat(),
                content={"query_id": message.id, "results": results},
            )
    ...
    def register_handler(self, message_type: str, handler: Callable[[MemoryMessage], Any]) -> None:
    ...
    async def get_shared_memories(
        ...
        try:
            ...
            messages = await self._broker.redis.xrange(
                self.STREAM_SHARED, min=since_id or "-", max="+", count=limit
            )
