...
class MemoryCoordinator:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            from mycosoft_mas.memory.myca_memory import (
                MemoryLayer,
                MemoryQuery,
                get_myca_memory,
            )
    ...
    def _ensure_initialized(self) -> None:
        ...
        if not self._initialized:
            raise RuntimeError(
                "MemoryCoordinator not initialized. Call await coordinator.initialize() first."
            )
    ...
    async def get_recent_episodes(
        self, agent_id: Optional[str] = None, event_type: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    async def store_system_config(self, namespace: str, key: str, config: Dict[str, Any]) -> UUID:
        """
        ...
        return await self.agent_remember(
            ...
            content={"type": "config", "key": key, "config": config},
            layer="system",
            importance=1.0,
            tags=["config", namespace, key],
        )

    async def load_system_config(self, namespace: str, key: str) -> Optional[Dict[str, Any]]:
        """Load system configuration."""
        self._ensure_initialized()

        results = await self.agent_recall(
            agent_id=f"system:{namespace}", tags=["config", key], layer="system", limit=1
        )
    ...
    async def semantic_search(
        self, query: str, limit: int = 5, collections: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if self._myca_memory and hasattr(self._myca_memory, "search"):
            try:
                memory_results = await self._myca_memory.search(query=query, limit=limit)
                if memory_results:
                    results.extend(
                        [
                            {
