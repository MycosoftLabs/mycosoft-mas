...
class MASCheckpointer:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            db_url = os.getenv("MINDEX_DATABASE_URL", os.getenv("MINDEX_DATABASE_URL"))
            ...
            try:
                async with self._pool.acquire() as conn:
                    ...
                    if row:
                        ...
                        metadata = (
                            CheckpointMetadata(**json.loads(row["metadata"]))
                            if row["metadata"]
                            else None
                        )
                        parent_config = (
                            self._make_config(thread_id, row["parent_id"])
                            if row["parent_id"]
                            else None
                        )
        ...
        if self._memory:
            try:
                results = await self._memory.agent_recall(
                    agent_id=f"langgraph:{thread_id}", tags=["checkpoint"], layer="system", limit=1
                )
            ...
            try:
                await self._memory.agent_remember(
                    ...
                    content={
                        ...
                        "metadata": {"source": metadata.source, "step": metadata.step},
                    },
                    layer="system",
                    importance=0.8,
