...
class GraphIndexer:
    ...
    async def build_graph(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Graph built: {results['nodes_created']} nodes, {results['edges_created']} edges"
            )
    ...
    async def _index_agents(self) -> Dict[str, int]:
        ...
        for agent in agents:
            ...
            if mas_node:
                edge = Edge(
                    source_id=mas_node.id, target_id=agent_node.id, edge_type=EdgeType.CONTAINS
                )
            ...
            if api.system_id:
                ...
                for system in systems:
                    if system.id == api.system_id:
                        system_node = await self._graph.find_node_by_name(
                            system.name, NodeType.SYSTEM
                        )
                        if system_node:
                            edge = Edge(
    ...
    async def _index_devices(self) -> Dict[str, int]:
        ...
        for device in devices:
            ...
            if mycobrain:
                edge = Edge(
                    source_id=mycobrain.id, target_id=device_node.id, edge_type=EdgeType.MANAGES
                )
                await self._graph.add_edge(edge)
                edges += 1
    ...
    async def _create_relationships(self) -> Dict[str, int]:
        ...
        for db_name, system_name, purpose in databases:
            db_node = Node(
                node_type=NodeType.DATABASE, name=db_name, properties={"purpose": purpose}
            )
