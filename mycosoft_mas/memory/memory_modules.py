...
class ConversationMemory:
    ...
    def add_turn(
        ...
        turn = ConversationTurn(id=uuid4(), role=role, content=content, metadata=metadata or {})
        ...
        if self._turn_count % self._summary_interval == 0:
            self._create_summary(len(self._turns) - self._summary_interval, len(self._turns))
    ...
    def _create_summary(self, start: int, end: int) -> ConversationSummary:
        ...
        summary = ConversationSummary(
            id=uuid4(), start_turn=start, end_turn=end, summary=summary_text, key_topics=topics
        )
    ...
    def get_full_context_with_summaries(self) -> Dict[str, Any]:
        ...
        return {
            "summaries": [{"summary": s.summary, "topics": s.key_topics} for s in self._summaries],
...
class EpisodicMemory:
    ...
    def _prune_least_important(self) -> None:
        ...
        sorted_eps = sorted(self._episodes.values(), key=lambda e: (e.importance, e.timestamp))
...
class SemanticCompressor:
    ...
    def _merge_cluster(self, cluster: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        sorted_cluster = sorted(cluster, key=lambda m: m.get("importance", 0), reverse=True)
...
class AdaptiveDecay:
    ...
    def calculate_strength(
        ...
        if self._strategy == DecayStrategy.LINEAR:
            ...
            return self._importance_weighted_decay(age, importance, access_count, last_accessed)
    ...
    def should_forget(
        ...
        strength = self.calculate_strength(created_at, importance, access_count, last_accessed)
...
class CrossSessionMemory:
    ...
    def __init__(self, database_url: Optional[str] = None):
        ...
        if not self._database_url and not (
            bool(os.getenv("PYTEST_CURRENT_TEST")) or os.getenv("MYCA_TEST_MODE", "").strip() == "1"
        ):
            raise ValueError(
    ...
    async def save_user_context(
        self, user_id: str, context_key: str, context_data: Dict[str, Any]
    ) -> bool:
    ...
    async def load_user_context(self, user_id: str, context_key: str) -> Optional[Dict[str, Any]]:
                ...
                async with self._pool.acquire() as conn:
                    ...
                    if row:
                        data = (
                            ...
                            if isinstance(row["context_data"], str)
                            else row["context_data"]
                        )
    ...
    async def get_user_profile(self, user_id: str) -> Dict[str, Any]:
        ...
        profile = {
            ...
            "interaction_summary": await self.load_user_context(user_id, "interaction_summary")
            or {},
        }
        return profile

    async def update_learned_fact(self, user_id: str, fact: Dict[str, Any]) -> bool:
        """Add a learned fact to user profile."""
        facts = await self.load_user_context(user_id, "learned_facts") or []

        # Check for duplicates
        fact_hash = hashlib.sha256(json.dumps(fact, sort_keys=True).encode()).hexdigest()[:16]
