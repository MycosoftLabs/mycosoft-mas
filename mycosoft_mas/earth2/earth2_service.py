...
class Earth2StudioService:
    ...
    def __init__(
        ...
        self.model_cache_path = model_cache_path or os.getenv(
            "EARTH2_MODEL_CACHE", "/opt/earth2/models"
        )
    ...
    async def _load_model(self, model_name: str) -> Any:
        ...
        if not EARTH2_AVAILABLE:
            raise RuntimeError(
                "Earth2Studio is not installed. Install with: pip install nvidia-earth2studio"
            )
        ...
        try:
            ...
            if EARTH2_AVAILABLE:
                ...
                model = await self._load_model(
                    params.model.value if hasattr(params.model, "value") else params.model
                )

                # Run forecast in executor
                loop = asyncio.get_event_loop()
                outputs = await loop.run_in_executor(
                    None, self._execute_forecast, model, params, output_dir
                )
            else:
            ...
            await self._record_forecast_to_memory(
                model=str(params.model.value if hasattr(params.model, "value") else params.model),
                location={
                    "lat": (params.spatial_extent.min_lat + params.spatial_extent.max_lat) / 2,
            ...
            await self._record_model_error(
                str(params.model.value if hasattr(params.model, "value") else params.model)
            )
    ...
    def _execute_forecast(
        self, model: Any, params: ForecastParams, output_dir: Path
    ) -> List[ForecastOutput]:
    ...
    async def _simulate_forecast(
        self, params: ForecastParams, output_dir: Path
    ) -> List[ForecastOutput]:
        """Simulate forecast for development when Earth2Studio not available."""
        await asyncio.sleep(0.1)  # Simulate processing time
        return self._generate_mock_forecast_outputs(params, output_dir)

    def _generate_mock_forecast_outputs(
        self, params: ForecastParams, output_dir: Path
    ) -> List[ForecastOutput]:
    ...
    async def run_spore_dispersal(
        ...
        result = SporeDispersalResult(
            ...
            params=params,
            species=(
                params.species or (params.species_filter[0] if params.species_filter else None)
            ),
