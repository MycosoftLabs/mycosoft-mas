...
class NatureOSClient(BaseClient):
    ...
    async def list_projects(self, status: Optional[str] = None, limit: int = 20) -> ClientResponse:
        """
    ...
    async def update_project_status(
        self, project_id: str, status: str, notes: Optional[str] = None
    ) -> ClientResponse:
        """
        ...
        """
        data = {"status": status, **({"notes": notes} if notes else {})}
    ...
    async def allocate_resources(
        self, project_id: str, resources: List[Dict[str, Any]]
    ) -> ClientResponse:
        """
