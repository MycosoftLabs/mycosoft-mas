...
import time
from mycosoft_mas.agents.security.auth_service import auth_service
...
def log_audit(action: str, agent_id: str, requester: str):
    entry = {
        "action": action,
        "agent_id": agent_id,
        "requester": requester,
...
async def restart_agent(agent_id: str, user=Depends(auth_service.verify_token)):
    """Restart a specific agent."""
    try:
        await agent_manager.restart_agent(agent_id)
        log_audit("restart", agent_id, user.get("sub", "unknown"))
        return {"status": "success"}
    except Exception as e:
...
@app.get("/api/metrics")
async def dashboard_metrics(user=Depends(auth_service.verify_token)):
    """Secure endpoint for dashboard metrics (requires superuser JWT)."""
    try:
        metrics_data = generate_latest()
        return HTMLResponse(content=metrics_data, media_type=CONTENT_TYPE_LATEST)
    except Exception as e:
