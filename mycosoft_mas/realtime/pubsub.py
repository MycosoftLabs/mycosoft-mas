...
class WebSocketHub:
    ...
    async def publish(
        self, channel: str, data: Dict[str, Any], sender: Optional[str] = None
    ) -> int:
        """Publish message to channel."""
        message = ChannelMessage(
            channel=channel, data=data, timestamp=datetime.utcnow().isoformat(), sender=sender
        )
...
class ChannelManager:
    ...
    def get_channel_name(
        self, category: str, region: Optional[str] = None, entity_type: Optional[str] = None
    ) -> str:
    ...
    async def subscribe_to_timeline(
        self, connection_id: str, entity_types: List[str], region: Optional[str] = None
    ) -> List[str]:
        """Subscribe to timeline channels."""
        channels = []

        for entity_type in entity_types:
            channel = self.get_channel_name("timeline", region=region, entity_type=entity_type)
