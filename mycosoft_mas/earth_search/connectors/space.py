...
class SpaceConnector(BaseConnector):
    ...
    async def search(
        ...
        if EarthSearchDomain.SOLAR_FLARES in relevant or EarthSearchDomain.SPACE_WEATHER in relevant:
    ...
    async def _search_solar_flares(self, temporal: Optional[TemporalFilter], limit: int) -> List[EarthSearchResult]:
        ...
        start = (temporal.start if temporal and temporal.start else now - timedelta(days=30)).strftime("%Y-%m-%d")
