...
class UnifiedMemoryBridge:
    ...
    def __init__(
        self, mindex_url: Optional[str] = None, mas_url: Optional[str] = None, timeout: float = 30.0
    ):
        self._mindex_url = mindex_url or os.getenv("MINDEX_URL", "http://192.168.0.189:8000")
        self._mas_url = mas_url or os.getenv("MAS_URL", "http://192.168.0.188:8001")
    ...
    async def memory_read(
        self, scope: MemoryScope, namespace: str, key: Optional[str] = None
    ) -> Optional[Any]:
        """Read from memory."""
        client = await self._get_client()
        ...
        response = await client.post(
            ...
            json={"scope": scope.value, "namespace": namespace, "key": key},
        )
        response.raise_for_status()
        data = response.json()
        return data.get("value")

    async def memory_delete(self, scope: MemoryScope, namespace: str, key: str) -> bool:
        """Delete from memory."""
        client = await self._get_client()
    ...
    async def memory_list_keys(self, scope: MemoryScope, namespace: str) -> List[str]:
        """List keys in a namespace."""
        client = await self._get_client()

        response = await client.get(f"{self._mas_url}/api/memory/list/{scope.value}/{namespace}")
        response.raise_for_status()
    ...
    async def registry_list_apis(self, system: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        response = await client.get(f"{self._mas_url}/api/registry/apis", params=params)
        response.raise_for_status()
    ...
    async def registry_list_devices(
        ...
        response = await client.get(f"{self._mas_url}/api/registry/devices", params=params)
        response.raise_for_status()
        return response.json().get("devices", [])

    async def registry_update_device(
        self, device_id: str, status: str, telemetry: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    async def graph_list_nodes(
        ...
        response = await client.get(f"{self._mas_url}/api/graph/nodes", params=params)
        response.raise_for_status()
    ...
    async def graph_get_subgraph(self, node_id: str, depth: int = 2) -> Dict[str, Any]:
        """Get a subgraph centered on a node."""
        client = await self._get_client()

        response = await client.get(
            f"{self._mas_url}/api/graph/subgraph/{node_id}", params={"depth": depth}
        )
        response.raise_for_status()
        return response.json()
    ...
    async def audit_query(
        self, user_id: Optional[str] = None, action: Optional[str] = None, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """Query audit log."""
        client = await self._get_client()
        ...
        response = await client.get(f"{self._mas_url}/api/security/audit/query", params=params)
        response.raise_for_status()
