...
class MobilityFingerAdapter:
    """A2A adapter for mobility/infrastructure external agents."""

    def __init__(self, client: Optional[A2AClient] = None, base_url: Optional[str] = None) -> None:
        self.client = client or A2AClient()
        self.base_url = (base_url or os.getenv("A2A_MOBILITY_URL", "")).rstrip("/")

    async def ask(self, prompt: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        if not self.base_url:
            raise RuntimeError("A2A_MOBILITY_URL is not configured")
