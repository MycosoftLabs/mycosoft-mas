...
class EnvironmentalClient:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.purpleair_key = self.config.get("purpleair_api_key") or os.getenv(
            "PURPLEAIR_API_KEY", ""
        )
        ...
        self.breezometer_key = self.config.get("breezometer_api_key") or os.getenv(
            "BREEZOMETER_API_KEY", ""
        )
    ...
    async def get_air_quality_measurements(
        ...
        headers = {"X-API-Key": self.openaq_key} if self.openaq_key else {}
        response = await client.get(
            "https://api.openaq.org/v2/measurements", params=params, headers=headers
        )
        response.raise_for_status()
        return response.json()

    async def get_openaq_locations(
        self, country: Optional[str] = None, city: Optional[str] = None, limit: int = 100
    ) -> Dict[str, Any]:
        ...
        headers = {"X-API-Key": self.openaq_key} if self.openaq_key else {}
        response = await client.get(
            "https://api.openaq.org/v2/locations", params=params, headers=headers
        )
        response.raise_for_status()
        return response.json()
    ...
    async def get_airnow_current(
        ...
        client = await self._get_client()
        params = {"format": "application/json", "API_KEY": self.airnow_key, "distance": distance}
    ...
    async def get_airnow_forecast(
        self, zip_code: str, date: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        client = await self._get_client()
        params = {"format": "application/json", "API_KEY": self.airnow_key, "zipCode": zip_code}
        if date:
            params["date"] = date

        response = await client.get("https://www.airnowapi.org/aq/forecast/zipCode/", params=params)
        response.raise_for_status()
        return response.json()
    ...
    # IQAir APIs
    async def get_iqair_nearest(self, lat: float, lon: float) -> Dict[str, Any]:
    ...
    async def get_radiation_measurements(
        self, lat: float, lon: float, distance: int = 10000
    ) -> List[Dict[str, Any]]:
        """Get radiation measurements from Safecast."""
        client = await self._get_client()
        params = {"latitude": lat, "longitude": lon, "distance": distance}
        if self.safecast_key:
            params["api_key"] = self.safecast_key

        response = await client.get("https://api.safecast.org/measurements.json", params=params)
        response.raise_for_status()
        return response.json()
    ...
    async def get_breezometer_air_quality(
        ...
        response = await client.get(
            ...
            params={"lat": lat, "lon": lon, "key": self.breezometer_key, "features": features},
        )
        response.raise_for_status()
        return response.json()

    async def get_breezometer_pollen(self, lat: float, lon: float) -> Dict[str, Any]:
        ...
        response = await client.get(
            ...
            params={"lat": lat, "lon": lon, "key": self.breezometer_key, "days": 3},
        )
        response.raise_for_status()
        return response.json()

    # Ambee APIs
    async def get_ambee_air_quality(self, lat: float, lon: float) -> Dict[str, Any]:
    ...
    async def get_ambee_fire(self, lat: float, lon: float) -> Dict[str, Any]:
