...
class FinancialMarketsClient:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.coinmarketcap_key = self.config.get("coinmarketcap_key") or os.getenv(
            "COINMARKETCAP_API_KEY", ""
        )

        self.alpha_vantage_key = self.config.get("alpha_vantage_key") or os.getenv(
    ...
    async def get_crypto_quotes(self, symbols: List[str], convert: str = "USD") -> Dict[str, Any]:
    ...
    async def get_crypto_info(self, symbols: List[str]) -> Dict[str, Any]:
    ...
    async def get_polygon_quote(self, ticker: str) -> Dict[str, Any]:
    ...
    async def get_finnhub_quote(self, symbol: str) -> Dict[str, Any]:
        ...
        response = await client.get(
            "https://finnhub.io/api/v1/quote", params={"symbol": symbol, "token": self.finnhub_key}
        )
    ...
    async def get_coingecko_prices(
        self, ids: List[str], vs_currencies: str = "usd"
    ) -> Dict[str, Any]:
    ...
    async def get_coingecko_markets(
        self, vs_currency: str = "usd", order: str = "market_cap_desc", per_page: int = 100
    ) -> List[Dict[str, Any]]:
