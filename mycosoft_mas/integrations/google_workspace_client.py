...
SERVICE_ACCOUNT_KEY = os.environ.get(
    ...
    os.environ.get(
        "GOOGLE_SERVICE_ACCOUNT_JSON_PATH", "/opt/myca/credentials/google/service_account.json"
    ),
)
...
class GoogleWorkspaceClient:
    ...
    def _get_credentials(self, scopes: List[str]):
        ...
        if path.startswith("{") or os.environ.get("GOOGLE_SERVICE_ACCOUNT_JSON"):
            ...
            creds_dict = (
                json.loads(path)
                if path.startswith("{")
                else json.loads(os.environ["GOOGLE_SERVICE_ACCOUNT_JSON"])
            )
    ...
    async def replace_text_in_doc(
        ...
        requests = [{"replaceAllText": {"containsText": {"text": find}, "replaceText": replace}}]
        return (
            self._get_docs()
            .documents()
            .batchUpdate(
