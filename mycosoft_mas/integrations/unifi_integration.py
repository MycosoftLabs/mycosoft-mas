...
class UniFiIntegration:
    ...
    def block_client(self, mac: str) -> bool:
        ...
        try:
            self._request(f"s/{self.site}/cmd/stamgr", "POST", {"cmd": "block-sta", "mac": mac})
    ...
    def unblock_client(self, mac: str) -> bool:
        ...
        try:
            self._request(f"s/{self.site}/cmd/stamgr", "POST", {"cmd": "unblock-sta", "mac": mac})
    ...
    def reconnect_client(self, mac: str) -> bool:
        ...
        try:
            self._request(f"s/{self.site}/cmd/stamgr", "POST", {"cmd": "kick-sta", "mac": mac})
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            response = self._session.get(
                f"https://{self.host}:{self.port}/proxy/network/api/s/{self.site}/self", timeout=10
            )

            if response.status_code == 200:
                return {"status": "online", "host": self.host, "site": self.site}
            else:
