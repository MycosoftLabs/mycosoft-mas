...
class UnifiedIntegrationManager:
    ...
    async def initialize(self) -> None:
        ...
        for name, status in health_results.items():
            if status.get("status") == "ok":
                ...
                self.logger.error(
                    f"✗ {name} integration failed: {status.get('error', 'Unknown error')}"
                )
    ...
    async def sync_mindex_to_notion(
        self, database_id: str, taxon_ids: Optional[List[int]] = None, limit: int = 100
    ) -> Dict[str, Any]:
        """
            ...
            for taxon in taxa:
                try:
                    ...
                    properties = {
                        "Name": {
                            "title": [
                                {"text": {"content": taxon.get("scientific_name", "Unknown")}}
                            ]
                        },
                        "Canonical Name": {
                            "rich_text": [{"text": {"content": taxon.get("canonical_name", "")}}]
                        },
                        "Rank": {"select": {"name": taxon.get("rank", "unknown")}},
                    }
    ...
    async def trigger_n8n_workflow_for_agent(
        self, workflow_id: str, agent_id: str, event_type: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    async def log_to_notion(
        self, database_id: str, title: str, content: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
