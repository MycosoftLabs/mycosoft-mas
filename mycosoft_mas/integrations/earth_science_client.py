...
class EarthScienceClient:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.marine_traffic_key = self.config.get("marine_traffic_key") or os.getenv(
    ...
    async def get_earthquake_count(
        self, start_time: Optional[str] = None, min_magnitude: float = 2.5
    ) -> int:
        """Get count of earthquakes matching criteria."""
        client = await self._get_client()
        params = {"format": "text", "minmagnitude": min_magnitude}
        if start_time:
            params["starttime"] = start_time

        response = await client.get(
            "https://earthquake.usgs.gov/fdsnws/event/1/count", params=params
        )
        response.raise_for_status()
    ...
    async def get_water_levels(
        self, station_id: str, begin_date: Optional[str] = None, end_date: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def get_tide_stations(self, state: Optional[str] = None) -> Dict[str, Any]:
        """Get list of tide stations."""
        client = await self._get_client()
        params = {"type": "tidepredictions", "units": "metric"}

        response = await client.get(
            "https://api.tidesandcurrents.noaa.gov/mdapi/prod/webapi/stations.json", params=params
        )
        response.raise_for_status()
        data = response.json()
        ...
        if state and "stations" in data:
            data["stations"] = [
                s for s in data["stations"] if s.get("state", "").upper() == state.upper()
            ]

        return data

    # NDBC Buoy Data
    async def get_buoy_data(self, station_id: str, data_type: str = "txt") -> str:
    ...
    async def get_buoy_stations(self) -> str:
        ...
        client = await self._get_client()
        response = await client.get("https://www.ndbc.noaa.gov/activestations.xml")
        response.raise_for_status()
        return response.text

    # USGS Water Services
    async def get_streamflow(
        self, site_id: Optional[str] = None, state: Optional[str] = None, period: str = "P7D"
    ) -> Dict[str, Any]:
        """Get streamflow (discharge) data from USGS."""
        client = await self._get_client()
        params = {"format": "json", "parameterCd": "00060", "period": period}  # Discharge
        if site_id:
            params["sites"] = site_id
        if state:
            params["stateCd"] = state

        response = await client.get("https://waterservices.usgs.gov/nwis/iv/", params=params)
        response.raise_for_status()
        return response.json()
    ...
    async def get_water_levels_usgs(
        ...
        client = await self._get_client()
        params = {"format": "json", "parameterCd": "00065", "period": period}  # Gage height
        if site_id:
            params["sites"] = site_id
        if state:
            params["stateCd"] = state
    ...
    # NWS Flood Data
    async def get_flood_status(self, gage_id: str) -> Dict[str, Any]:
        """Get flood status for a river gage."""
        client = await self._get_client()
        response = await client.get(
            "https://water.weather.gov/ahps2/hydrograph_to_xml.php",
    ...
    async def get_reservoir_levels(
        self, office: Optional[str] = None, location: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        response = await client.get(
            "https://cwms-data.usace.army.mil/cwms-data/levels", params=params
        )
        response.raise_for_status()
        return response.json()
    ...
    async def get_seismic_events(
        ...
        response = await client.get("https://service.iris.edu/fdsnws/event/1/query", params=params)
        response.raise_for_status()
    ...
    async def get_tsunami_alerts(self) -> Dict[str, Any]:
        ...
        client = await self._get_client()
        response = await client.get("https://www.tsunami.gov/events/xml/PAAQAtom.xml")
        response.raise_for_status()
