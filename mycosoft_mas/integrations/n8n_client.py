...
class N8NClient:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.webhook_url = self.config.get(
            "webhook_url", os.getenv("N8N_WEBHOOK_URL", "http://localhost:5678")
        ).rstrip("/")

        # API URL (for management operations)
        self.api_url = self.config.get(
            "api_url", os.getenv("N8N_API_URL", self.webhook_url.replace("/webhook", ""))
        ).rstrip("/")

        # Authentication
        self.api_key = self.config.get("api_key", os.getenv("N8N_API_KEY", ""))
        ...
        self.logger.info(
            f"N8N client initialized - Webhook: {self.webhook_url}, API: {self.api_url}"
        )
    ...
    async def _get_webhook_client(self) -> httpx.AsyncClient:
        ...
        if self._webhook_client is None:
            self._webhook_client = httpx.AsyncClient(
                base_url=self.webhook_url, timeout=self.timeout, follow_redirects=True
            )
    ...
    async def _ensure_memory(self):
        ...
        if not os.getenv("MINDEX_DATABASE_URL"):
            return
        try:
            from mycosoft_mas.memory.n8n_memory import ExecutionStatus, N8NMemory, WorkflowCategory
    ...
    async def trigger_workflow(
        self, workflow_id: str, data: Dict[str, Any], method: str = "POST"
    ) -> Dict[str, Any]:
        """
