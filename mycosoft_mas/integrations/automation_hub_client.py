...
class AutomationHubClient:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.pipedream_endpoint = self.config.get("pipedream_endpoint") or os.getenv(
            "PIPEDREAM_ENDPOINT", ""
        )
        self.activepieces_url = self.config.get("activepieces_url") or os.getenv(
    ...
    async def trigger_pipedream(
        self, event: str, data: Dict[str, Any], endpoint: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def trigger(
        self, platform: str, event: str, data: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        if platform == "zapier":
            ...
            return await self.trigger_ifttt(
                event, data.get("value1"), data.get("value2"), data.get("value3")
            )
        elif platform == "make":
    ...
    async def broadcast(
        self, event: str, data: Dict[str, Any], platforms: Optional[list] = None
    ) -> Dict[str, Any]:
        """
