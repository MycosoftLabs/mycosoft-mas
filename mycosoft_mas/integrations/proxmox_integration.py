...
class ProxmoxIntegration:
    ...
    def _request(
        self, node: ProxmoxNode, endpoint: str, method: str = "GET", data: Optional[Dict] = None
    ) -> Dict[str, Any]:
    ...
    def clone_vm(
        self, node_name: str, vmid: int, new_vmid: int, name: str, full: bool = True
    ) -> bool:
        """Clone a VM"""
        for node in self.nodes:
            if node.name == node_name:
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        results = {"healthy": True, "nodes": {}}

        for node in self.nodes:
            try:
                response = self._session.get(f"{node.url}/api2/json/version", timeout=10)
                ...
                results["nodes"][node.name] = {"status": "timeout", "ip": node.ip}
                results["healthy"] = False
            except Exception as e:
                results["nodes"][node.name] = {
                    "status": "unreachable",
