...
class InfrastructureOpsService:
    ...
    async def proxmox_inventory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            client = ProxmoxClient.from_vault(host="192.168.0.202", vault_addr=self.vault_addr)
            ...
            self._audit_log(
                "proxmox.inventory",
                "success",
                {"nodes": len(nodes), "vms": len(vms), "containers": len(containers)},
            )
    ...
    async def proxmox_snapshot(
        self, node: str, vmid: int, snapshot_name: str, confirm: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = client._request(
                "POST", f"nodes/{node}/qemu/{vmid}/snapshot", data={"snapname": snapshot_name}
            )

            self._audit_log(
                "proxmox.snapshot",
                "success",
                {"node": node, "vmid": vmid, "snapshot_name": snapshot_name},
            )
            ...
            self._audit_log(
                "proxmox.snapshot", "failure", {"node": node, "vmid": vmid, "error": str(e)}
            )
            raise
    ...
    async def proxmox_rollback(
        ...
        try:
            ...
            # Rollback
            result = client._request(
                "POST", f"nodes/{node}/qemu/{vmid}/snapshot/{snapshot_name}/rollback"
            )
            ...
            self._audit_log(
                "proxmox.rollback", "failure", {"node": node, "vmid": vmid, "error": str(e)}
            )
            raise
    ...
    async def unifi_topology(self) -> Dict[str, Any]:
        ...
        try:
            client = UniFiClient.from_vault(host="192.168.0.1", vault_addr=self.vault_addr)
            ...
            self._audit_log(
                "unifi.topology", "success", {"devices": len(devices), "clients": len(clients)}
            )
    ...
    async def unifi_client_info(self, mac: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._audit_log("unifi.client_info", "failure", {"mac": mac, "error": str(e)})
            raise
    ...
    async def gpu_run_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "success", "gpus": gpus, "test_passed": True}
    ...
    async def uart_tail(self, lines: int = 100) -> Dict[str, Any]:
        ...
        try:
            if not os.path.exists(uart_log_path):
                return {"status": "no_data", "message": "No UART logs found", "lines": []}

            # Read last N lines
            import subprocess

            result = subprocess.run(
                ["tail", "-n", str(lines), uart_log_path], capture_output=True, text=True, timeout=5
            )
            ...
            return {"status": "success", "lines": log_lines, "total": len(log_lines)}
    ...
    async def nas_status(self) -> Dict[str, Any]:
        ...
        try:
            # Check if mounted
            result = subprocess.run(
                ["mountpoint", "-q", "/mnt/mycosoft-nas"], capture_output=True, timeout=5
            )
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        status = {"timestamp": datetime.utcnow().isoformat() + "Z", "services": {}}
