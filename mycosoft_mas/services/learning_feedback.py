...
class LearningFeedbackService:
    ...
    def __init__(self, data_dir: Optional[str] = None):
        self._data_dir = Path(
            data_dir or os.path.join(os.path.dirname(__file__), "../../data/learning")
        )
        ...
        self._agent_stats: Dict[str, AgentPerformance] = defaultdict(
            lambda: AgentPerformance(agent_id="")
        )

        # Approach success tracking
        self._approach_outcomes: Dict[str, Dict[str, List[bool]]] = defaultdict(
    ...
    def _update_statistics(self, outcome: TaskOutcome) -> None:
        ...
        stats.success_rate = (
            stats.successful_tasks / stats.total_tasks if stats.total_tasks > 0 else 0
        )
        ...
        if outcome.approach_used:
            self._approach_outcomes[outcome.task_type][outcome.approach_used].append(
                outcome.success
            )
        ...
        for approach, results in approaches.items():
            if len(results) >= 5:  # Minimum sample size
                ...
                if success_rate >= 0.8:  # High success rate threshold
                    ...
                    logger.info(
                        f"Learned pattern: {approach} works well for {task_type} ({success_rate:.0%} success)"
                    )
    ...
    def get_learning_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_patterns": [
                ...
                for p in sorted(
                    self._patterns.values(), key=lambda x: x.success_rate, reverse=True
                )[:5]
            ],
