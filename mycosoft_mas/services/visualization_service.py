...
class VisualizationService:
    ...
    def __init__(self, config: Dict):
        ...
        self.metrics_history = {"system_metrics": [], "agent_metrics": {}, "resource_metrics": []}
    ...
    async def update_metrics(self, metrics: Dict):
        ...
        try:
            ...
            self.metrics_history["system_metrics"].append(
                {"timestamp": timestamp, "metrics": metrics.get("system", {})}
            )
            ...
            for agent_id, agent_metrics in metrics.get("agents", {}).items():
                ...
                self.metrics_history["agent_metrics"][agent_id].append(
                    {"timestamp": timestamp, "metrics": agent_metrics}
                )

            # Update resource metrics
            self.metrics_history["resource_metrics"].append(
                {"timestamp": timestamp, "metrics": metrics.get("resources", {})}
            )
    ...
    def get_dashboard_html(self) -> HTMLResponse:
        ...
        return self.templates.TemplateResponse(
            "dashboard.html", {"request": None, "metrics": self.metrics_data, "config": self.config}
        )
