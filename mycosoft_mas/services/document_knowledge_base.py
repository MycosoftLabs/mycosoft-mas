...
class DocumentKnowledgeBase:
    ...
    async def index_document(self, doc_path: str, force: bool = False) -> bool:
        ...
        try:
            ...
            if self.redis_client:
                ...
                await self.redis_client.setex(cache_key, self.CACHE_TTL, json.dumps(doc_info))
    ...
    async def index_all_documents(self, batch_size: int = 10) -> Dict[str, int]:
        ...
        stats = {"total": total, "indexed": indexed, "failed": failed}

        logger.info(f"Indexing complete: {stats}")
        return stats

    async def search(
        self, query: str, limit: int = 10, category: Optional[str] = None, min_score: float = 0.5
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self.redis_client and results:
                await self.redis_client.setex(cache_key, self.CACHE_TTL, json.dumps(results))
    ...
    async def get_related_documents(self, doc_path: str, limit: int = 5) -> List[Dict[str, Any]]:
        """
