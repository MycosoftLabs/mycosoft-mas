...
class DeploymentFeedbackService:
    ...
    def __init__(self, data_dir: Optional[str] = None):
        self._data_dir = Path(
            data_dir or os.path.join(os.path.dirname(__file__), "../../data/deployments")
        )
                ...
                if status:
                    ...
                    if status in ["success", "failed", "rolled_back"]:
                        ...
                        completed = datetime.fromisoformat(
                            deployment.completed_at.replace("Z", "+00:00")
                        )
    ...
    async def verify_deployment(
        self, deployment_id: str, max_attempts: int = 10, delay_seconds: int = 5
    ) -> bool:
    ...
    def get_common_failures(
        ...
        return [{"error": err, "count": count} for err, count in sorted_failures[:limit]]
...
async def main():
    ...
    dep_id = service.start_deployment(target="sandbox", version="1.0.0", commit_sha="abc123")
