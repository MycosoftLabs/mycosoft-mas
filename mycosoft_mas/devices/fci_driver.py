...
class FCISimulatorDriver(FCIDriverBase):
    ...
    def __init__(self, device_id: str, sample_rate: float = 100.0, channels: int = 4):
    ...
    async def send_stimulation(self, command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"[FCI Simulator] Stimulation command accepted for {self.device_id}: {command}")
        return {
            "status": "simulated",
            "device_id": self.device_id,
...
class MycoBrainFCIDriver(FCIDriverBase):
    ...
    async def get_sample(self) -> FCISample:
        ...
        try:
            ...
            logger.warning("[FCI Hardware] Sample timeout, falling back to simulator")
            return await self._fallback.get_sample()
    ...
    async def discover_device(self) -> Optional[str]:
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                response = await client.get("http://192.168.0.188:8001/api/fci/devices/active")
