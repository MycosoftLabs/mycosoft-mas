...
class MycoNodeDevice(BaseDevice):
    ...
    def __init__(self, config: DeviceConfig = None):
        if config is None:
            config = DeviceConfig(
                device_id=uuid4(),
                device_type="myconode",
                capabilities=["soil_moisture", "soil_ph", "bioelectric", "stimulation", "ble"],
            )
    ...
    async def read_sensors(self) -> List[TelemetryReading]:
        ...
        return [
            TelemetryReading(
                sensor_type="soil_moisture", value=45.0, unit="percent", timestamp=now
            ),
            TelemetryReading(sensor_type="soil_ph", value=6.5, unit="pH", timestamp=now),
            TelemetryReading(
                sensor_type="soil_temperature", value=18.0, unit="celsius", timestamp=now
            ),
        ]

    async def send_command(self, command: str, params: Dict[str, Any]) -> Dict[str, Any]:
        if command == "stimulate":
            return await self._stimulate(params)
        if command == "record":
