...
class TruffleBotDevice(BaseDevice):
    ...
    async def read_sensors(self) -> List[TelemetryReading]:
        ...
        return [
            TelemetryReading(
                sensor_type="battery", value=self.battery_percent, unit="percent", timestamp=now
            ),
            TelemetryReading(
                sensor_type="position", value=self.position, unit="meters", timestamp=now
            ),
        ]
    ...
    async def send_command(self, command: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if command == "collect_sample":
            return await self._collect_sample(params)
        if command == "deploy_probe":
            return await self._deploy_probe(params)
