...
class PetraeusDevice(BaseDevice):
    ...
    def __init__(self, config: DeviceConfig = None):
        if config is None:
            config = DeviceConfig(
                device_id=uuid4(),
                device_type="petraeus",
                capabilities=["hdmea_recording", "stimulation", "impedance", "temperature_control"],
            )
    ...
    async def read_sensors(self) -> List[TelemetryReading]:
        ...
        return [
            TelemetryReading(
                sensor_type="dish_temperature", value=25.0, unit="celsius", timestamp=now
            )
        ]
    ...
    async def send_command(self, command: str, params: Dict[str, Any]) -> Dict[str, Any]:
        commands = {
            ...
            "stimulate_pattern": self._stimulate_pattern,
            "measure_impedance": self._measure_impedance,
        }
