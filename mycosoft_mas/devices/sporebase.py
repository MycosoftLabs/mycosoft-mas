...
class SporeBaseDevice(BaseDevice):
    ...
    def __init__(self, config: DeviceConfig = None):
        if config is None:
            config = DeviceConfig(
                ...
                capabilities=["spore_collection", "voc_sensing", "particle_count", "tape_imaging"],
            )
    ...
    async def read_sensors(self) -> List[TelemetryReading]:
        ...
        return [
            TelemetryReading(
                sensor_type="particle_count_pm25", value=15.0, unit="ug/m3", timestamp=now
            ),
            TelemetryReading(sensor_type="voc_index", value=100.0, unit="index", timestamp=now),
            TelemetryReading(
                sensor_type="spore_density", value=50.0, unit="spores/m3", timestamp=now
            ),
        ]

    async def send_command(self, command: str, params: Dict[str, Any]) -> Dict[str, Any]:
        if command == "start_collection":
            return await self._start_collection(params)
        if command == "stop_collection":
            return await self._stop_collection(params)
        if command == "advance_tape":
            return await self._advance_tape(params)
