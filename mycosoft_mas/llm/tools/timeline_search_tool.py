...
class TimelineSearchInput(BaseModel):
    ...
    entity_type: Optional[str] = Field(
        None, description="Type of entity (aircraft, vessel, satellite, wildlife, etc.)"
    )
    time_range: Optional[str] = Field(
        None, description="Time range like 'last 24 hours', 'last week', or ISO date range"
    )
    location: Optional[str] = Field(
        None, description="Location name or coordinates (lat,lng,radius_km)"
    )
                ...
                results = [
                    {
                        ...
                        "timestamp": (
                            row.get("timestamp", "").isoformat() if row.get("timestamp") else None
                        ),
                        "lat": row.get("lat"),
                        "lng": row.get("lng"),
                        "properties": (
                            dict(row.get("properties", {})) if row.get("properties") else {}
                        ),
                    }
                    for row in rows
                ]
