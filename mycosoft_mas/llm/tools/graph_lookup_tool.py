...
class GraphLookupInput(BaseModel):
    ...
    entity_type: Optional[str] = Field(
        None, description="Type of entity (species, device, location, etc.)"
    )
...
class GraphLookupTool:
    ...
    async def _arun(
        ...
        try:
            ...
            nodes = await graph.find_nodes(node_type=node_type, name_contains=entity_name, limit=5)
            ...
            traversal = await graph.get_neighbors(node.id, edge_type=edge_type, max_depth=max_depth)
