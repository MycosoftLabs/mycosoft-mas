    ...
    async def stream(self, messages: List[Message], **kwargs) -> AsyncIterator[str]:
        try:
            ...
            stream = await client.chat.completions.create(
                model=self.model_name, messages=[m.dict() for m in messages], stream=True, **kwargs
            )
    ...
    async def stream(self, messages: List[Message], **kwargs) -> AsyncIterator[str]:
        try:
            ...
            async with client.messages.stream(
                model=self.model_name, max_tokens=4096, system=system_msg or "", messages=chat_msgs
            ) as stream:
...
class LocalLLMWrapper(LLMWrapper):
    ...
    async def generate(
        self, messages: List[Message], temperature: float = 0.7, **kwargs
    ) -> LLMResponse:
        import aiohttp
