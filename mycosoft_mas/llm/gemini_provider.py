...
from .provider import LLMError, LLMProvider, LLMResponse, Message, MessageRole
...
class GeminiProvider(LLMProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not self.api_key:
            raise LLMError(
                "Gemini API key not found in config or environment", provider="gemini", model="N/A"
            )
    ...
    async def chat(
        ...
        **kwargs,
        ...
        raise LLMError(
            f"Gemini chat failed after {self.max_retries} attempts", provider="gemini", model=model
        )

    async def embed(
        self, texts: List[str], model: str = "embedding-001", **kwargs
    ) -> List[List[float]]:
        """Generate embeddings."""
