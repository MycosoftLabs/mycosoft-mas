...
class OpenAIProvider(LLMProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not self.api_key:
            raise LLMError(
                "OpenAI API key not found in config or environment", provider="openai", model="N/A"
            )
    ...
    async def chat(
        ...
        **kwargs,
        ...
        raise LLMError(
            f"OpenAI chat failed after {self.max_retries} attempts", provider="openai", model=model
        )
