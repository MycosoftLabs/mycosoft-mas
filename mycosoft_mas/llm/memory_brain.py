...
class MYCAMemoryBrain:
    ...
    async def _load_memory_context(
        self, user_message: str, context: MemoryAwareContext
    ) -> MemoryAwareContext:
        ...
        try:
            ...
            episodes = await self._memory_coordinator.get_recent_episodes(
                agent_id="myca_brain", limit=self.max_recent_episodes
            )
            ...
            if self._search_memory and context.user_id:
                ...
                if search_session_id:
                    ...
                    if "error" not in search_ctx:
                        ...
                        logger.debug(
                            f"Loaded search context with {len(search_ctx.get('queries', []))} queries"
                        )
            ...
            if self._earth2_memory and context.user_id:
                try:
                    ...
                    if earth2_ctx:
                        ...
                        logger.debug(
                            f"Loaded Earth2 context with {len(earth2_ctx.get('recent_forecasts', []))} forecasts"
                        )
                except Exception as e2e:
            ...
            if self._gpu_memory:
                try:
                    ...
                    if gpu_ctx:
                        ...
                        logger.debug(
                            f"Loaded GPU context: {gpu_ctx.get('utilization_percent', 0)}% VRAM"
                        )
                except Exception as gpue:
                    logger.debug(f"Error loading GPU context: {gpue}")

            logger.debug(
                f"Loaded memory context: {len(memories)} memories, {len(episodes)} episodes"
            )
    ...
    def _build_memory_enhanced_prompt(self, context: MemoryAwareContext) -> str:
        ...
        if context.voice_context and context.voice_context.get("sessions_count", 0) > 0:
            ...
            if context.voice_context.get("last_session_summary"):
                sections.append(
                    f"Last session: {context.voice_context['last_session_summary'][:100]}"
                )
        ...
        if context.earth2_context:
            ...
            if e2.get("favorite_locations"):
                locs = [
                    ...
                    for loc in e2["favorite_locations"][:3]
                    if loc.get("name")
                ]
                if locs:
            ...
            if e2.get("recent_forecasts"):
                ...
                for fc in e2["recent_forecasts"][:2]:
                    sections.append(
                        f"  - {fc.get('location', 'Unknown')} ({fc.get('model', 'fcn')}, {fc.get('lead_time', 24)}h)"
                    )
        ...
        if context.gpu_context:
            ...
            sections.append(
                f"VRAM: {gpu.get('vram_used_mb', 0)}MB / {gpu.get('vram_total_mb', 0)}MB ({gpu.get('utilization_percent', 0)}%)"
            )
    ...
    async def stream_response(
        ...
        try:
            ...
            if memory_prompt:
                ...
                frontier_context.history.insert(
    ...
    async def _store_turn(self, context: MemoryAwareContext, role: str, content: str) -> None:
    ...
    async def _extract_and_store_facts(
        self, context: MemoryAwareContext, user_message: str, response: str
    ) -> None:
    ...
    async def recall_context(self, query: str, user_id: str, limit: int = 10) -> Dict[str, Any]:
        """
        ...
        try:
            memories = await self._memory_coordinator.agent_recall(
                agent_id="myca_brain", query=query, limit=limit
            )

            episodes = await self._memory_coordinator.get_recent_episodes(
                agent_id="myca_brain", limit=5
            )
            ...
            return {
                ...
                "episodes": episodes,
                "profile": (
                    {"user_id": profile.user_id, "preferences": profile.preferences}
                    if profile
                    else None
                ),
            }
