...
class SignalProcessor:
    ...
    async def process(
        ...
        try:
            ...
            result = ProcessedSignal(
                ...
                pattern_class=pattern_class.value,
    ...
    def _find_peaks(self, samples: np.ndarray, threshold: float = 0.5) -> List[int]:
        ...
        for i in range(1, len(samples) - 1):
            if (
                ...
                and samples[i] > threshold
            ):
                peaks.append(i)
        return peaks

    def _classify_pattern_internal(
        self, samples: np.ndarray, features: Dict[str, float]
    ) -> Tuple[PatternClass, float]:
        if features.get("num_peaks", 0) > 10 and features.get("range", 0) > 2.0:
            if features.get("mean_peak_interval", 0) < 10:
                return PatternClass.BURST, 0.85
            return PatternClass.SPIKE, 0.80
        if features.get("zero_crossings", 0) > len(samples) * 0.3:
            if 5 < features.get("spectral_centroid", 0) < 50:
                return PatternClass.OSCILLATION, 0.75
    ...
    def _calculate_anomaly_score(self, features: Dict[str, float], signal_type: str) -> float:
        ...
        if std < r["std"][0]:
            score += 0.3 * (r["std"][0] - std) / r["std"][0]
        elif std > r["std"][1]:
            score += 0.3 * (std - r["std"][1]) / r["std"][1]
