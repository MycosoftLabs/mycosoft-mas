...
class NatureOSGateway:
    def __init__(self, config: Any, services: Dict[str, Any]):
        ...
        self.app = FastAPI(
            title="NatureOS API Gateway",
            version="1.0.0",
            description="Unified API for NatureOS platform",
        )
    ...
    def _setup_routes(self) -> None:
        ...
        async def register_device(reg: DeviceRegistration):
            device_id = await self.services["device_manager"].register_device(
                reg.device_type, reg.dict()
            )
        ...
        async def send_command(device_id: UUID, cmd: CommandRequest):
            cmd_id = await self.services["device_manager"].send_command(
