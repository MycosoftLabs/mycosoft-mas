...
class TestTechnologyTracker:
    ...
    async def test_update_technology(self, tracker):
        ...
        await tracker.register_technology(
            name="test-tech", version="1.0.0", description="Test technology"
        )

        # Update the technology
        await tracker.update_technology(
            name="test-tech", version="2.0.0", description="Updated technology"
        )
    ...
    async def test_error_handling(self, tracker):
        ...
        with pytest.raises(TechnologyTrackerError):
            await tracker.register_technology(
                name="", version="1.0.0", description="Test technology"
            )
    ...
    async def test_concurrent_operations(self, tracker):
        ...
        tasks = [
            tracker.register_technology(
                name=f"test-tech-{i}", version="1.0.0", description=f"Test technology {i}"
            )
            for i in range(5)
        ]
    ...
    async def test_technology_status(self, tracker):
        ...
        await tracker.register_technology(
            name="test-tech", version="1.0.0", description="Test technology", status="active"
        )
    ...
    async def test_technology_metadata(self, tracker):
        ...
        await tracker.register_technology(
            name="test-tech", version="1.0.0", description="Test technology", metadata=metadata
        )
    ...
    async def test_technology_versioning(self, tracker):
        ...
        # Update version
        await tracker.update_technology(name="test-tech", version="2.0.0")
    ...
    async def test_technology_compatibility(self, tracker):
        ...
        await tracker.register_technology(
            ...
            version="1.0.0",
            description="Test technology",
            compatibility={"python": ">=3.8", "os": ["linux", "windows"]},
        )
