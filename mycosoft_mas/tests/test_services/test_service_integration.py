...
class TestServiceIntegration:
    ...
    async def test_service_data_consistency(self, services):
        ...
        assert any(tech["name"] == "test-tech" for tech in evolution_updates["new_technologies"])

        # Check if technology appears in security issues
        assert any(vuln["package"] == "test-tech" for vuln in security_issues["vulnerabilities"])
