...
Provide accurate, scientifically-grounded responses based on the Mycosoft knowledge base."""
    ...
    def from_env(cls) -> "NLMConfig":
        ...
        config.architecture.base_model = os.getenv("NLM_BASE_MODEL", config.architecture.base_model)
        config.architecture.use_lora = os.getenv("NLM_USE_LORA", "true").lower() == "true"
        ...
        config.data.mindex_url = os.getenv("MINDEX_API_URL", config.data.mindex_url)

        # Inference settings
        config.inference.device = os.getenv("NLM_DEVICE", config.inference.device)
        config.inference.fallback_to_api = (
            os.getenv("NLM_FALLBACK_TO_API", "true").lower() == "true"
        )
