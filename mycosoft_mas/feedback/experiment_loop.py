...
class ExperimentLoop:
    ...
    async def create_experiment(
        self, name: str, parameters: Dict[str, Any], hypothesis_id: Optional[UUID] = None
    ) -> Experiment:
        exp = Experiment(
            experiment_id=uuid4(),
    ...
    async def suggest_next_experiment(self, previous_experiments: List[UUID]) -> Dict[str, Any]:
        return {
            ...
            "rationale": "Based on previous results",
            "confidence": 0.5,
        }
    ...
    async def run_loop(
        ...
        for i in range(max_iterations):
            ...
            results.append(
                {"iteration": i, "experiment_id": str(exp.experiment_id), "result": result}
            )
