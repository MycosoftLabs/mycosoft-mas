...
class VoiceMemoryBridge:
    ...
    def _ensure_initialized(self) -> None:
        ...
        if not self._initialized:
            raise RuntimeError(
                "Voice-Memory Bridge not initialized. Call await bridge.initialize() first."
            )
    ...
    async def start_voice_session(
        ...
        voice_session = await self._personaplex.start_session(
            user_id=user_id, speaker_profile=speaker_profile, context=context or {}
        )
        ...
        self._cross_session.start_conversation(user_id)
        ...
        previous_context = await self._autobiographical.get_context_for_response(
            user_id=user_id, current_message="", context_window=5  # Initial context
        )
        ...
        logger.info(
            f"Started voice session {voice_session.id} for {user_name} with full memory context"
        )
    ...
    async def add_voice_interaction(
        ...
        try:
            ...
            emotional_state = {emotion or "neutral": 0.7, "engagement": 0.8}
            ...
            logger.debug("Stored voice interaction in all memory systems")
            return True
    ...
    async def end_voice_session(
        self, session_id: str, summary_override: Optional[str] = None
        ...
        try:
            ...
            await self._coordinator.agent_remember(
                ...
                content={
                    ...
                    "duration_seconds": (
                        voice_session.duration.total_seconds() if voice_session.duration else 0
                    ),
                },
                layer="semantic",
                importance=0.7,
    ...
    async def get_voice_context(
        ...
        try:
            ...
            if session_id:
                ...
                if memory_session_id:
                    recent_history = await self._coordinator.get_conversation_context(
                        session_id=memory_session_id, turns=10
                    )
            ...
            autobio_context = await self._autobiographical.get_context_for_response(
                user_id=user_id, current_message=current_message, context_window=10
            )
            ...
            semantic_results = await self._coordinator.agent_recall(
                agent_id="voice_system", query=current_message, layer="semantic", limit=5
            )
    ...
    async def search_voice_memory(
        ...
        try:
            ...
            semantic_results = await self._coordinator.agent_recall(
                agent_id="voice_system", query=query, layer="semantic", limit=limit
            )
    ...
    async def learn_from_voice(
        self, user_id: str, fact: str, importance: float = 0.7, tags: Optional[List[str]] = None
    ) -> bool:
        """
    ...
    async def update_voice_preference(
        ...
        try:
            ...
            await self._coordinator.update_user_preference(
                user_id=user_id, key=f"voice_{preference_key}", value=preference_value
            )

            logger.info(
                f"Updated voice preference for {user_id}: {preference_key}={preference_value}"
            )
            return True
