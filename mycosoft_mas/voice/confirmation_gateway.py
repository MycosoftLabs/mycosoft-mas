...
class ConfirmationGateway:
    ...
    async def process_response(self, request_id: str, response: str) -> ConfirmationResult:
        if request_id not in self.pending_requests:
            if request_id in self.completed_requests:
                ...
                return ConfirmationResult(
                    request_id=request_id, status=req.status, message="Already processed."
                )
            return ConfirmationResult(
                request_id=request_id, status=ConfirmationStatus.CANCELLED, message="Not found."
            )
        ...
        if request.is_expired():
            ...
            return ConfirmationResult(
                request_id=request_id, status=ConfirmationStatus.EXPIRED, message="Expired."
            )
        ...
        if is_denied:
            ...
            self._move_to_completed(request_id)
            return ConfirmationResult(
                request_id=request_id, status=ConfirmationStatus.DENIED, message="Cancelled."
            )
            ...
            if request.callback:
                try:
                    ...
                    return ConfirmationResult(
                        ...
                        message=f"Confirmed but failed: {e}",
                        action_executed=False,
                    )
            return ConfirmationResult(
                request_id=request_id,
        ...
        if not request.has_attempts_remaining():
            ...
            self._move_to_completed(request_id)
            return ConfirmationResult(
                request_id=request_id, status=ConfirmationStatus.DENIED, message="Max attempts."
            )
    ...
    def get_pending_for_user(self, user_id: str) -> list:
        return [
            r for r in self.pending_requests.values() if r.user_id == user_id and not r.is_expired()
        ]
