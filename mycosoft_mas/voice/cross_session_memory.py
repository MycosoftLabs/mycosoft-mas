...
class CrossSessionMemory:
    ...
    def get_memory_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        user_memories = [
            m for m in self.memories.values() if self._can_access(m, user_id) and not m.is_expired()
        ]

        return {
            "total_memories": len(user_memories),
            "by_type": {
                t.value: len([m for m in user_memories if m.memory_type == t]) for t in MemoryType
            },
            "by_scope": {
                s.value: len([m for m in user_memories if m.scope == s]) for s in MemoryScope
            },
