...
class OrchestratorMCPServer:
    ...
    def __init__(self, mas_url: Optional[str] = None):
        self._mas_url = mas_url or os.getenv("MAS_API_URL", "http://192.168.0.188:8001")
        self._n8n_url = os.getenv("N8N_URL", "http://192.168.0.188:5678")
    ...
    async def _handle_agent_invoke(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await self._make_request(
            ...
            json={"agent_id": agent_id, "task": task, "wait_for_result": wait},
        )
    ...
    async def _handle_agent_status(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await self._make_request("GET", f"{self._mas_url}/api/registry/agents/{agent_id}")

        if "error" in result:
            return {"success": False, "agent_id": agent_id, "error": result["error"]}
        ...
        if include_metrics:
            ...
            metrics_result = await self._make_request(
                "GET", f"{self._mas_url}/api/registry/agents/{agent_id}/metrics"
            )
        ...
        if include_history:
            ...
            history_result = await self._make_request(
                "GET", f"{self._mas_url}/api/orchestrator/tasks?agent_id={agent_id}&limit=10"
            )
    ...
    async def _handle_workflow_trigger(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"success": True, "workflow": workflow_id or workflow_name, "result": result}
        ...
        if method == "initialize":
            return {
                ...
                "result": {
                    ...
                    "serverInfo": {"name": "mycosoft-orchestrator", "version": "1.0.0"},
                    "capabilities": {"tools": {}},
                },
            }
