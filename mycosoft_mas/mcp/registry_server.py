...
class RegistryMCPServer:
    ...
    def __init__(self, workspace_root: Optional[str] = None):
        self._workspace_root = Path(
            workspace_root
            or os.getenv("WORKSPACE_ROOT", "c:/Users/admin2/Desktop/MYCOSOFT/CODE/MAS/mycosoft-mas")
        )
    ...
    async def _handle_add_agent(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results = {"agent_id": agent_id, "actions": []}
...
"""
            ...
            results["actions"].append(
                {"action": "created_cursor_agent", "path": str(cursor_agent_path)}
            )
    ...
    async def _handle_add_api(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Update API catalog
        self._docs_dir / "API_CATALOG_FEB04_2026.md"

        entry = f"\n| {method} | `{path}` | {router} | {description} |"

        results = {
            "api": {"path": path, "method": method},
            "actions": [{"action": "reminder", "message": f"Add to API_CATALOG: {entry}"}],
        }
...
"""
        ...
        if args.get("sync_agents", True) or args.get("sync_skills", True):
            if sync_script.exists():
                try:
                    ...
                    results["actions"].append(
                        {"action": "sync_cursor_system", "success": False, "error": str(e)}
                    )
            else:
                results["actions"].append(
                    {
        ...
        if args.get("sync_docs", True):
            ...
            if docs_script.exists():
                try:
                    ...
                    results["actions"].append(
                        {"action": "build_docs_manifest", "success": result.returncode == 0}
                    )
                except Exception as e:
                    results["actions"].append(
                        {"action": "build_docs_manifest", "success": False, "error": str(e)}
                    )
        ...
        if args.get("sync_gaps", False):
            ...
            if gap_script.exists():
                try:
                    ...
                    results["actions"].append(
                        {"action": "gap_scan", "success": result.returncode == 0}
                    )
                except Exception as e:
                    results["actions"].append(
                        {"action": "gap_scan", "success": False, "error": str(e)}
                    )
    ...
    async def _handle_agent_list(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._agents_dir.exists():
            for agent_file in self._agents_dir.glob("*.md"):
                agent_info = {"id": agent_file.stem, "path": str(agent_file)}
    ...
    async def _handle_docs_rebuild(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not docs_script.exists():
            return {"success": False, "error": "build_docs_manifest.py not found"}
        ...
        if method == "initialize":
            return {
                ...
                "result": {
                    ...
                    "serverInfo": {"name": "mycosoft-registry", "version": "1.0.0"},
                    "capabilities": {"tools": {}},
                },
            }
