...
class BlockchainLedger:
    ...
    def _create_genesis_block(self) -> None:
        genesis = Block(
            block_number=0, previous_hash="0" * 64, timestamp=datetime.now(timezone.utc)
        )
    ...
    def add_entry(
        self, entry_type: str, data: Any, metadata: Optional[Dict[str, Any]] = None
    ) -> LedgerEntry:
        entry = LedgerEntry(
            entry_id=uuid4(),
    ...
    def commit_block(self) -> Optional[Block]:
        ...
        block = Block(
            ...
            entries=[e.dict() for e in self._pending_entries],
        )
