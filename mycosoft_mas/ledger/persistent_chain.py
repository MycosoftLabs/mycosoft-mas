...
class PersistentBlockchainLedger:
    ...
    def __init__(self, database_url: Optional[str] = None):
        self.database_url = database_url or os.getenv(
            "DATABASE_URL", "postgresql://mycosoft:mycosoft@localhost:5432/mycosoft"
        )
        self._pool = None
        ...
        async with self._pool.acquire() as conn:
            async with conn.transaction():
                ...
                merkle_root = self._compute_merkle_root(
                    [e.data_hash for e in self._pending_entries]
                )
    ...
    async def get_entry(self, entry_id: UUID) -> Optional[LedgerEntry]:
        ...
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow("SELECT * FROM ledger.entries WHERE id = $1", entry_id)
            if row:
                return LedgerEntry(
                    id=row["id"],
                    block_number=row["block_number"],
    ...
    async def verify_chain(self) -> bool:
        ...
        async with self._pool.acquire() as conn:
            blocks = await conn.fetch("SELECT * FROM ledger.blocks ORDER BY block_number ASC")
            ...
            for i in range(1, len(blocks)):
                ...
                expected_hash = self._hash_block_data(
                    current["block_number"], current["previous_hash"], current["timestamp"]
                )
