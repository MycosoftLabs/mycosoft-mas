...
class IPNFTManager:
    ...
    async def create_ip_claim(
        self, title: str, creators: List[str], claim_type: str, evidence: Dict[str, Any]
    ) -> IPNFT:
        nft_id = uuid4()
        entry_id = uuid4()
        if self.ledger:
            entry = self.ledger.add_entry(
                "ip_claim", {"title": title, "creators": creators, "evidence": evidence}
            )
            entry_id = entry.entry_id
        nft = IPNFT(
            nft_id=nft_id, entry_id=entry_id, title=title, creators=creators, claim_type=claim_type
        )
