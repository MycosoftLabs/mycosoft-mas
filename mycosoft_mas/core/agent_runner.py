...
class AgentCycleRunner:
    ...
    async def _notification_worker(self):
        ...
        while self.running:
            try:
                notification = await asyncio.wait_for(self._notification_queue.get(), timeout=10)
                ...
                # Log for monitoring
                logger.info(
                    f"[ADMIN NOTIFICATION] {notification.priority.upper()}: {notification.title}"
                )
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_cycles": [asdict(c) for c in list(self.cycles.values())[-10:]],
