...
try:
    from mycosoft_mas.security.crypto_signing import AuditLogSigner, get_audit_signer
...
class ActionAuditLogger:
        ...
        if SIGNING_AVAILABLE:
            try:
                ...
                if self._signer.is_signing_enabled:
                    logger.info(
                        f"Audit log signing enabled with key: {self._signer.get_public_key_fingerprint()}"
                    )
                else:
    ...
    def _ensure_table(self):
        ...
        try:
            with conn.cursor() as cur:
                ...
                # Create indexes
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_action_logs_correlation_id ON audit.action_logs(correlation_id)"
                )
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_action_logs_agent_id ON audit.action_logs(agent_id)"
                )
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_action_logs_status ON audit.action_logs(status)"
                )
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_action_logs_created_at ON audit.action_logs(created_at)"
                )
                conn.commit()
    ...
    async def log_action(
        ...
        log_entry = ActionAuditLog(
            ...
            outputs=None,
            status=(
                ActionStatus.PENDING.value if requires_approval else ActionStatus.EXECUTING.value
            ),
        ...
        try:
            with conn.cursor() as cur:
                ...
                conn.commit()
                logger.debug(
                    f"Logged action {action_id} for agent {agent_name} (signed: {log_entry.signature is not None})"
                )
        except Exception as e:
    ...
    async def update_action(
        ...
        try:
            ...
            if updates:
                ...
                with conn.cursor() as cur:
                    cur.execute(
                        f"UPDATE audit.action_logs SET {', '.join(updates)} WHERE id = %s", params
                    )
                    conn.commit()
        except Exception as e:
