...
class TaskManager:
            ...
            try:
                ...
                processes.append(
                    ProcessInfo(
                        ...
                        network_connections=[
                            conn._asdict() for conn in process_info["connections"]
                        ],
                    )
                )
    ...
    async def _restart_agent(self, agent_id: str) -> Dict[str, str]:
        ...
        try:
            ...
            if not agent:
                raise HTTPException(
                    status_code=404, detail=f"Agent {agent_id} not found in registry"
                )
    ...
    async def _get_dependencies_info(self) -> List[DependencyInfo]:
        ...
        try:
            result = subprocess.run(
                ["poetry", "show", "--outdated", "--json"], capture_output=True, text=True
            )
    ...
    async def _restart_orchestrator(self) -> Dict[str, str]:
        ...
        try:
            ...
            if os.path.exists("/etc/systemd/system/mas-orchestrator.service"):
                logger.warning(
                    "Orchestrator restart requires systemd: sudo systemctl restart mas-orchestrator"
                )
                return {
                    "status": "pending",
    ...
    async def _restart_cluster(self, cluster_name: str) -> Dict[str, str]:
        ...
        try:
            ...
            cluster_agents = [
                a for a in all_agents if cluster_name in a.metadata.get("clusters", [])
            ]
    ...
    async def _autonomous_monitoring(self):
        ...
        while True:
            try:
                ...
                self.orchestrator_health.set(
                    1.0 if orchestrator_status.status == "running" else 0.0
                )

                # Check cluster health
                clusters = await self._get_clusters()
                if clusters:  # Only calculate if there are clusters
                    cluster_health = sum(1 for c in clusters if c.health_status == "healthy") / len(
                        clusters
                    )
                else:
