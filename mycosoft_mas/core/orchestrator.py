...
class Orchestrator:
    ...
    async def _record_health_event(
        self, service_id: str, state: str, error: Optional[str] = None
    ) -> None:
    ...
    async def _save_recovery_state_to_memory(self, service_id: str) -> None:
        ...
        try:
            ...
            await self._memory.agent_remember(
                agent_id="orchestrator",
                content={"type": "recovery_state", "service_id": service_id, "data": recovery_data},
                layer="system",
                importance=1.0,
                    ...
                    if service_id and service_id in self._status:
                        ...
                        self._recovery_states[service_id] = RecoveryState(
                            ...
                            saved_at=datetime.fromisoformat(
                                data.get("saved_at", datetime.now(timezone.utc).isoformat())
                            ),
                            state_data=data,
    ...
    def register_default_services(self) -> None:
        ...
        defaults = [
            ...
            ServiceConfig(
                id="n8n", name="n8n Workflows", health_url="http://localhost:5678/healthz"
            ),
            ServiceConfig(
                id="mindex_db",
            ...
            ServiceConfig(id="redis", name="Redis Cache", host="localhost", port=6379),
        ]
        ...
        if self._memory:
            try:
                history = await self._memory.agent_recall(
                    agent_id="orchestrator", tags=["recovery", service_id], layer="system", limit=5
                )
        ...
        if service_id:
            ...
            if status:
                return {
                    ...
                    "last_healthy": (
                        status.last_healthy.isoformat() if status.last_healthy else None
                    ),
