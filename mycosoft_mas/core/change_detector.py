...
class ChangeDetector:
    ...
    async def _snapshot_git(self) -> Optional[Snapshot]:
        ...
        try:
            ...
            data = {"commit": current_commit, "branch": current_branch}

            snapshot = Snapshot(
                snapshot_id=uuid4(), source="git", data_hash=self._hash_data(data), data=data
            )
    ...
    async def detect_git_changes(self) -> List[DetectedChange]:
        ...
        if old_commit != new_commit:
            change = DetectedChange(
                ...
                severity=ChangeSeverity.MEDIUM,
                source="git",
                description="New commit deployed",
                old_value=old_commit,
                new_value=new_commit,
                metadata={"branch": new_snapshot.data.get("branch")},
            )
            changes.append(change)
            self._emit_change(change)
    ...
    async def detect_health_changes(
        self, current_health: Dict[str, str], previous_health: Dict[str, str]
    ) -> List[DetectedChange]:
