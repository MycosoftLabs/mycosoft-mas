...
from mycosoft_mas.services.document_knowledge_base import DocumentKnowledgeBase, get_knowledge_base
...
async def search_documents(
    ...
    """
    try:
        results = await kb.search(query=q, limit=limit, category=category, min_score=min_score)

        return DocumentSearchResponse(results=results, total=len(results), query=q)
...
async def search_documents_post(
    request: DocumentSearchRequest, kb: DocumentKnowledgeBase = Depends(get_knowledge_base)
) -> DocumentSearchResponse:
    """
    ...
    try:
        ...
        return DocumentSearchResponse(results=results, total=len(results), query=request.query)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Search failed: {str(e)}")
...
async def get_document(
    doc_path: str, kb: DocumentKnowledgeBase = Depends(get_knowledge_base)
...
async def get_related_documents(
    ...
    try:
        ...
        return DocumentSearchResponse(
            results=results, total=len(results), query=f"related to {doc_path}"
        )
    except Exception as e:
...
async def get_statistics(kb: DocumentKnowledgeBase = Depends(get_knowledge_base)) -> dict:
    """
...
async def index_document(
    ...
    try:
        ...
        return {"status": "success", "path": doc_path, "indexed": True}
    except HTTPException:
        raise
    except Exception as e:
...
async def start_watcher(kb: DocumentKnowledgeBase = Depends(get_knowledge_base)) -> dict:
    """
...
async def force_scan(kb: DocumentKnowledgeBase = Depends(get_knowledge_base)) -> dict:
    """
