...
def get_simulator(device_id: str) -> FCISignalSimulator:
    """Get or create a simulator for a device."""
...
async def fci_stream_websocket(
    ...
    simulator = get_simulator(device_id)
    simulator.sample_rate = sample_rate
    ...
    try:
        ...
        send_task = asyncio.create_task(_stream_samples(
            websocket, device_id, simulator, sample_rate
        ))
        receive_task = asyncio.create_task(_receive_commands(
            websocket, device_id, simulator
        ))
...
async def _stream_samples(
    ...
    try:
        while True:
            # Generate sample
            raw_sample = simulator.generate_sample()
...
async def websocket_status():
    ...
    return {
        "active_devices": list(manager.active_connections.keys()),
