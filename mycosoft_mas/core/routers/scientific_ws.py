...
async def broadcast_experiment_step(
    experiment_id: str, step: int, total_steps: int, status: str, message: Optional[str] = None
):
    await ws_manager.broadcast(
        EventType.EXPERIMENT_STEP.value,
        {
            "id": experiment_id,
            "step": step,
...
async def broadcast_telemetry(device_id: str, readings: dict):
    await ws_manager.broadcast(
        EventType.TELEMETRY_UPDATE.value,
...
async def broadcast_mycobrain_result(job_id: str, status: str, result: Optional[Any] = None):
    await ws_manager.broadcast(
        EventType.MYCOBRAIN_RESULT.value, {"jobId": job_id, "status": status, "result": result}
    )
