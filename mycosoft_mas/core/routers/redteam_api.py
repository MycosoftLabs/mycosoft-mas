...
async def run_phishing_simulation(sim_id: str, request: PhishingSimRequest):
    ...
    try:
        ...
        metrics["credentials_submitted"] = (
            int(simulated_users * 0.08) if request.track_credentials else 0
        )
...
async def run_pivot_test(sim_id: str, request: PivotTestRequest):
    ...
    try:
        logger.info(
            f"[RedTeam] Testing pivot from {request.source_network} to {request.target_network}"
        )
        ...
        for protocol in request.protocols:
            for port in test_ports:
                ...
                is_blocked = request.target_network.startswith(
                    "10."
                ) or port in [  # Private network assumed segmented
                    22,
                    3389,
                    5432,
                ]  # Sensitive ports assumed blocked
        ...
        blocked_rate = (
            metrics["connections_blocked"]
            / (metrics["connections_blocked"] + metrics["connections_allowed"])
            * 100
            if (metrics["connections_blocked"] + metrics["connections_allowed"]) > 0
            else 100
        )
...
async def run_exfil_test(sim_id: str, request: ExfilTestRequest):
    ...
    try:
        ...
        for scenario in exfil_scenarios:
            ...
            is_detected = (
                scenario["size"] > 500  # Large transfers detected
                or request.exfil_method == "dns"  # DNS exfil usually detected
                or scenario["chunks"] > 10  # Suspicious fragmentation detected
            )

            is_blocked = is_detected and request.exfil_method not in [
                "icmp"
            ]  # ICMP harder to block
        ...
        detection_rate = (
            metrics["detected_attempts"] / metrics["exfil_attempts"] * 100
            if metrics["exfil_attempts"] > 0
            else 0
        )
...
async def run_credential_test_endpoint(
    ...
    approved_by = await _require_guardian_approval_if_needed(
        "credential_test", request.target_system
    )
...
async def run_phishing_sim_endpoint(
    ...
    return {
        "simulation_id": sim_id,
        "status": "started",
        "message": "Phishing simulation initiated",
    }
