...
async def health_check():
    ...
    return {"status": "healthy", "service": "earth2-memory", "stats": stats}


@router.get("/forecasts/{user_id}", response_model=ForecastHistoryResponse)
async def get_forecast_history(
    user_id: str, limit: int = Query(default=10, ge=1, le=100), model: Optional[str] = None
):
    """Get a user's forecast history."""
    memory = await get_memory()
    forecasts = await memory.get_user_forecasts(user_id, limit=limit, model=model)

    return ForecastHistoryResponse(forecasts=[f.to_dict() for f in forecasts], total=len(forecasts))
...
async def get_model_stats(model: Optional[str] = None):
    ...
    return ModelStatsResponse(model=model, stats=stats)
...
async def check_cached_forecast(request: CachedForecastRequest):
    ...
    if cached:
        return {"cached": True, "forecast": cached.to_dict()}
