...
from mycosoft_mas.memory.persistent_graph import Edge, EdgeType, Node, NodeType, get_knowledge_graph
...
async def create_node(request: NodeCreate):
    ...
    node = Node(
        node_type=NodeType(request.node_type), name=request.name, properties=request.properties
    )
...
async def get_neighbors(
    ...
    return {"neighbors": [n.dict() for n in neighbors], "count": len(neighbors)}
...
async def list_edges(
    edge_type: Optional[str] = Query(None), limit: int = Query(100, ge=1, le=1000)
):
    ...
    return {"edges": [e.dict() for e in edges], "count": len(edges)}
...
async def find_path(request: PathRequest):
    ...
    path = await graph.find_path(
        UUID(request.source_id), UUID(request.target_id), request.max_depth
    )

    if not path:
        return {"found": False, "message": "No path found"}

    return {"found": True, "path": path.dict()}
...
async def trigger_graph_build(background_tasks: BackgroundTasks):
    ...
    return {"status": "building", "message": "Graph build started in background"}
