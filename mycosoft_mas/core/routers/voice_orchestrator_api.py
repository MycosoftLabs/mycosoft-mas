...
class ActionTaken(BaseModel):
    ...
    type: str = Field(
        ..., description="Action type: memory_write, workflow_executed, agent_routed, tool_called"
    )
...
class VoiceOrchestratorResponse(BaseModel):
    ...
    tool_calls: List[ToolCallInfo] = Field(
        default_factory=list, description="Tool calls for debug panel"
    )
    agents_invoked: List[AgentInvocation] = Field(
        default_factory=list, description="Agent invocations for debug panel"
    )
...
class MYCAOrchestrator:
        ...
        if self._n8n_client is None:
            try:
                ...
                self._n8n_client = N8NClient(
                    config={"webhook_url": webhook_url} if webhook_url else {}
                )
            except ImportError:
    ...
    async def _load_conversation_from_db(
        self, conversation_id: str, actions_taken: List[ActionTaken]
    ) -> bool:
        """
        ...
        try:
            ...
            if history:
                ...
                logger.info(
                    f"Loaded {len(history)} turns from DB for conversation {conversation_id[:8]}..."
                )
                return True
    ...
    async def _persist_turn(self, session_id: str, speaker: str, text: str) -> None:
        """Persist a turn to the database via supabase_client."""
        voice_store = await self._get_voice_store()
        if voice_store:
            try:
                await voice_store.add_turn(session_id=session_id, speaker=speaker, text=text)
            except Exception as e:
    ...
    async def process(
        ...
        if request.conversation_id and conv_id not in self._conversations:
            history_loaded_from_db = await self._load_conversation_from_db(conv_id, actions_taken)
        ...
        if coordinator:
            try:
                ...
                actions_taken.append(
                    ActionTaken(
                        type="memory_write", detail={"scope": "conversation", "role": "user"}
                    )
                )
            except Exception as e:
        ...
        if coordinator:
            try:
                await coordinator.add_conversation_turn(
                    session_id=conv_id, role="assistant", content=response_text
                )
            except Exception as e:
