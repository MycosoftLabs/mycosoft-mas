...
async def get_sensor_status():
    ...
    sensor_checks = [
        ...
        (
            "Earth2",
            os.environ.get("EARTH2_API_URL", "http://192.168.0.187:3000/api/earth2"),
            "/health",
        ),
        (
            "NatureOS",
            os.environ.get("NATUREOS_API_URL", "http://192.168.0.187:3000/api/natureos"),
            "/health",
        ),
        ("MINDEX", os.environ.get("MINDEX_API_URL", "http://192.168.0.189:8000"), "/health"),
        (
            "MycoBrain",
            os.environ.get("MYCOBRAIN_API_URL", "http://192.168.0.188:8001/api/devices"),
            "/health",
        ),
    ]
    ...
    for name, base_url, path in sensor_checks:
        ...
        sensors.append({"name": name, "connected": connected, "url": base_url, "error": error})
...
async def get_agents_status():
    ...
    return {"agents": [], "total": 0, "active": 0, "error": "Could not connect to orchestrator"}
...
async def get_world_status():
    ...
    return {"available": False, "state": None, "timestamp": datetime.now(timezone.utc).isoformat()}
