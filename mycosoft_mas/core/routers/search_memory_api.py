...
class FocusRequest(BaseModel):
    ...
    topic: Optional[str] = Field(
        None,
        description="Topic: species, chemistry, genetics, research, taxonomy, gallery, ai_chat",
    )
...
async def update_query_insight(request: QueryInsightRequest, fastapi_request: Request):
    ...
    try:
        ...
        return {"success": True, "session_id": request.session_id, "query_id": request.query_id}
...
async def record_click(request: ClickRequest, fastapi_request: Request):
    ...
    try:
        ...
        return {"success": True, "session_id": request.session_id, "result_id": request.result_id}
...
async def add_ai_turn(request: AITurnRequest, fastapi_request: Request):
    ...
    try:
        ...
        return AITurnResponse(
            message_id=str(message.id), session_id=request.session_id, role=message.role
        )
...
async def end_session(session_id: str, request: Request):
    ...
    try:
        ...
        return EndSessionResponse(
            ...
            top_interests=[
                {"species_id": sid, "score": score} for sid, score in summary.top_interests
            ],
        )
...
async def record_widget_interaction(
    ...
    try:
        from mycosoft_mas.memory.search_memory import SearchTopic, get_search_memory
