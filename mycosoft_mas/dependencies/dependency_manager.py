...
class DependencyManager:
    def __init__(self, project_root: Optional[str] = None):
        ...
        self.version_conflicts: Dict[str, List[Tuple[str, str]]] = (
            {}
        )  # package -> [(agent_id, version)]
    ...
    def register_agent_dependencies(
        self, agent_id: str, dependencies: Dict[str, str]
    ) -> Dict[str, Any]:
    ...
    def _resolve_agent_dependencies(self, agent_id: str) -> Dict[str, Any]:
        ...
        for package, version in agent_deps.items():
            if package in self.dependencies:
                ...
                if current_version != version:
                    ...
                    conflicts[package] = {"current": current_version, "requested": version}
        ...
        return {"status": "success", "message": "Dependencies registered successfully"}

    def add_dependency(
        self, package: str, version: str, agent_id: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def update_dependency(
        self, package: str, version: Optional[str] = None, agent_id: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def install_dependency(self, package: str, version: str) -> Dict[str, Any]:
        result = subprocess.run(
            ["poetry", "add", f"{package}=={version}"], capture_output=True, text=True
        )
        if getattr(result, "returncode", 1) == 0:
            self.dependencies[package] = version
            return {"status": "success"}
    ...
    def install_dependencies(self, agent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if result["status"] == "success":
            self.update_history.append(
                {"package": "all", "timestamp": datetime.now(), "success": True}
            )
        return result
        ...
        if agent_id:
            ...
            for package, version in agent_deps.items():
                ...
                if result["status"] == "success" and result["output"]:
                    ...
                    if len(parts) >= 3:
                        updates[package] = {"current": parts[1], "latest": parts[2]}
