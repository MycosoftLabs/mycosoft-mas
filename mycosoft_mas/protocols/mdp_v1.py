...
class MDPFrame:
    ...
    def to_bytes(self) -> bytes:
        ...
        header = struct.pack(
            "!BHHQ", self.message_type.value, self.sequence, len(self.payload), self.timestamp
        )
    ...
    def from_bytes(cls, data: bytes) -> "MDPFrame":
        ...
        if crc16_received != crc16_calculated:
            raise ValueError(
                f"CRC16 mismatch: received {crc16_received:04x}, calculated {crc16_calculated:04x}"
            )
        ...
        if len(frame_data) < 13 + payload_len:
            raise ValueError(
                f"Payload length mismatch: expected {payload_len}, got {len(frame_data) - 13}"
            )
