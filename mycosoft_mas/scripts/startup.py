...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("logs/startup.log")],
)
    ...
    async def start_services(self) -> bool:
        ...
        try:
            ...
            redis_container = self.docker_client.containers.run(
                "redis", detach=True, ports={"6379/tcp": 6379}, name="mycosoft_redis"
            )
            ...
            prometheus_container = self.docker_client.containers.run(
                ...
                ports={"9090/tcp": 9090},
                volumes={
                    "./prometheus.yml": {"bind": "/etc/prometheus/prometheus.yml", "mode": "ro"}
                },
            ...
            grafana_container = self.docker_client.containers.run(
                "grafana/grafana", detach=True, ports={"3000/tcp": 3000}, name="mycosoft_grafana"
            )
    ...
    async def start_dashboard(self) -> bool:
        ...
        try:
            ...
            dashboard_process = await asyncio.create_subprocess_exec(
                ...
                "-m",
                "uvicorn",
                "mycosoft_mas.monitoring.dashboard:app",
                "--host",
                "0.0.0.0",
                "--port",
                "8000",
