...
from mycosoft_mas.registry.system_registry import CodeFileInfo, SystemRegistry, get_registry
...
class CodeIndexer:
            ...
            try:
                ...
                if not repo_path.exists():
                    ...
                    results["errors"].append(
                        {"repository": repo_name, "error": f"Path not found: {repo_path}"}
                    )
                    continue
                ...
                results["errors"].append({"repository": repo_name, "error": str(e)})

        return results

    async def _index_repository(
        self, repo_name: str, repo_path: Path, extensions: List[str], exclude_dirs: List[str]
    ) -> Dict[str, Any]:
        """Index a single repository."""
        stats = {"file_count": 0, "total_lines": 0, "by_type": {}, "classes": [], "functions": []}
    ...
    def _find_files(
        self, root_path: Path, extensions: List[str], exclude_dirs: List[str]
    ) -> List[Path]:
    ...
    async def _index_file(
        self, repo_name: str, repo_path: Path, file_path: Path
