...
class DeviceRegistry:
    ...
    async def initialize_known_devices(self) -> Dict[str, Any]:
        ...
        for device_config in KNOWN_DEVICES:
            try:
                ...
                results["errors"].append({"device_id": device_config["device_id"], "error": str(e)})
    ...
    async def update_firmware(self, device_id: str, firmware_version: str) -> Optional[DeviceInfo]:
        ...
        device.metadata["firmware_history"].append(
            {"version": old_version, "updated_at": datetime.now(timezone.utc).isoformat()}
        )
    ...
    async def get_firmware_report(self) -> Dict[str, Any]:
        ...
        report = {"by_device_type": {}, "outdated_devices": [], "firmware_versions": {}}
