...
class SystemRegistry:
    ...
    async def register_api(self, api: APIInfo, system_name: str) -> APIInfo:
        ...
        async with self._pool.acquire() as conn:
            ...
            """,
                ...
                api.description,
                api.tags,
                json.dumps(api.request_schema) if api.request_schema else None,
                json.dumps(api.response_schema) if api.response_schema else None,
                api.auth_required,
                api.deprecated,
                json.dumps(api.metadata),
            )
            ...
            for row in rows:
                apis.append(
                    APIInfo(
                        ...
                        tags=row["tags"] or [],
                        request_schema=(
                            json.loads(row["request_schema"]) if row["request_schema"] else None
                        ),
                        response_schema=(
    ...
    async def get_api_count(self) -> int:
        ...
        async with self._pool.acquire() as conn:
            return await conn.fetchval(
                "SELECT COUNT(*) FROM registry.apis WHERE deprecated = FALSE"
            )
    ...
    async def get_registry_stats(self) -> Dict[str, Any]:
        ...
        async with self._pool.acquire() as conn:
            ...
            stats["apis"] = await conn.fetchval(
                "SELECT COUNT(*) FROM registry.apis WHERE deprecated = FALSE"
            )
