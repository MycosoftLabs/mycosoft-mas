...
def audit_blocked_event(
    ...
    """
    try:
        from_agent = getattr(event, "from_agent", None) or (
            event.get("from_agent") if isinstance(event, dict) else "unknown"
        )
        to_agent = getattr(event, "to_agent", None) or (
            event.get("to_agent") if isinstance(event, dict) else "unknown"
        )
        event_type = getattr(event, "type", None) or (
            event.get("type") if isinstance(event, dict) else "unknown"
        )
        trace_id = getattr(event, "trace_id", None) or (
            event.get("trace_id") if isinstance(event, dict) else None
        )
