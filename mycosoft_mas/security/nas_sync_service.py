...
class NASSyncService:
    ...
    async def sync_with_integrity(
        self, source_dir: str, dest_subpath: str, record_in_ledger: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    async def verify_nas_integrity(self, subpath: str) -> Dict[str, Any]:
        ...
        for rel_path, expected_hash in manifest.get("files", {}).items():
            ...
            if actual_hash != expected_hash:
                mismatches.append(
                    {"path": rel_path, "expected": expected_hash, "actual": actual_hash}
                )
            else:
                verified += 1
...
async def sync_to_nas(
    source_dir: str, dest_subpath: str, record_in_ledger: bool = True
) -> Dict[str, Any]:
    """
    ...
    return await service.sync_with_integrity(
        source_dir=source_dir, dest_subpath=dest_subpath, record_in_ledger=record_in_ledger
    )
