...
class LongTermMemory:
    ...
    async def get_activity_history(
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.mindex_url}/api/agent_logs", params=params) as resp:
                    if resp.status == 200:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.qdrant_url}/collections/{self.collection_name}/points/search", json=body
                ) as resp:
                    if resp.status == 200:
...
class UnifiedMemoryManager:
    ...
    async def recall(self, key: str) -> Optional[Any]:
        ...
        # Then try long-term
        history = await self.long_term.get_activity_history(limit=1, action_type="knowledge:store")
        for item in history:
