...
class AgentRuntime:
    ...
    async def start(self):
        ...
        await self.message_broker.subscribe(f"agent:{self.agent_id}", self._handle_message)
    ...
    async def _metrics_handler(self, request: web.Request) -> web.Response:
        ...
        metrics = AgentMetrics(
            ...
            uptime_seconds=(
                int((datetime.utcnow() - self.started_at).total_seconds()) if self.started_at else 0
            ),
        )
    ...
    async def _send_task_response(self, task: AgentTask):
        ...
        if self.message_broker and task.requester_agent:
            ...
            await self.message_broker.publish(f"agent:{task.requester_agent}", response.to_json())
...
async def main():
    ...
    parser.add_argument(
        "--agent-id", default=os.environ.get("AGENT_ID", "agent-" + str(uuid4())[:8])
    )
