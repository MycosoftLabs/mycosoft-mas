...
class AgentPool:
    ...
    async def _ensure_network(self):
        ...
        try:
            ...
            self.docker_client.networks.create(
                self.network_name, driver="bridge", labels={"created_by": "mas-pool"}
            )
            logger.info(f"Created network {self.network_name}")

    async def _discover_agents(self):
        """Discover existing agent containers"""
        containers = self.docker_client.containers.list(filters={"label": "mas.agent=true"})
            ...
            container_config = {
                ...
                "environment": {
                    ...
                    "AGENT_CATEGORY": (
                        ...
                        if isinstance(config.category, AgentCategory)
                ...
                "restart_policy": (
                    {"Name": "unless-stopped"} if config.auto_restart else {"Name": "no"}
                ),
            }
    ...
    async def get_agents_by_category(self, category: AgentCategory) -> List[AgentState]:
        ...
        return [
            ...
            if self.configs.get(a.agent_id) and self.configs[a.agent_id].category == category
        ]
