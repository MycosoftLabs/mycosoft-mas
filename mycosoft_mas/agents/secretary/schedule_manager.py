...
class ScheduleManager:
    ...
    async def save_data(self) -> None:
        ...
        try:
            ...
            with open(prefs_file, "w") as f:
                json.dump(
                    {k: v.name for k, v in self.notification_preferences.items()}, f, indent=2
                )
    ...
    async def set_weekly_schedule(
        self, activity: ActivityType, day_of_week: str, start_time: time, end_time: time
    ) -> None:
        """
    ...
    async def get_current_presence(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_update": {
                s.name: self.last_presence_update[s].isoformat() for s in active_sources
            },
        }
    ...
    async def set_notification_preference(
        self, message_type: str, preference: NotificationPreference
    ) -> None:
        """
    ...
    async def should_notify(self, message_type: str, priority: MessagePriority) -> bool:
        """
        ...
        preference = self.notification_preferences.get(
            message_type, NotificationPreference.WORKING_HOURS
        )
        ...
        if preference == NotificationPreference.ALWAYS:
            ...
            return schedule["activity"] == ActivityType.WORK and presence["location"] in [
                LocationType.LAB,
                LocationType.OFFICE,
            ]
            ...
            return (
                schedule["activity"] != ActivityType.SLEEP
                and presence["location"] != LocationType.BEDROOM
            )
