...
class NotificationManager:
                ...
                with open(notifications_file, "r") as f:
                    ...
                    for notification_data in notifications_data:
                        notification = Notification(
                            ...
                            scheduled_for=(
                                datetime.fromisoformat(notification_data["scheduled_for"])
                                if notification_data.get("scheduled_for")
                                else None
                            ),
                            expires_at=(
                                datetime.fromisoformat(notification_data["expires_at"])
                            ...
                            last_attempt=(
                                datetime.fromisoformat(notification_data["last_attempt"])
                                if notification_data.get("last_attempt")
                                else None
                            ),
                            delivered_at=(
                                datetime.fromisoformat(notification_data["delivered_at"])
                                if notification_data.get("delivered_at")
                                else None
                            ),
                            read_at=(
                                datetime.fromisoformat(notification_data["read_at"])
                                if notification_data.get("read_at")
                                else None
                            ),
                        )
    ...
    async def get_pending_notifications(self) -> List[Notification]:
        ...
        """
        return [self.notifications[notification_id] for notification_id in self.pending_queue]
    ...
    async def get_scheduled_notifications(self) -> List[Notification]:
        ...
        """
        return [self.notifications[notification_id] for notification_id in self.scheduled_queue]

    # Notification Rules

    async def add_notification_rule(
        self, notification_type: NotificationType, rule: Dict[str, Any]
    ) -> None:
        """
        ...
        try:
            ...
            if "schedule_rules" in rule:
                ...
                if "blocked_activities" in schedule_rules:
                    if (
                        current_schedule
                        and current_schedule.activity in schedule_rules["blocked_activities"]
                    ):
                        return False
                ...
                if "blocked_locations" in schedule_rules:
                    if (
                        current_presence
                        and current_presence.location in schedule_rules["blocked_locations"]
                    ):
                        return False
            ...
            if "presence_rules" in rule:
                ...
                if "required_sources" in presence_rules:
                    if (
                        not current_presence
                        or current_presence.source not in presence_rules["required_sources"]
                    ):
                        return False
            ...
            for channel in notification.channels:
                try:
                    if channel == NotificationChannel.EMAIL:
                        ...
                        await self.communication_service.send_telegram(notification.content)
                    elif channel == NotificationChannel.SIGNAL:
                        await self.communication_service.send_signal(notification.content)
                    elif channel == NotificationChannel.DISCORD:
                        await self.communication_service.send_discord(notification.content)
                    elif channel == NotificationChannel.VOICE:
                        await self.communication_service.send_voice(notification.content)
                    elif channel == NotificationChannel.TEXT:
                        await self.communication_service.send_text(notification.content)
                    ...
                    self.logger.error(
                        f"Error delivering notification through {channel.name}: {str(e)}"
                    )
