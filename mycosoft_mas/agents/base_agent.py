...
class BaseAgent(AgentMonitorable, AgentSecurable):
    ...
    def set_myca_orchestrator(self, myca):
        """
        Set the MYCA orchestrator instance for notification routing.
        Should be called by the orchestrator when registering the agent.
        """
        self.myca_orchestrator = myca

    async def send_notification(self, message: str, channel: str = "dashboard", priority: str = "normal", event_type: str = None, context: dict = None):
        """
        Send a notification via MYCA if available, otherwise use the local notification queue.
        Supports rich context: event_type, context.
        This is the preferred way for agents to notify the system or user.
        """
        if hasattr(self, "myca_orchestrator") and self.myca_orchestrator:
            # Route to MYCA (direct call)
            return self.myca_orchestrator.notify(
                message,
                channel,
                priority,
                agent_id=self.agent_id,
            ...
            # Fallback: put in local notification queue
            await self.notification_queue.put({
                "message": message,
                "channel": channel,
                "priority": priority,
