...
class GuardianAgentV2(BaseAgentV2):
    ...
    async def _handle_evaluate_action(self, task: AgentTask) -> Dict[str, Any]:
        ...
        if not approved:
            ...
            logger.warning(
                f"Blocked action: {action_type} (risk: {risk.value}, threshold: {threshold.value})"
            )
        else:
        ...
        return {
            ...
            "action": action_type,
            "reason": (
                None if approved else f"Risk {risk.value} exceeds threshold {threshold.value}"
            ),
        }
    ...
    async def _handle_emergency_lockdown(self, task: AgentTask) -> Dict[str, Any]:
        ...
        logger.critical(
            f"EMERGENCY LOCKDOWN activated for {duration_minutes} minutes. Reason: {reason}"
        )
    ...
    async def _handle_release_lockdown(self, task: AgentTask) -> Dict[str, Any]:
        ...
        task.payload.get("authorization")
