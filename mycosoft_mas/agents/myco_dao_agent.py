...
class MycoDAOAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str, name: str, config: dict):
        ...
        self.dao_status = DAOStatus.ACTIVE
        self.proposals = {}
        self.members = {}
        self.tokens = {}
        self.voting_power = {}
    ...
    async def initialize(self) -> bool:
        ...
        # Avoid filesystem side-effects in __init__ (tests instantiate agents).
    ...
    async def submit_proposal(self, proposal_id: str) -> Dict:
        ...
        try:
            ...
            return {"success": True, "message": f"Proposal {proposal.title} submitted successfully"}
    ...
    async def start_voting(self, proposal_id: str, duration_days: int = 7) -> Dict:
        ...
        try:
            ...
            if proposal.status != ProposalStatus.SUBMITTED:
                return {
                    "success": False,
                    "message": f"Proposal is {proposal.status.value}, not submitted",
                }
    ...
    async def cast_vote(self, proposal_id: str, member_id: str, vote_type: str) -> Dict:
        ...
        try:
            ...
            if proposal.status != ProposalStatus.VOTING:
                return {
                    "success": False,
                    "message": f"Proposal is {proposal.status.value}, not in voting",
                }
            ...
            return {"success": True, "message": f"Vote cast successfully: {vote.value}"}
    ...
    async def end_voting(self, proposal_id: str) -> Dict:
        ...
        try:
            ...
            yes_power = sum(
                self.voting_power.get(member_id, 0)
    ...
    async def execute_proposal(self, proposal_id: str) -> Dict:
        ...
        try:
            ...
            if proposal.status != ProposalStatus.PASSED:
                return {
                    "success": False,
                    "message": f"Proposal is {proposal.status.value}, not passed",
                }
    ...
    async def _execute_proposal_action(self, proposal: Proposal) -> Dict:
        ...
        if proposal_type == "funding":
            ...
            return {"success": False, "message": f"Unknown proposal type: {proposal_type}"}
    ...
    async def add_member(self, member_data: Dict) -> Dict:
        ...
        try:
            ...
            return {"success": True, "member_id": member_id, "message": "Member added successfully"}
        ...
        while True:
            try:
                ...
                if proposal and proposal.status == ProposalStatus.VOTING:
                    ...
                    if datetime.now() > proposal.voting_end:
                        await self.end_voting(proposal_id)
    ...
    async def _validate_proposal(self, proposal: Proposal) -> Dict:
        ...
        if proposal_type == "funding":
            ...
            if amount > self.treasury:
                errors.append(f"Funding amount ({amount}) exceeds treasury ({self.treasury})")
    ...
    def _dict_to_proposal(self, data: Dict) -> Proposal:
        ...
        return Proposal(
            ...
            voting_start=(
                datetime.fromisoformat(data["voting_start"]) if data.get("voting_start") else None
            ),
            voting_end=(
                datetime.fromisoformat(data["voting_end"]) if data.get("voting_end") else None
            ),
        ...
        try:
            ...
            if error_type == "proposal_error":
                ...
                if proposal_id in self.proposals:
                    ...
                    if proposal.status == ProposalStatus.VOTING:
                        ...
                        async with aiofiles.open(proposal_file, "w") as f:
                            await f.write(json.dumps(self._proposal_to_dict(proposal), default=str))
                        self.logger.warning(
                            f"Proposal {proposal_id} reverted to draft due to error: {error_msg}"
                        )
                        return {
                            "success": True,
                            "message": f"Proposal {proposal_id} reverted to draft",
                            "action_taken": "revert_proposal",
                        }
                ...
                if proposal_id in self.proposals and member_id in self.members:
                    ...
                    if member_id in proposal.votes:
                        ...
                        async with aiofiles.open(proposal_file, "w") as f:
                            await f.write(json.dumps(self._proposal_to_dict(proposal), default=str))
                        self.logger.warning(
                            f"Vote removed for proposal {proposal_id} by member {member_id} due to error: {error_msg}"
                        )
                        return {
                            "success": True,
                            "message": "Vote removed",
                            "action_taken": "remove_vote",
                        }
                ...
                if proposal_id in self.proposals:
                    ...
                    if proposal.status == ProposalStatus.PASSED:
                        ...
                        return {
                            ...
                            "action_taken": "mark_execution_failed",
                        }
                ...
                if member_id in self.members:
                    ...
                    return {
                        "success": True,
                        "message": f"Member {member_id} suspended",
                        "action_taken": "suspend_member",
                    }
            ...
            # Handle health check errors
                ...
                self.dao_status = DAOStatus.MAINTENANCE
                # Notify members
                await self.notification_queue.put(
                    {
                ...
                return {
                    "success": True,
                    "message": "DAO put in maintenance mode",
                    "action_taken": "maintenance_mode",
                }
