...
class MycologyKnowledgeAgent(BaseAgent):
    ...
    async def export_knowledge(self, export_data: Dict) -> Dict:
        ...
        try:
            ...
            if export_type == "all":
                ...
                data = {"nodes": [self._node_to_dict(node) for node in self.nodes.values()]}
                ...
                if not node_ids:
                    return {
                        "success": False,
                        "message": "Node IDs are required for subgraph export",
                    }

                subgraph = self.knowledge_graph.subgraph(node_ids)
                data = {
                    "nodes": [
                        self._node_to_dict(self.nodes[node_id]) for node_id in subgraph.nodes()
                    ],
                    "relations": [
                        self._relation_to_dict(self.relations[rel_id])
                        for rel_id in self.relations
                        if self.relations[rel_id].source_id in node_ids
                        and self.relations[rel_id].target_id in node_ids
                    ],
                }
            ...
            if format_type == "json":
                ...
                if "relations" in data:
                    pd.DataFrame(data["relations"]).to_csv(
                        f"{output_path}_relations.csv", index=False
                    )
    ...
    async def _monitor_knowledge_updates(self):
        ...
        while True:
            try:
                ...
                if outdated:
                    await self.update_queue.put({"type": "outdated_knowledge", "items": outdated})
    ...
    async def _query_paths(self, query: Dict) -> List[Dict]:
        ...
        try:
            ...
            for path in nx.all_simple_paths(
                self.knowledge_graph, source_id, target_id, cutoff=max_length
            ):
                path_data = []
                for i in range(len(path) - 1):
                    ...
                    if (
                        relation_types
                        and RelationType[edge_data["type"].upper()] not in relation_types
                    ):
                        continue
    ...
    async def _semantic_search(
        self, query: str, node_type: Optional[str] = None, threshold: float = 0.5
    ) -> List[Dict]:
        """Perform semantic search over the knowledge graph."""
        try:
            if not self.semantic_model:
                self.logger.warning(
                    "Semantic search requested but sentence-transformers is not installed"
                )
                return []
        ...
        try:
            ...
            if node.type == KnowledgeType.SPECIES:
                if "scientific_name" not in node.metadata:
                    return {
                        "success": False,
                        "message": "Species nodes must have a scientific name",
                    }
                ...
                if "molecular_formula" not in node.metadata:
                    return {
                        "success": False,
                        "message": "Compound nodes must have a molecular formula",
                    }
        ...
        try:
            ...
            if relation.type == RelationType.IS_A:
                ...
                if source_node.type != target_node.type:
                    return {
                        "success": False,
                        "message": "IS_A relations must connect nodes of the same type",
                    }
    ...
    async def _add_relation_to_rdf(self, relation: KnowledgeRelation):
        ...
        try:
            ...
            # Add confidence
            self.rdf_graph.add(
                (relation_uri, self.myco_ns.confidence, Literal(relation.confidence))
            )
            ...
            # Add timestamps
            self.rdf_graph.add(
                (relation_uri, DCTERMS.created, Literal(relation.created_at.isoformat()))
            )
            self.rdf_graph.add(
                (relation_uri, DCTERMS.modified, Literal(relation.updated_at.isoformat()))
            )
    ...
    async def _update_relation_in_rdf(self, relation: KnowledgeRelation):
        ...
        try:
            URIRef(f"{self.myco_ns}{relation.source_id}")
            URIRef(f"{self.myco_ns}{relation.target_id}")
            relation_uri = URIRef(f"{self.myco_ns}{relation.id}")
    ...
    async def scrape_data_sources(self, sources: Optional[List[str]] = None) -> Dict:
        ...
        try:
            ...
            results = {"success": True, "sources": {}, "total_nodes": 0, "total_relations": 0}
            ...
            for source in sources:
                try:
                    ...
                    results["sources"][source] = {"success": False, "error": str(e)}
    ...
    async def _scrape_inaturalist(self) -> Dict:
        ...
        try:
            ...
            headers = {"User-Agent": "Mycosoft Knowledge Agent/1.0", "Accept": "application/json"}
                        ...
                        for observation in data["results"]:
                            ...
                            species_data = {
                                ...
                                "metadata": {
                                    ...
                                    "common_name": observation["taxon"].get(
                            ...
                            location_data = {
                                ...
                                "type": "FOUND_IN",
                                "metadata": {"observed_at": observation["observed_on"]},
                                "confidence": 1.0,
                                "sources": [observation["uri"]],
                            }
            ...
            return {"success": True, "nodes_added": nodes_added, "relations_added": relations_added}
                    ...
                    if response.status == 200:
                        ...
                        for genus in genera:
                            ...
                            genus_data = {
                                ...
                                "sources": [
                                    f"https://www.mycobank.org/page/Name_details/{genus['id']}"
                                ],
                            }
                            ...
                            async with session.get(
                                f"{api_url}/genera/{genus['id']}/species"
                            ) as species_response:
                                if species_response.status == 200:
                                    ...
                                    for sp in species:
                                        ...
                                        species_data = {
                                            ...
                                            "sources": [
                                                f"https://www.mycobank.org/page/Name_details/{sp['id']}"
                                            ],
                                        }
                    ...
                    if response.status == 200:
                        ...
                        for sp in species:
                            ...
                            species_data = {
                                ...
                                "sources": [
                                    f"https://fungidb.org/fungidb/app/record/Organism/{sp['id']}"
                                ],
                            }
                                ...
                                if genes_response.status == 200:
                                    ...
                                    for gene in genes:
                                        ...
                                        gene_data = {
                                            ...
                                            "sources": [
                                                f"https://fungidb.org/fungidb/app/record/Gene/{gene['id']}"
                                            ],
                                        }
    ...
    async def _scrape_atcc(self) -> Dict:
        ...
        try:
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                ...
                params = {"category": "fungi", "pageSize": 100, "page": 1}
    ...
    async def _scrape_researchhub(self) -> Dict:
        ...
        try:
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                ...
                params = {"q": "fungi OR mushroom OR mycology", "limit": 100, "offset": 0}
                ...
                async with session.get(f"{api_url}/search", params=params) as response:
                    if response.status == 200:
                        ...
                        for paper in data["results"]:
                            ...
                            species_mentions = await self._extract_species_mentions(
                ...
                async with session.get(f"{api_url}/esearch.fcgi", params=search_params) as response:
                    if response.status == 200:
                        ...
                        for pmid in search_data["esearchresult"]["idlist"]:
                            ...
                            async with session.get(
                                f"{api_url}/efetch.fcgi", params=fetch_params
                            ) as paper_response:
    ...
    async def _scrape_sciencedirect(self) -> Dict:
        ...
        try:
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                # Search for fungal research papers
                params = {"query": "fungi OR mushroom OR mycology", "count": 100, "start": 0}
        ...
        try:
            ...
            for ent in doc.ents:
                if ent.label_ in [
                    "ORG",
                    "PRODUCT",
                ]:  # Changed from SPECIES, TAXON to more general labels
    ...
    async def _evaluate_source(self, source: Dict) -> Dict:
        ...
        try:
            ...
            return {"viability_score": 0.0, "error": str(e)}
    ...
    async def _check_source_accessibility(self, source: Dict) -> Dict:
        ...
        try:
            ...
            return {"score": 0.0, "notes": [f"Error: {str(e)}"]}
    ...
    async def _check_mycology_relevance(self, source: Dict) -> Dict:
        ...
        try:
            ...
            keyword_count = sum(
                1
                for keyword in mycology_keywords
    ...
    async def approve_source(self, source_id: str) -> Dict:
        ...
        try:
            ...
            if not discovered_sources_file.exists():
                return {"success": False, "message": "No discovered sources found"}
            ...
            if not source:
                return {"success": False, "message": f"Source with ID {source_id} not found"}
    ...
    async def _check_data_freshness(self, source: Dict) -> Dict:
        ...
        # This is a placeholder - implement with actual checking logic
        return {"score": 0.5, "notes": ["Data freshness check not implemented"]}

    async def _check_data_completeness(self, source: Dict) -> Dict:
        """Check how complete the data in a source is."""
        # This is a placeholder - implement with actual checking logic
        return {"score": 0.5, "notes": ["Data completeness check not implemented"]}
        ...
        try:
            while True:
                ...
                if result["success"] and result["total_recommended"] > 0:
                    self.logger.info(
                        f"Discovered {result['total_recommended']} new fungal data sources"
                    )
                elif not result["success"]:
                    self.logger.error(
                        f"Error in periodic source discovery: {result.get('error', 'Unknown error')}"
                    )
        ...
        try:
            if error_type == "species_error":
                ...
                if species_id in self.species:
                    ...
                    self.logger.warning(
                        f"Species {species_id} marked as invalid: {error_data.get('message')}"
                    )
                ...
                if research_id in self.research:
                    ...
                    self.logger.warning(
                        f"Research {research_id} marked as invalid: {error_data.get('message')}"
                    )
                    return {
                        "success": True,
                        "action": "research_invalid",
                ...
                if taxonomy_id in self.taxonomy:
                    ...
                    self.logger.warning(
                        f"Taxonomy {taxonomy_id} marked as invalid: {error_data.get('message')}"
                    )
                    return {
                        "success": True,
                        "action": "taxonomy_invalid",
