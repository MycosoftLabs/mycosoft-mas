...
class FinancialOperationsAgent(BaseAgent):
            ...
            try:
                async with self.mercury_session.get(f"{base_url}/api/v1/accounts") as response:
                    if response.status == 200:
                        ...
                        self.bank_accounts = {
                            acc["id"]: acc for acc in accounts.get("accounts", [])
                        }
                        self.logger.info(
                            f"Mercury initialized successfully - {len(self.bank_accounts)} accounts found"
                        )
                        ...
                        self.logger.warning(
                            f"Mercury connection test returned status {response.status}"
                        )
    ...
    async def _initialize_quickbooks(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not all([client_id, client_secret, realm_id, refresh_token]):
                self.logger.warning(
                    "QuickBooks credentials not fully configured - QuickBooks integration disabled"
                )
                return None
            ...
            quickbooks_client["token_expires_at"] = (
            ...
            try:
                ...
                async with self.quickbooks_session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        self.logger.warning(
                            f"QuickBooks connection test returned status {response.status}"
                        )
            except Exception as e:
        ...
        try:
            ...
            async with client["session"].post(token_url, data=data, headers=headers) as response:
                if response.status == 200:
                    ...
                    self.logger.error(
                        f"Failed to refresh QuickBooks token: status {response.status}"
                    )
                    return None
    ...
    async def _ensure_quickbooks_token(self) -> bool:
        ...
        if (
            not self.quickbooks_client.get("access_token")
            or not self.quickbooks_client.get("token_expires_at")
            or datetime.now().timestamp() >= self.quickbooks_client["token_expires_at"] - 300
        ):
            ...
            try:
                async with self.pulley_session.get(f"{base_url}/v1/company") as response:
                    if response.status == 200:
                        ...
                        self.logger.warning(
                            f"Pulley connection test returned status {response.status}"
                        )
            except Exception as e:
        ...
        try:
            ...
            async with session.get(f"{base_url}/v1/stakeholders") as response:
                if response.status == 200:
                    ...
                    self.logger.info(
                        f"Fetched {len(self.cap_table['stakeholders'])} stakeholders from Pulley"
                    )
            ...
            async with session.get(f"{base_url}/v1/securities") as response:
                if response.status == 200:
                    ...
                    self.logger.info(
                        f"Fetched {len(self.cap_table['securities'])} securities from Pulley"
                    )
            ...
            async with session.get(f"{base_url}/v1/option_grants") as response:
                if response.status == 200:
                    ...
                    self.logger.info(
                        f"Fetched {len(self.cap_table['option_grants'])} option grants from Pulley"
                    )
            ...
            for account_id in self.bank_accounts.keys():
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        self.logger.info(
                            f"Loaded {len(transactions)} transactions for account {account_id}"
                        )
    ...
    async def get_mercury_account_balance(self, account_id: str) -> Optional[Dict[str, Any]]:
        ...
        if self.mercury_api_client and self.mercury_api_client.is_configured():
            try:
                ...
                self.logger.warning(
                    "Mercury API client balance lookup failed, falling back: %s", exc
                )
    ...
    async def get_mercury_transactions(
        ...
        if self.mercury_api_client and self.mercury_api_client.is_configured():
            try:
                ...
                self.logger.warning(
                    "Mercury API client transaction lookup failed, falling back: %s", exc
                )
        ...
        try:
            ...
            async with session.post(url, json=payload) as response:
                if response.status == 200 or response.status == 201:
                    ...
                    error_text = await response.text()
                    self.logger.error(
                        f"Mercury transaction failed: status {response.status}, {error_text}"
                    )
                    return None
        ...
        try:
            ...
            async with session.post(url, json=payload, headers=headers) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    self.logger.error(
                        f"QuickBooks transaction failed: status {response.status}, {error_text}"
                    )
                    return None
        ...
        try:
            ...
            async with session.post(url, json=payload) as response:
                if response.status == 200 or response.status == 201:
                    ...
                    error_text = await response.text()
                    self.logger.error(
                        f"Pulley SAFE creation failed: status {response.status}, {error_text}"
                    )
                    return True
    ...
    async def _generate_report(
        ...
        try:
            ...
            report_data = await self.get_quickbooks_financial_report(
                qb_report_type, period["start_date"], period["end_date"]
            )
