...
{{
    ...
    async def _generate_code_fix(
        self, analysis: Dict, workflow_file: Optional[str]
    ) -> Dict[str, Any]:
...
Focus on making the workflow resilient to the error."""
                    ...
                    response = await client.post(
                        ...
                        json={
                            ...
                            "messages": [
                                {
                                    "role": "system",
                                    "content": "You are an expert at debugging N8n workflows and fixing code issues.",
                                },
    ...
    def _classify_error_type(self, error_message: str) -> str:
        ...
        if any(
            x in error_lower for x in ["connection", "refused", "unreachable", "cannot connect"]
        ):
            return "connection"
    ...
    async def push_to_github(
        ...
        try:
            ...
            subprocess.run(
                ["git", "commit", "-m", commit_message], cwd=self.workspace_path, check=True
            )
            subprocess.run(
                ["git", "push", "-u", "origin", branch_name], cwd=self.workspace_path, check=True
            )
