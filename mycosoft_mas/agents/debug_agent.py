...
class DebugAgent:
    ...
    async def _check_mas_agents(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.mas_url}/agents/registry/", timeout=10.0)
                    ...
                    if len(inactive_agents) > 10:
                        self.improvements.append(
                            ImprovementSuggestion(
                                ...
                                affected_components=[
                                    a.get("name", "unknown") for a in inactive_agents[:5]
                                ],
        ...
        try:
            ...
            return WorkflowError(
                ...
                timestamp=datetime.fromisoformat(
                    execution.get("startedAt", datetime.now().isoformat())
                ),
    ...
    def _is_duplicate_error(self, error: WorkflowError) -> bool:
        ...
        for existing in self.errors[-10:]:
            if (
                existing.workflow_name == error.workflow_name
                and existing.node_name == error.node_name
                and existing.error_message == error.error_message
            ):
                return True
        return False
...
Provide a concise fix suggestion.""",
    ...
    async def _report_to_orchestrator(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                await client.post(
                    f"{self.mas_url}/tasks/submit",
                    json={"type": "debug_report", "priority": "medium", "data": report},
                    timeout=10.0,
                )
        except Exception as e:
    ...
    def _calculate_efficiency_metrics(self) -> Dict[str, Any]:
        ...
        recent_errors = [
            e for e in self.errors if e.timestamp > datetime.now() - timedelta(hours=1)
        ]

        return {
            "errors_per_hour": len(recent_errors),
            "most_problematic_workflow": self._get_most_problematic_workflow(),
            "auto_fixable_percentage": sum(1 for p in self.patterns.values() if p.auto_fixable)
