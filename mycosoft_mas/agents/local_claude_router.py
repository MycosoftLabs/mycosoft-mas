...
class LocalClaudeRouter:
    ...
    def register_local_instance(
        self, endpoint: str, capabilities: List[str], metadata: Dict[str, Any]
    ):
    ...
    async def route_task(
        ...
        if not available:
            return {"status": "error", "message": "No Claude Code instances available"}
    ...
    async def _execute_on_instance(
        self, instance: Dict[str, Any], task: str, repo: str
    ) -> Dict[str, Any]:
    ...
    async def _execute_local(self, endpoint: str, task: str, repo: str) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                f"{endpoint}/task",
                json={"description": task, "repo": repo, "priority": 5},
                timeout=10,
            )
            ...
            return {"status": "error", "message": f"Connection failed: {endpoint}"}

    async def _poll_task_completion(
        self, endpoint: str, task_id: int, timeout: int = 600
    ) -> Dict[str, Any]:
    ...
    async def _execute_parallel(
        self, task: str, repo: str, instances: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Execute task on multiple instances in parallel."""
        logger.info(f"Executing task on {len(instances)} instances in parallel")

        # Create tasks
        tasks = [self._execute_on_instance(instance, task, repo) for instance in instances]
