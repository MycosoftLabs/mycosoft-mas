...
class SecretaryAgent(BaseAgent):
    ...
    async def process_email(self, email_data: Dict) -> Dict:
        ...
        try:
            ...
            response = await self._generate_email_response(response_type, sender, subject, content)
    ...
    async def _find_available_time_slots(
        self, attendees: List[str], duration: int, preferred_time_range: Optional[Dict] = None
    ) -> List[Dict]:
    ...
    async def _generate_email_response(
        self, response_type: str, sender: str, subject: str, content: str
    ) -> str:
        ...
        try:
            ...
            if error_type == "meeting_error":
                ...
                if meeting_id:
                    try:
                        ...
                        self.logger.warning(
                            f"Meeting {meeting_id} cancelled due to error: {error_msg}"
                        )
                        return {
                            "success": True,
                            "message": "Meeting cancelled and attendees notified",
                            "action_taken": "cancel_meeting",
                        }
                    except Exception as e:
                ...
                if email_id and sender:
                    try:
                        ...
                        return {
                            "success": True,
                            "message": "Sender notified and email queued for review",
                            "action_taken": "queue_for_review",
                        }
                    except Exception as e:
                ...
                if doc_type:
                    try:
                        ...
                        draft_path = (
                            f"drafts/{doc_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
                        )
                        ...
                        self.logger.warning(
                            f"Document creation failed for type {doc_type}: {error_msg}"
                        )
                        return {
                            "success": True,
                            "message": "Document draft saved for review",
                            "action_taken": "save_draft",
                        }
                    except Exception as e:
                ...
                if service_name:
                    try:
                        ...
                        await self._init_google_services()
                        self.logger.warning(
                            f"Reinitialized Google services after error: {error_msg}"
                        )
                        return {
                            "success": True,
                            "message": "Services reinitialized",
                            "action_taken": "reinit_services",
                        }
                    except Exception as e:
            ...
            # Handle health check errors
                ...
                try:
                    ...
                    return {
                        "success": True,
                        "message": "Reconnected to Google services",
                        "action_taken": "reconnect_services",
                    }
                except Exception as e:
                    self.logger.error(f"Failed to reconnect to Google services: {str(e)}")
                    return {
                        "success": False,
                        "message": "Failed to reconnect to services",
                        "action_taken": "none",
                    }
