...
class MycoBrainProtocolExtension:
    ...
    async def subscribe_device_to_protocol(self, device_id: str, protocol_id: str) -> bool:
        """
    ...
    async def unsubscribe_device_from_protocol(self, device_id: str, protocol_id: str) -> bool:
    ...
    async def handle_mycobrain_telemetry(self, device_id: str, telemetry: MDPTelemetry) -> None:
        """
    ...
    async def _notify_protocol_telemetry(
        self, protocol_id: str, device_id: str, telemetry: MDPTelemetry
    ) -> None:
    ...
    def _get_protocol_step(self, protocol_id: str, step_id: str) -> Optional[ProtocolStep]:
    ...
    async def _check_step_conditions(
        ...
        if step.humidity is not None and telemetry.humidity is not None:
            ...
            if humidity_diff > 5.0:  # 5% tolerance
                self.logger.warning(
                    f"Humidity mismatch: expected {step.humidity}%, " f"got {telemetry.humidity}%"
                )
    ...
    async def send_protocol_command(
        ...
        self.logger.info(
            f"Sending command {command_type} to device {device_id} " f"(protocol: {protocol_id})"
        )
    ...
    def map_telemetry_to_protocol_fields(self, telemetry: MDPTelemetry) -> Dict[str, Any]:
        """
