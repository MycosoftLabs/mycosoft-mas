...
class DataFlowCoordinatorAgent(BaseAgent):
    ...
    async def register_transform_function(
        self, name: str, function: Callable[[Dict[str, Any]], Awaitable[Dict[str, Any]]]
    ) -> None:
        """Register a transform function"""
        self.transform_functions[name] = function

    async def register_validation_function(
        self, name: str, function: Callable[[Dict[str, Any]], Awaitable[bool]]
    ) -> None:
    ...
    async def get_events(
        self, flow_id: Optional[str] = None, item_id: Optional[str] = None
    ) -> List[DataFlowEvent]:
    ...
    async def _process_data_item(self, item_id: str) -> None:
        ...
        for flow in applicable_flows:
            try:
                ...
                await self._record_event(flow.flow_id, item.item_id, "flow_start", "processing")
                ...
                is_valid = True
                if (
                    flow.validation_function
                    and flow.validation_function in self.validation_functions
                ):
                ...
                await self._record_event(flow.flow_id, item.item_id, "flow_complete", "success")

                self.metrics["items_processed"] += 1

            except Exception as e:
                self.logger.error(
                    f"Error processing flow {flow.flow_id} for item {item_id}: {str(e)}"
                )
                await self._record_event(
                    flow.flow_id, item.item_id, "flow_error", "error", {"error": str(e)}
                )
