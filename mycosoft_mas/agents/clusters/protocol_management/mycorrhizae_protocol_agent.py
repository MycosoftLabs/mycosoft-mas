...
class MycorrhizaeProtocolAgent(BaseAgent):
    ...
    async def complete_step(self, execution_id: str, step_id: str, results: Dict[str, Any]) -> bool:
        ...
        current_step_index = next(
            (i for i, step in enumerate(protocol.steps) if step.step_id == step_id), -1
        )
    ...
    async def get_optimizations(
        ...
        if protocol_id:
            return [opt for opt in self.optimizations.values() if opt.protocol_id == protocol_id]
    ...
    async def _check_optimization_opportunities(self, execution: ProtocolExecution) -> None:
