...
class DeviceCoordinatorAgent(BaseAgent):
    ...
    async def connect_device(self, device_id: str) -> bool:
        ...
        try:
            ...
            await self.connection_queue.put({"device_id": device_id, "action": "connect"})
    ...
    async def disconnect_device(self, device_id: str) -> bool:
        ...
        try:
            ...
            # Add to connection queue
            await self.connection_queue.put({"device_id": device_id, "action": "disconnect"})
    ...
    async def send_command(self, device_id: str, command: DeviceCommand) -> bool:
        ...
        try:
            ...
            await self.command_queue.put({"device_id": device_id, "command": command})
        ...
        try:
            ...
            if start_time or end_time:
                data = [
                    point
                    for point in data
                    if (not start_time or point.timestamp >= start_time)
                    and (not end_time or point.timestamp <= end_time)
                ]
