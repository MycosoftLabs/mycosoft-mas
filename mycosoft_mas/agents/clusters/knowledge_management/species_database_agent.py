...
class SpeciesDatabaseAgent(BaseAgent):
    ...
    async def get_species_relationships(self, species_id: str) -> List[SpeciesRelationship]:
        ...
        return [
            rel
            for rel in self.species_relationships.values()
            if rel.source_id == species_id or rel.target_id == species_id
        ]

    async def search_species(
        self, query: str, rank: Optional[TaxonomicRank] = None, limit: Optional[int] = None
    ) -> List[TaxonomicEntry]:
        ...
        for entry in self.taxonomic_entries.values():
            ...
            if query.lower() in entry.scientific_name.lower() or any(
                query.lower() in name.lower() for name in entry.common_names
            ):
