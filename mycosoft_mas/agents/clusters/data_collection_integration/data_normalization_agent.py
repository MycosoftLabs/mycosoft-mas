...
class DataNormalizationAgent(BaseAgent):
    ...
    async def register_schema(
        self, name: str, version: str, format: DataFormat, schema: Dict[str, Any]
    ) -> str:
        """Register a new data schema"""
        schema_id = f"schema_{len(self.schemas)}"

        data_schema = DataSchema(
            schema_id=schema_id, name=name, version=version, format=format, schema=schema
        )
        ...
        while self.status == AgentStatus.RUNNING:
            try:
                ...
                if result.status == NormalizationStatus.COMPLETED:
                    ...
                    self.metrics["data_points_normalized"] += self._count_data_points(
        ...
        try:
            ...
            if task.custom_transformation:
                normalized_data = self._apply_custom_transformation(
                    task.data, task.custom_transformation
                )
            else:
                normalized_data = self._apply_standard_transformation(
                    task.data, task.source_format, task.target_format
                )
