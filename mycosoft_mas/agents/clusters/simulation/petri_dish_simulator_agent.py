...
class PetriDishSimulatorAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str, name: str, config: dict):
        ...
        self.petridishsim_url = config.get("petridishsim_url") or os.environ.get(
            "PETRIDISHSIM_URL", ""
        )
            ...
            asyncio.create_task(
                notify_nlm_petri_outcome(
                    ...
                    summary={
                        ...
                        "biomass_final": result.biomass_values[-1] if result.biomass_values else 0,
                    },
                    metrics={
                        "time_points": len(result.time_points),
                        "phases": len(result.growth_phases),
                    },
                )
            )
    ...
    def _dict_to_growth_parameters(self, d: Dict[str, Any]) -> GrowthParameters:
        ...
        opt_cond = (
            self._dict_to_env_conditions(opt)
    ...
    def _dict_to_simulation_result(self, d: Dict[str, Any]) -> Optional[SimulationResult]:
        ...
        try:
            ...
            params = (
                self._dict_to_growth_parameters(params_d) if isinstance(params_d, dict) else None
            )
            if params is None:
    ...
    async def _run_growth_simulation(self, parameters: GrowthParameters) -> SimulationResult:
        ...
        for step in range(max(1, min(parameters.time_steps, 120))):
            chemical_config = (
                parameters.metadata.get("chemical_simulation") if parameters.metadata else None
            )
            if chemical_config:
            ...
            biomass += (
                ...
                * growth_factor
                * (parameters.duration / max(parameters.time_steps, 1))
            )
            ...
            phase = (
                ...
                if biomass > parameters.initial_biomass * 1.2
                else GrowthPhase.LAG
            )
    ...
    async def _process_environmental_conditions(
        self, data: EnvironmentalConditions
    ) -> Dict[str, Any]:
