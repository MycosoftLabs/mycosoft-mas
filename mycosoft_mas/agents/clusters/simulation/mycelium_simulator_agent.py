...
class MyceliumSimulatorAgent(BaseAgent):
    ...
    async def update_simulation(self, simulation_id: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await self.data_queue.put({"simulation_id": simulation_id, "action": "update"})
    ...
    async def export_simulation(
        self, simulation_id: str, formats: List[DataFormat]
    ) -> SimulationResult:
        """
        ...
        try:
            ...
            await self.export_queue.put(
                {"result_id": result_id, "simulation_id": simulation_id, "formats": formats}
            )
        ...
        while self.is_running:
            try:
                ...
                if simulation_id in self.simulations:
                    ...
                    self.logger.info(
                        f"Exported simulation {simulation_id} to formats: {[f.value for f in formats]}"
                    )
    ...
    async def _process_screenshot(self, simulation: SimulationData, format: DataFormat) -> str:
        ...
        screenshot_path = str(
            self.screenshot_dir
            / f"{simulation.id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{extension}"
        )
