...
class EnvironmentalDataAgent(BaseAgent):
    ...
    async def start_collection(
        self, dataset_id: str, device_id: str, factors: List[EnvironmentalFactor], interval: int
    ) -> bool:
        """
    ...
    async def stop_collection(self, dataset_id: str) -> bool:
        ...
        try:
            ...
            # Add to collection queue
            await self.collection_queue.put({"dataset_id": dataset_id, "action": "stop"})
    ...
    async def add_reading(self, dataset_id: str, reading: EnvironmentalReading) -> bool:
        ...
        try:
            ...
            await self.processing_queue.put({"dataset_id": dataset_id, "reading": reading})
    ...
    async def analyze_dataset(
        self, dataset_id: str, factors: Optional[List[EnvironmentalFactor]] = None
    ) -> bool:
        """
        ...
        try:
            ...
            await self.analysis_queue.put({"dataset_id": dataset_id, "factors": factors})
    ...
    async def get_analysis(
        self, dataset_id: str, factor: Optional[EnvironmentalFactor] = None
    ) -> List[EnvironmentalAnalysis]:
        """
    ...
    def _detect_anomalies(
        self, values: List[float], mean: float, std: float, threshold: float = 3.0
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for i, z_score in enumerate(z_scores):
                if z_score > threshold:
                    anomalies.append({"index": i, "value": values[i], "z_score": z_score})
