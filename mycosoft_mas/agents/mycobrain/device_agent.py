...
class MycoBrainDeviceAgent(BaseAgent):
    ...
    async def _connect_uart(self) -> None:
        ...
        try:
            self.serial_reader, self.serial_writer = await serial_asyncio.open_serial_connection(
                url=self.uart_port, baudrate=self.uart_baudrate
            )
    ...
    async def _handle_event(self, frame: Any, event: MDPEvent) -> None:
        ...
        await self._emit_event(
            "device_event",
            {"device_id": self.device_id, "event": event.to_dict(), "sequence": frame.sequence},
        )

    async def send_command(
        self, command_type: str, parameters: Dict[str, Any], timeout: Optional[float] = None
    ) -> bool:
        """
        ...
        """
        command = MDPCommand(
            command_id=len(self.pending_commands), command_type=command_type, parameters=parameters
        )
        ...
        while self.is_running:
            try:
                ...
                await self.send_command(
                    command["command_type"], command["parameters"], command.get("timeout")
                )
            except asyncio.TimeoutError:
                continue
            except Exception as e:
            ...
            try:
                ...
                if self.last_telemetry_time:
                    ...
                    if time_since_telemetry > 120:  # No telemetry for 2 minutes
                        self.logger.warning(
                            f"No telemetry received for {time_since_telemetry:.0f} seconds"
                        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_telemetry_time": (
                self.last_telemetry_time.isoformat() if self.last_telemetry_time else None
            ),
