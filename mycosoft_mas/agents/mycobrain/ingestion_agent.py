...
class MycoBrainIngestionAgent(BaseAgent):
    ...
    async def ingest_telemetry(
        self, device_id: str, telemetry: MDPTelemetry, sequence: int, timestamp: datetime
    ) -> bool:
        """
        ...
        while self.is_running:
            try:
                ...
                telemetry_data = await asyncio.wait_for(
                    self.telemetry_queue.get(), timeout=self.batch_timeout
                )
    ...
    async def _ingest_single_telemetry(self, telemetry_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = await self.integration_manager.mindex.ingest_mycobrain_telemetry(
                device_id=device_id, telemetry_data=mindex_data, api_key=api_key
            )
    ...
    def _map_telemetry_to_mindex(
        ...
        # I²C addresses
        mindex_record["i2c_sensors"] = {"detected_addresses": telemetry.i2c_addresses}
