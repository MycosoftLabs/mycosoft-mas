...
from mycosoft_mas.agents.messaging.message_types import MessageStatus
from mycosoft_mas.agents.enums.task_status import TaskStatus
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("agent_manager.log"), logging.StreamHandler()],
)
...
class AgentManager:
    ...
    def _load_configurations(self):
        ...
        try:
            ...
            department_configs_path = os.getenv(
                "DEPARTMENT_CONFIGS_PATH", "config/department_configs.json"
            )
            ...
            application_configs_path = os.getenv(
                "APPLICATION_CONFIGS_PATH", "config/application_configs.json"
            )
                ...
                for agent_id, agent in self.agents.items():
                    if agent.get("status") == AgentStatus.ACTIVE.value:
                        ...
                        if not await self._check_agent_health(agent_id):
                            ...
                            self._log_error(
                                f"Agent {agent_id} health check failed", "Agent not responding"
                            )
            ...
            try:
                for agent_id, heuristics in self.auto_heuristics.items():
                    if (
                        ...
                        and self.agents[agent_id].get("status") == AgentStatus.ACTIVE.value
                    ):
                        # Apply heuristics
    ...
    async def _apply_error_correction(self, agent_id: str):
        ...
        try:
            ...
            # Log the error
            self._log_error(
                f"Agent {agent_id} error correction", "Applying error correction protocol"
            )
        ...
        try:
            ...
            if "schedule" in recipient.get("presence", {}):
                ...
                if current_day in schedule:
                    ...
                    for time_range in day_schedule:
                        start_hour = time_range.get("start", 0)
                        time_range.get("end", 24)
                        activity = time_range.get("activity", "unknown")

                        if (
                            activity not in ["sleeping", "meeting", "focus"]
                            and start_hour > current_hour
                        ):
    ...
    async def create_agent(self, agent_type: str, config: Dict[str, Any]) -> str:
        ...
        try:
            ...
            self.auto_heuristics[agent_id] = {
                ...
                "resource_management": {"memory_limit": "1GB", "cpu_limit": "2", "gpu_limit": "0"},
    ...
    async def get_agents(
        self, agent_type: Optional[str] = None, status: Optional[AgentStatus] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def add_llm_provider(
        self, provider: Union[LLMProvider, str], config: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            if error_type == "agent_error":
                ...
                if agent_id in self.agents:
                    ...
                    self.logger.warning(
                        f"Agent {agent_id} marked as error: {error_data.get('message')}"
                    )
                ...
                if message_id in self.messages:
                    ...
                    self.logger.warning(
                        f"Message {message_id} marked as failed: {error_data.get('message')}"
                    )
                ...
                if task_id in self.tasks:
                    ...
                    self.logger.warning(
                        f"Task {task_id} marked as failed: {error_data.get('message')}"
                    )
