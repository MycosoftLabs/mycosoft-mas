...
try:
    ...
    from rdflib.namespace import DCTERMS, OWL, RDFS, SKOS, XSD  # type: ignore
except ImportError:  # pragma: no cover - optional dependency
    Graph = Literal = RDF = URIRef = Namespace = None  # type: ignore
    RDFS = XSD = OWL = DCTERMS = SKOS = None  # type: ignore
...
class MycologyBioAgent(BaseAgent):
    ...
    async def _connect_to_agents(self):
        ...
        try:
            ...
            self.dao_agent = MycoDAOAgent(
                self.dao_agent_id, "MycoDAO Agent", self.config.get("dao_agent_config", {})
            )
    ...
    async def import_bio_data(self, import_data: Dict) -> Dict:
        ...
        try:
            ...
            # Add to import queue
            await self.import_queue.put({"import_id": import_id, "import_data": import_data})
            ...
            return {"success": True, "import_id": import_id, "message": "Bio data import started"}
    ...
    async def export_bio_data(self, export_data: Dict) -> Dict:
        ...
        try:
            ...
            # Add to export queue
            await self.export_queue.put({"export_id": export_id, "export_data": export_data})
            ...
            return {"success": True, "export_id": export_id, "message": "Bio data export started"}
        ...
        while True:
            try:
                ...
                if format == "fasta" and destination == "file":
                    ...
                    self.logger.warning(
                        f"Unknown export format/destination: {format}/{destination}"
                    )
        ...
        try:
            if error_type == "culture_error":
                ...
                if culture_id in self.cultures:
                    ...
                    self.logger.warning(
                        f"Culture {culture_id} marked as contaminated: {error_data.get('message')}"
                    )
                    return {
                        "success": True,
                        "action": "culture_contaminated",
                ...
                if experiment_id in self.experiments:
                    ...
                    self.logger.warning(
                        f"Experiment {experiment_id} marked as failed: {error_data.get('message')}"
                    )
                    return {
                        "success": True,
                ...
                if sample_id in self.samples:
                    ...
                    self.logger.warning(
                        f"Sample {sample_id} marked as invalid: {error_data.get('message')}"
                    )
