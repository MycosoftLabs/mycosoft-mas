        ...
        try:
            ...
            if error_type == "campaign_error":
                ...
                if content_id in self.content_library:
                    ...
                    self.logger.warning(
                        f"Reverted content {content_id} to draft due to error: {error_msg}"
                    )
                    return {
                        "success": True,
                        "message": f"Content {content_id} reverted to draft",
                        "action_taken": "revert_content",
                    }
                ...
                self.logger.warning(
                    f"Social media error for post {post_id} on {platform}: {error_msg}"
                )
                # Attempt to reschedule the post
                if "retry_time" in error_data:
                    return {
                        "success": True,
                        "message": f"Post {post_id} rescheduled",
                        "action_taken": "reschedule_post",
                    }
                ...
                if metric_type in self.analytics_data:
                    return {
                        "success": True,
                        "message": "Using cached analytics data",
            ...
            # Handle health check errors
                ...
                if await self._reconnect_service(service):
                    return {
                        ...
                        "action_taken": "service_reconnect",
                    }
                return {
                    "success": False,
                    "message": f"Failed to reconnect to {service}",
                    "action_taken": "none",
                }
