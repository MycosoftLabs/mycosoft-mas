...
class ExperimentAgent(BaseAgent):
    ...
    async def create_experiment(self, experiment_data: Dict) -> Dict:
        ...
        try:
            experiment_id = experiment_data.get(
            ...
            experiment = Experiment(
                ...
                end_date=(
                    ...
                    if "end_date" in experiment_data
                    else None
                ),
    ...
    async def start_experiment(self, experiment_id: str) -> Dict:
        ...
        try:
            ...
            if experiment.status != ExperimentStatus.PLANNED:
                return {
                    "success": False,
                    "message": f"Experiment is already {experiment.status.value}",
                }
    ...
    async def record_data_point(self, experiment_id: str, data_point: Dict) -> Dict:
        ...
        try:
            ...
            if experiment.status != ExperimentStatus.IN_PROGRESS:
                return {
                    "success": False,
                    "message": f"Cannot record data for experiment in {experiment.status.value} status",
                }
    ...
    async def analyze_experiment_data(
        self, experiment_id: str, analysis_type: str = "comprehensive"
    ) -> Dict:
        ...
        try:
            ...
            analysis_file = (
                analysis_dir / f"{analysis_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    async def complete_experiment(self, experiment_id: str, conclusions: str) -> Dict:
        ...
        try:
            ...
            if experiment.status != ExperimentStatus.IN_PROGRESS:
                return {
                    "success": False,
                    "message": f"Cannot complete experiment in {experiment.status.value} status",
                }
    ...
    async def _process_data_queue(self):
        ...
        while True:
            ...
            try:
                await self._process_data_point(
                    data_item["experiment_id"], data_item["data_point_id"], data_item["data_type"]
                )
            except Exception as e:
    ...
    async def _handle_error_type(self, error_type: str, error: Dict) -> Dict:
        ...
        try:
            ...
            if error_type == "experiment_error":
                ...
                if experiment_id in self.experiments:
                    ...
                    if experiment.status == ExperimentStatus.IN_PROGRESS:
                        ...
                        self.logger.warning(
                            f"Experiment {experiment_id} paused due to error: {error_msg}"
                        )
                        return {
                            "success": True,
                            "message": f"Experiment {experiment_id} paused",
                            "action_taken": "pause_experiment",
                        }
                ...
                if experiment_id in self.experiments:
                    ...
                    self.logger.warning(
                        f"Data collection error for experiment {experiment_id}, point {data_point_id}: {error_msg}"
                    )
                    return {
                        "success": True,
                        "message": "Data collection retry scheduled",
                        "action_taken": "retry_collection",
                    }
                ...
                if equipment_id in self.equipment:
                    ...
                    self.logger.warning(
                        f"Equipment {equipment_id} marked as malfunctioning: {error_msg}"
                    )
                    return {
                        "success": True,
                        "message": f"Equipment {equipment_id} marked as malfunctioning",
                        "action_taken": "mark_equipment_error",
                    }
                ...
                if experiment_id in self.experiments:
                    ...
                    self.logger.warning(
                        f"Analysis error for experiment {experiment_id}, type {analysis_type}: {error_msg}"
                    )
                    return {
                        "success": True,
                        "message": "Fallback analysis scheduled",
                        "action_taken": "use_fallback_analysis",
                    }
            ...
            # Handle health check errors
                ...
                return {
                    "success": True,
                    "message": "Experiment data backup initiated",
                    "action_taken": "backup_data",
                }
