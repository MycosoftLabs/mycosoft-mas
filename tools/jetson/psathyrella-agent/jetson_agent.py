...
HARDWARE (per PSATHYRELLA_PROCUREMENT_BRIEFING_JUL01_2026 / HARDWARE_WIRE_PLAN):
  - TD1.2 thrusters + Diamond Dynamics WP ESCs — 50 Hz PWM, 1000–2000 µs, NO BEC.
    DD arming: power the ESC with the signal held at **1500 µs** (beeps) → ready.
    Disarmed/boot uses ESC_ARMING_US (default 1600 — Aug 17 2026 bench: all four ESCs stop at 1600 µs;
    1500 is ~100 µs into reverse). Armed zero-throttle uses ESC_NEUTRAL_US (same 1600).
...
SAFETY:
  - Arm gate: ESCs held at ESC_ARMING_US while disarmed; armed zero uses ESC_NEUTRAL_US.
...
Env: JETSON_AGENT_PORT|PORT(8788) MOCK(0) PWM_FREQ(50) ESC_CH(8,9,10,11) SERVO_CH(4,5,6,7)
     PCA9685_I2C_ADDRESS(0x60) ESC_ARMING_US(1600) ESC_NEUTRAL_US(1600)
     SERVO_STOP_US_BY_CHANNEL(4:1600,...) ESC_ARM_HOLD_S(8)
...
# Aug 17 2026: measured stop is 1600 µs. 1500 spins reverse. Park/disarm at 1600.
ESC_ARMING_US = int(os.getenv("ESC_ARMING_US", "1600"))
ESC_ARM_HOLD_S = float(os.getenv("ESC_ARM_HOLD_S", "8"))
...
def esc_arming_us_for_channel(channel: int) -> float:
    # Optional per-channel override via ESC_ARMING_US_CH{n}; else global ESC_ARMING_US.
    env_value = os.getenv(f"ESC_ARMING_US_CH{channel}")
    if env_value:
        try:
            return float(clamp(float(env_value), 1000, 2000))
        except ValueError:
            pass
    return float(clamp(ESC_ARMING_US, 1000, 2000))


def esc_arming_us_for_thruster(tid: int) -> float:
    if 0 <= tid < len(ESC_CH):
        return esc_arming_us_for_channel(ESC_CH[tid])
    return float(clamp(ESC_ARMING_US, 1000, 2000))
...
class Propulsion:
    ...
    def _apply_esc(self, t: Thruster) -> None:
        if t.id >= len(ESC_CH):
            return
        # Disarmed / faulted: hold measured stop (ESC_ARMING_US, default 1600).
        if (not self.armed) or t.faulted:
            self.pwm.set_us(ESC_CH[t.id], esc_arming_us_for_thruster(t.id))
            return
        self.pwm.set_us(ESC_CH[t.id], self._esc_us(t.throttle_pct, tid=t.id))
    ...
    def _rotate_worker(self, t: Thruster, target: float, cancel: threading.Event) -> None:
        # NO-SLIP-RING WIND LIMIT: az_actual lives UNWRAPPED in [-180, +180] relative to home
        # (home = wires relaxed, set by nav.az_zero). Pick the target representation inside the
        # limit and move DIRECTLY to it - taking the long way back through home when needed -
        # so the pod can never rotate past +/-180 and twist the harness.
        tn = ((float(target) + 180.0) % 360.0) - 180.0
        cands = [c for c in (tn, tn - 360.0, tn + 360.0) if -180.0 <= c <= 180.0]
        tgt = min(cands, key=lambda c: abs(c - t.az_actual))
        delta = tgt - t.az_actual
        if abs(delta) < 2:
            self._servo_stop(t)
            t.az_actual = tgt
            return
        ...
        t.az_actual = t.az_actual + direction * min(1.0, elapsed / duration) * abs(delta)  # unwrapped, stays in [-180,180]
    ...
    def set_armed(self, armed: bool) -> None:
        with self._lock:
            ...
            # Disarm -> park pulse (ESC_ARMING_US=1600). Arm -> run neutral at 0% throttle.
    ...
    def play_esc_arming_tones(
        ...
        *,
        ...
        """
        Software re-arm helper for Diamond Dynamics WP ESCs.
        Forces ESC_ARMING_US (~1500) on each ESC channel for hold_s seconds while disarmed.
        Operator should have ESC pack powered (or power-cycle pack during the hold) to hear beeps.
        """
        if self.armed:
            return {"ok": False, "error": "disarm_before_esc_arm_tone"}
        hold = max(1.0, float(hold_s if hold_s is not None else ESC_ARM_HOLD_S))
        ids = thruster_ids if thruster_ids is not None else list(range(len(self.thrusters)))
        actions: List[Dict[str, Any]] = []
        with self._lock:
            for tid in ids:
                if not (0 <= tid < len(self.thrusters)) or tid >= len(ESC_CH):
                    continue
                ch = ESC_CH[tid]
                pulse = esc_arming_us_for_thruster(tid)
                actions.append({"thruster_id": tid, "channel": ch, "pulse_us": round(pulse, 1), "hold_s": hold})
                if not dry_run:
                    self.pwm.set_us(ch, pulse)
            if not dry_run:
                # Keep holding so all four ESCs see arming pulse together.
                time.sleep(hold)
                self.neutral_all()
        return {
            "ok": True,
            "dry_run": dry_run,
            "hold_s": hold,
            "detail": "esc_arming_tones",
            "esc_arming_us": ESC_ARMING_US,
            "actions": actions,
        }
    ...
    def run_selftest(self, *, dry_run: bool, thruster_id: int, apply_neutral: bool) -> Dict[str, Any]:
        ...
        if apply_neutral:
            if esc_channel is not None:
                # Prefer DD arming pulse so selftest doesn't leave ESCs stuck at run-trim 1600.
                pulse = esc_arming_us_for_thruster(thruster_id) if not self.armed else esc_neutral_us_for_thruster(thruster_id)
                actions.append({"kind": "esc_arming_or_neutral", "channel": esc_channel, "pulse_us": pulse, "armed": self.armed})
                if not dry_run:
                    self.pwm.set_us(esc_channel, pulse)
...
async def command(body: MdpCommand, request: Request) -> Dict[str, Any]:  # noqa: ARG001
    ...
    try:
        ...
        if cmd == "nav.esc_arm_tone":
            tid_raw = p.get("id")
            ids = [int(tid_raw)] if tid_raw is not None else None
            result = PROP.play_esc_arming_tones(
                dry_run=bool(p.get("dry_run", False)),
                hold_s=p.get("hold_s"),
                thruster_ids=ids,
            )
            detail = result.get("detail") or result.get("error") or "esc_arm_tone"
            PROP.note_command(cmd, p, detail)
            if not result.get("ok"):
                return _feedback(cmd, ok=False, detail=str(result.get("error") or detail))
            return {**_feedback(cmd, detail=detail), "arming": result}
        ...
        if cmd == "nav.thruster_group":
            # ATOMIC multi-thruster apply: all ESC throttle writes happen in ONE lock pass
            # (tight I2C burst - effectively simultaneous), then azimuth rotations start.
            # No per-command network stagger = synced actuation, like nav.thrust_vector.
            # items: [{id, throttle?, azimuth?}, ...]  OR  {throttle: v} = same throttle to ALL.
            items = p.get("items")
            if not isinstance(items, list):
                thr = p.get("throttle")
                items = [{"id": t.id, "throttle": thr} for t in PROP.thrusters] if thr is not None else []
            applied = 0
            with PROP._lock:
                for it in items:
                    tid = int(it.get("id", 0))
                    ...
                    if it.get("throttle") is not None:
                        t.throttle_pct = clamp(float(it["throttle"]), -100, 100)
                        PROP._apply_esc(t)  # arm-gated inside, same as thrust_vector
                    if it.get("azimuth") is not None:
                        PROP.set_azimuth(t, float(it["azimuth"]))
                    applied += 1
            PROP.note_command(cmd, p, f"group x{applied}")
            return _feedback(cmd, detail=f"group applied x{applied}")
...
def _wifi_stats():
    """Live radio stats from /proc/net/wireless (no root): link quality + signal dBm."""
    try:
        with open("/proc/net/wireless") as f:
            for ln in f:
                if "wlP1p1s0" in ln:
                    parts = ln.split()
                    return {"iface": "wlP1p1s0", "link_quality": float(parts[2].rstrip(".")),
