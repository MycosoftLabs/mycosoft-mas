/*
  ...
  Indicator lights:
  - Uses SK6805 NeoPixel on GPIO15 (NEO_1)
  ...
  MODIFIED: Fixed LED to use NeoPixelBus on GPIO15 (SK6805) instead of PWM
*/
...
// AO pins are MOSFET outputs (NOT LEDs!) - for external 3.3V switching
static const int AO_PINS[3]  = {12, 13, 14}; // OUT_1, OUT_2, OUT_3

// NeoPixel (SK6805) on GPIO15 - single addressable RGB LED
#define NEOPIXEL_PIN 15
#define PIXEL_COUNT  1

static const int BUZZER_PIN  = 16;

// FastLED array for SK6805 (GRB order like WS2812)
...
void setup() {
  ...
  // Setup AO pins as MOSFET outputs (not LEDs)
  for (int i = 0; i < 3; i++) {
    pinMode(AO_PINS[i], OUTPUT);
    digitalWrite(AO_PINS[i], LOW);
  }

  // Initialize NeoPixel (SK6805 on GPIO15)
