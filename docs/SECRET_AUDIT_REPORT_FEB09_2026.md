...
| 83 | `MINDEX_DATABASE_URL` | Real PostgreSQL credentials (`mycosoft:REDACTED_DB_PASSWORD@192.168.0.189`) |
...
**Line:** 12
**Content:** `PASSWORD = "REDACTED_VM_SSH_PASSWORD"`
...
**Line:** 12
**Content:** `c.connect("192.168.0.189", username="mycosoft", password="REDACTED_VM_SSH_PASSWORD", ...)`
...
**Line:** 182
**Content:** `"postgresql://mycosoft:REDACTED_VM_SSH_PASSWORD@192.168.0.189:5432/mindex"`
...
2. **Change VM SSH password** on all VMs (187, 188, 189) since `REDACTED_VM_SSH_PASSWORD` is in source code
3. **Change MINDEX database password** since `REDACTED_VM_SSH_PASSWORD` and `REDACTED_DB_PASSWORD` are in source code
