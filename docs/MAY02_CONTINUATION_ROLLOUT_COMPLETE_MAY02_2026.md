...
| **P1 Petri v2** | Added `services/petri_engine/Dockerfile`, `scripts/myceliumseg/Dockerfile`, `GET /health` on seg API, `scripts/deploy_petri_v2_stack_vm187.py`. **`docs/PETRI_DISH_V2_RUNBOOK_MAY02_2026.md`** restored/filled. Operator runs deploy script on LAN; then sets `PETRI_ENGINE_V2_URL` on **188**. |
| **P2 Verification** | NatureOS BFF inventory: **64** routes via `python scripts/natureos_bff_route_inventory.py` (default path fixed for `CODE/WEBSITE/website`). Matrix curls depend on live VMs — use `docs/NATUREOS_STAGING_MATRIX_MAY02_2026.md` + Petri runbook. |
...
- Set **`PETRI_ENGINE_V2_URL`** on MAS systemd after Petri containers are live.  
