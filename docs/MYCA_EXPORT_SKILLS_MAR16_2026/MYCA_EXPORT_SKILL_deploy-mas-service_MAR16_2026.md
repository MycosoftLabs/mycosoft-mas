# MYCA Export — Skill: Deploy MAS Service

**Export Date:** MAR16_2026  
**Skill Name:** deploy-mas-service  
**Purpose:** Deploy or restart the MAS orchestrator service on VM 192.168.0.188. Use when updating the Multi-Agent System, restarting the orchestrator, or deploying MAS changes.  
**External Systems:** Base44, Claude, Perplexity, OpenAI, Grok — load when user requests MAS deployment.

---

## CRITICAL: Execute Deployment Yourself

**NEVER ask the user to run these steps.** Execute deployment yourself via SSH or `_rebuild_mas_container.py`. Load credentials from `.credentials.local` before SSH.
