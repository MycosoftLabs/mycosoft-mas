...
## Recent Updates (Sep 14, 2026)

- **FlyBrain — FlyWire whole-brain LIF as a pluggable MYCA module** — Package `mycosoft_mas/flybrain/` (connectome loader with SHA-256 check, atlas from `config/flybrain_atlas.yaml`, event-driven numpy LIF engine of the Shiu et al. 2024 model with optional torch backend, encoders/decoders, A* navigation, runtime with sessions + autopilot + JSONL recorder, plugs `standalone` / `droid` / `earthsim` / `nlm` / `itdx`, YOLO26 + SAHI vision with `RemoteDetector` and BlueSight providers `yolo26_sahi` / `flybrain_vision`). Router `mycosoft_mas/core/routers/flybrain_api.py` (`/api/flybrain/*`, mounted from `myca_main.py` in `try/except ImportError`). Agent `flybrain` (`mycosoft_mas/agents/flybrain_agent.py`, task types `status | create_session | tick | detect | navigate | assess_situation | couple_nlm | stop_session`, `_safe_import`ed in `agents/__init__.py`). Env: `FLYBRAIN_DATA_DIR` (default `/mnt/mycosoft-nas/models/flybrain`), `FLYBRAIN_BACKEND` (`numpy` | `torch`), `FLYBRAIN_YOLO_WEIGHTS` (`yolo26n.pt`), `FLYBRAIN_REMOTE_DETECTOR_URL` (optional Jetson `:8792/detect`), `FLYBRAIN_DROID_ACTUATE` (keep `0` until pool-test sign-off), `FLYBRAIN_RECORD_DIR`, `FLYBRAIN_DT_MS`, `FLYBRAIN_WINDOW_MS`, `FLYBRAIN_MAX_SESSIONS`, `MINDEX_API_URL` (189:8000, unchanged). Data on NAS: FlyWire v783 `2025_Completeness_783.csv` (138,639 neurons) + `2025_Connectivity_783.npz` (15,091,983 synapses, converted from the upstream parquet) fetched once on MAS VM 188 by `scripts/flybrain_fetch_connectome.py --dest /mnt/mycosoft-nas/models/flybrain`; SHA-256 verified at fetch and at load. License: FlyWire public-release data terms (CC BY-NC 4.0 at time of writing; commercial use needs FlyWire permission) — data is **never committed** to any Mycosoft repo; engine is written from the MIT `model.py` equations, no GPL source copied from `eonsystemspbc/fly-brain`. All outputs `origin: "SIMULATED"`; no connectome → `unavailable` / 503; no detector → 503; droid actuation triple-gated (dry-run + env + AVANI). Website: owner-gated BFF `/api/fusarium/flybrain/[...path]`, panel `components/itdx/ITDXFlyBrainPanel.tsx`. Doc: `docs/FLYBRAIN_MODULE_SEP14_2026.md`. Tests: `tests/test_flybrain_*.py`.
