"""Enhance MYCA's local response generation with Mycosoft knowledge."""

import re

# The file path
file_path = r"mycosoft_mas\core\routers\voice_orchestrator_api.py"

# Read the current file
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Find and replace the _generate_local_response method
old_method = '''    def _generate_local_response(self, message: str, intent: Dict[str, Any]) -> str:
...
new_method = '''    def _generate_local_response(self, message: str, intent: Dict[str, Any]) -> str:
...
if old_method in content:
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("SUCCESS: Enhanced MYCA's local responses with Mycosoft knowledge!")
else:
    print("ERROR: Could not find the exact method to replace")
    # Try to find partial match
    if "_generate_local_response" in content:
        print("The method exists but pattern doesn't match exactly. Checking...")
