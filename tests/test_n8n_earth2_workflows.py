...
class TestMASEarth2Endpoints:
    ...
    def test_earth2_forecast_endpoint(self):
        ...
        payload = {
            ...
            "spatial_extent": {"north": 50.0, "south": 25.0, "east": -65.0, "west": -125.0},
        }

        try:
            response = requests.post(f"{MAS_API_URL}/api/earth2/forecast", json=payload, timeout=30)
    ...
    def test_earth2_spore_dispersal_endpoint(self):
        ...
        try:
            response = requests.post(
                f"{MAS_API_URL}/api/earth2/spore-dispersal", json=payload, timeout=30
            )
            assert response.status_code in [
                200,
                202,
    ...
    def test_earth2_nowcast_endpoint(self):
        ...
        payload = {
            "model": "stormscope",
            "forecast_hours": 6,
            "spatial_extent": {"north": 40.0, "south": 30.0, "east": -90.0, "west": -100.0},
        }

        try:
            response = requests.post(f"{MAS_API_URL}/api/earth2/nowcast", json=payload, timeout=30)
            assert response.status_code in [
                200,
                202,
                400,
    ...
    def test_workflow_exists(self, workflow_id):
        ...
        try:
            response = requests.get(
                f"{N8N_BASE_URL}/api/v1/workflows/{workflow_id}", headers=headers, timeout=10
            )
            ...
            assert response.status_code == 200, f"Failed to get workflow: {response.status_code}"
        except requests.exceptions.ConnectionError:
...
class TestSporeAlertWorkflow:
    ...
    def test_alert_threshold_logic(self):
        ...
        # Alert thresholds
        ALERT_THRESHOLDS = {"low": 25, "moderate": 75, "high": 125, "critical": 175}

        # Find locations exceeding high threshold
        high_risk_locations = [
            c
            for c in sample_spore_data["concentrations"]
...
class TestNowcastAlertWorkflow:
        ...
        try:
            response = requests.post(
                ...
                json={
                    ...
                    "spatial_extent": {"north": 42.0, "south": 32.0, "east": -85.0, "west": -105.0},
    ...
    def test_severe_weather_detection(self):
        ...
        severe_storms = [
            s
            for s in sample_nowcast["storm_cells"]
            if (
                s["tornado_probability"] >= SEVERE_CRITERIA["tornado_prob_threshold"]
...
class TestWorkflowIntegration:
    ...
    def test_end_to_end_weather_pipeline(self):
        ...
        results = {"forecast_requested": False, "data_received": False, "alerts_generated": False}
        ...
        try:
            ...
            alert_response = requests.get(f"{MAS_API_URL}/api/earth2/alerts", timeout=10)
        ...
        assert (
            results["forecast_requested"] or True
    ...
    def test_webhook_notification_format(self):
        ...
        notification_payload = {
            ...
            "location": {"lat": 35.0, "lon": -95.0, "region": "Central Oklahoma"},
            "data": {
                "species": "Fusarium_oxysporum",
                "concentration": 150,
