...
def base_agent():
    ...
    return BaseAgent(
        agent_id="test_agent",
        name="Test Agent",
        config={"health_check_interval": 60, "retry_interval": 300, "max_retries": 3},
    )
...
def mycology_bio_agent():
    ...
    return MycologyBioAgent(agent_id="mycology_bio", name="Mycology BioAgent", config=config)
...
def marketing_agent():
    ...
    return MarketingAgent(agent_id="marketing", name="Marketing Agent", config=config)
...
def project_manager_agent():
    ...
    return ProjectManagerAgent(
        agent_id="project_manager", name="Project Manager Agent", config=config
    )
...
def myco_dao_agent():
    ...
    return MycoDAOAgent(agent_id="myco_dao", name="MycoDAO Agent", config=config)
...
async def test_health_check(base_agent):
    ...
    with patch.object(
        base_agent, "_check_services_health", new_callable=AsyncMock
    ) as mock_services:
        with patch.object(
            base_agent, "_check_resource_usage", new_callable=AsyncMock
        ) as mock_resources:
...
async def test_process_task(base_agent):
    ...
    task = {"type": "test_task", "id": "task_123", "payload": {"data": "test"}}
...
async def test_handle_error(base_agent):
    """Test error handling."""
    error = {"type": "test_error", "id": "error_123", "error": "Test error message"}
        ...
        with patch.object(base_agent, "_process_tasks", new_callable=AsyncMock) as mock_tasks:
            with patch.object(base_agent, "_handle_errors", new_callable=AsyncMock) as mock_errors:
                with patch.object(
                    ...
                    assert all(
                        isinstance(task, asyncio.Task) for task in base_agent.background_tasks
                    )
