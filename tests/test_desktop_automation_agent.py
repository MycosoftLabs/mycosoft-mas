...
def agent():
    config = {
        ...
        "keyboard_delay": 0.1,
        "browser_profiles": {
            "default": {"browser_type": "chrome", "user_data_dir": "browser_profiles/default"}
        },
        "applications": {"test_app": {"path": "C:\\test\\app.exe", "args": []}},
    }
...
async def test_handle_message_browser(agent):
    message = Message(
        message_type=MessageType.BROWSER, content={"action": "start_browser", "profile": "default"}
    )
...
async def test_handle_message_desktop(agent):
    message = Message(
        ...
        content={"action": "start_application", "app_name": "test_app"},
    )
...
async def test_handle_message_invalid_type(agent):
    message = Message(message_type=MessageType.TASK, content={"action": "test"})
...
async def test_handle_message_missing_action(agent):
    message = Message(message_type=MessageType.BROWSER, content={"profile": "default"})
