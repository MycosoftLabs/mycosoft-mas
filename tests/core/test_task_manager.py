...
def mock_cluster():
    ...
    cluster.get_status.return_value = {"status": "running", "node_count": 1, "active_nodes": 1}
    return cluster
...
def test_load_config(task_manager):
    config = {
        "orchestrator": {"url": "http://localhost:8000", "monitoring_interval": 60},
        "clusters": {"monitoring_interval": 60},
        "dependencies": {"auto_update": True, "update_interval": 3600},
    }
...
def test_update_dependencies(task_manager):
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.assert_called_with(["poetry", "update"], capture_output=True, text=True)
