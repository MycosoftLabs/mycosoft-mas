...
class IntegrationTestSuite:
    ...
    async def test_prompt_manager(self):
        ...
        try:
            from mycosoft_mas.core.prompt_manager import (
                get_prompt_manager,
    ...
    async def test_voice_session_manager(self):
        ...
        try:
            ...
            self.log(
                f"  RTF tracking: current={session.rtf_current}, status={session.rtf_status.value}"
            )
            ...
            duration = (time.monotonic() - start) * 1000
            self.results.append(
                TestResult("Voice Session Manager", True, "All checks passed", duration)
            )
            self.log("  PASSED")
    ...
    async def test_memory_summarization(self):
        ...
        try:
            ...
            self.log(
                f"  Key info: preferences={len(info['preferences'])}, errors={len(info['errors_mentioned'])}"
            )
            ...
            duration = (time.monotonic() - start) * 1000
            self.results.append(
                TestResult("Memory Summarization", True, "All checks passed", duration)
            )
            self.log("  PASSED")
    ...
    async def test_orchestrator_chat(self):
        ...
        try:
            ...
            duration = (time.monotonic() - start) * 1000
            self.results.append(
                TestResult("Orchestrator Chat", True, "All checks passed", duration)
            )
            self.log("  PASSED")
