...
def test_update_dependency(dependency_manager):
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.assert_called_with(
            ["poetry", "update", "test_package"], capture_output=True, text=True
        )
...
def test_install_dependency(dependency_manager):
    with patch("subprocess.run") as mock_run:
        ...
        assert result["status"] == "success"
        mock_run.assert_called_with(
            ["poetry", "add", "test_package==1.0.0"], capture_output=True, text=True
        )
...
def test_uninstall_dependency(dependency_manager):
    with patch("subprocess.run") as mock_run:
        ...
        assert result["status"] == "success"
        mock_run.assert_called_with(
            ["poetry", "remove", "test_package"], capture_output=True, text=True
        )
