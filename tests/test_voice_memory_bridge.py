...
async def test_start_voice_session_with_memory_context():
    ...
    voice_session = bridge._personaplex._active_sessions.get(__import__("uuid").UUID(session_id))
...
async def test_add_voice_interaction_stores_in_all_systems():
    ...
    # Start session
    session_id = await bridge.start_voice_session(user_id="test_user", user_name="Test User")
    ...
    if memory_session_id:
        context = await bridge._coordinator.get_conversation_context(
            session_id=memory_session_id, turns=5
        )
...
async def test_get_voice_context_retrieves_from_all_sources():
    ...
    context = await bridge.get_voice_context(
        user_id="test_user", current_message="What temperature do they need?", session_id=session_id
    )
...
async def test_search_voice_memory():
    ...
    results = await bridge.search_voice_memory(user_id="test_user", query="shiitake", limit=5)

    # May not find results immediately due to async indexing
    print(f"✓ Searched voice memory, found {len(results)} results")
    if results:
        print(
            f"  - First result: {results[0].get('message', results[0].get('summary', 'N/A'))[:50]}..."
        )
...
async def test_update_voice_preference():
    ...
    success = await bridge.update_voice_preference(
        user_id="test_user", preference_key="voice_speed", preference_value=1.2
    )
...
async def test_full_voice_session_flow():
    ...
    session_id = await bridge.start_voice_session(
        user_id="morgan", user_name="Morgan Rockwell", speaker_profile="morgan_voice_profile"
    )
    ...
    interactions = [
        (
            "How are the oyster mushrooms doing?",
            "The oyster mushrooms are fruiting nicely. Humidity is at 85%.",
        ),
        (
            "What about the shiitake?",
            "Shiitake logs are in the colonization phase, about 2 weeks from pinning.",
        ),
        (
            "Any issues I should know about?",
            "All parameters are within optimal ranges. No issues detected.",
        ),
    ]
