...
async def test_base_agent_initialization(mock_config, mock_integration_service):
    agent = BaseAgent(
        agent_id=mock_config["agent_id"], name=mock_config["name"], config=mock_config
    )
...
async def test_qa_agent_initialization(mock_config, mock_integration_service):
    agent = QAAgent(agent_id=mock_config["agent_id"], name=mock_config["name"], config=mock_config)
    await agent.initialize(mock_integration_service)
    assert isinstance(agent, BaseAgent)
    assert agent.agent_id == mock_config["agent_id"]
    assert agent.name == mock_config["name"]
...
@pytest.mark.asyncio
async def test_agent_manager_register_agent(
    mock_agent_manager, mock_config, mock_integration_service
):
    agent = BaseAgent(
        agent_id=mock_config["agent_id"], name=mock_config["name"], config=mock_config
    )
...
async def test_agent_manager_create_agent(mock_agent_manager, mock_config):
    agent = await mock_agent_manager.create_agent(agent_type="project_manager", config=mock_config)
...
@pytest.mark.asyncio
async def test_agent_manager_remove_agent(
    mock_agent_manager, mock_config, mock_integration_service
):
    agent = BaseAgent(
        agent_id=mock_config["agent_id"], name=mock_config["name"], config=mock_config
